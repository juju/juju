...
`[1:],
		...
		func(result *internal.ManifestsResult, err error) {
			...
			c.Assert(result, jc.DeepEquals, &internal.ManifestsResult{Architectures: []string{"ppc64el"}})
		},
	)
}
...
func (s *baseSuite) TestGetManifestsSchemaVersion2List(c *gc.C) {
	s.assertGetManifestsSchemaVersion1(c,
		`
{
    "schemaVersion": 2,
...
`[1:],
		...
		func(result *internal.ManifestsResult, err error) {
			c.Assert(err, jc.ErrorIsNil)
			c.Assert(result, jc.DeepEquals, &internal.ManifestsResult{Architectures: []string{"amd64", "ppc64el"}})
		},
	)
}
