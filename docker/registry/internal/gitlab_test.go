		...
		gomock.InOrder(
			...
			s.mockRoundTripper.EXPECT().RoundTrip(gomock.Any()).DoAndReturn(
				func(req *http.Request) (*http.Response, error) {
					...
					c.Check(req.URL.String(), gc.Equals, `https://registry.gitlab.com/v2/`)
			...
			s.mockRoundTripper.EXPECT().RoundTrip(gomock.Any()).DoAndReturn(
				func(req *http.Request) (*http.Response, error) {
					...
					c.Check(req.Method, gc.Equals, `GET`)
					c.Check(req.URL.String(), gc.Equals, `https://gitlab.com/jwt/auth?scope=repository%3Ajujuqa%2Fjujud-operator%3Apull&service=container_registry`)
					return &http.Response{
						Request:    req,
	...
	gomock.InOrder(
		s.mockRoundTripper.EXPECT().RoundTrip(gomock.Any()).DoAndReturn(
			func(req *http.Request) (*http.Response, error) {
				c.Check(req.Header, jc.DeepEquals, http.Header{})
				c.Check(req.Method, gc.Equals, `GET`)
				c.Check(req.URL.String(), gc.Equals, `https://registry.gitlab.com/v2/jujuqa/jujud-operator/tags/list`)
				return &http.Response{
					Request:    req,
					StatusCode: http.StatusUnauthorized,
					Body:       ioutil.NopCloser(nil),
