...
# Print all units, each optionally postfixed by $2 (eg. 'myservice/0:')
...
# Print both services and units, currently used for juju status completion
_juju_services_and_units_from_file() {
    _juju_services_from_file "$@"
    _juju_units_from_file "$@"
}

# Print all juju commands
_juju_list_commands() {
    juju help commands 2>/dev/null | awk '{print $1}'
}

# Print flags for juju action, shamelessly excluding -e/--environment for
# cleaner completion for common usage cases (juju ssh <TAB>, etc)
_juju_flags_for() {
    test -z "${1}" && return 0
    juju help ${1} 2>/dev/null |egrep -o --  '(^|-)-[a-z]+'|egrep -v -- '^(-e|--environment)'|sort -u
}

# Print guessed completion function for cmd
# Guessing is done by parsing juju help <cmd>, and
# echoed for its function name (for later use by caller)
...
# Print filename from juju status cached output (if not expired),
...
# calls passed completion func, also adding flags for cmd
