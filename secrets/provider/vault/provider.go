...
func (p vaultProvider) CleanupModel(cfg *provider.ModelBackendConfig) (err error) {
	...
	if s == nil || s.Data == nil {
		logger.Warningf("vault list response missing data for mount %q during cleanup", k.mountPath)
		return sys.UnmountWithContext(ctx, k.mountPath)
	}
	keys, ok := s.Data["keys"].([]interface{})
	if !ok {
		logger.Warningf("vault list response has invalid keys type %T for mount %q during cleanup", s.Data["keys"], k.mountPath)
		return sys.UnmountWithContext(ctx, k.mountPath)
	}
	for i, idVal := range keys {
		id, ok := idVal.(string)
		if !ok {
			logger.Warningf("vault list response key at index %d has invalid type %T for mount %q during cleanup", i, idVal, k.mountPath)
			continue
		}
