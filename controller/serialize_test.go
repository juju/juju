...
type EncodeForJSONSuite struct {
	testing.IsolationSuite
}

var _ = gc.Suite(&EncodeForJSONSuite{})

func (s *EncodeForJSONSuite) TestEncodeForJSON(c *gc.C) {
	cfg := jujutesting.FakeControllerConfig()
	cfg[controller.QueryTracingThreshold] = time.Second

	encoded := controller.EncodeForJSON(cfg)

	c.Assert(encoded, gc.DeepEquals, map[string]any{
		"controller-uuid":           jujutesting.ControllerTag.Id(),
		"ca-cert":                   jujutesting.CACert,
		"state-port":                1234,
		"api-port":                  17777,
