...
The documentation is in `juju/docs`. Most pages are hand-written Markdown.
A subset are autogenerated from Go source (CLI command reference, hook command
reference, controller configuration keys) — **do not edit these directly**;
changes must be made in the Go source and then regenerated.
...
**Editing hand-written pages (tutorials, how-tos, explanations, most reference
pages):** No Go required. From `juju/docs`:

```sh
make html
```

This sets up a Python venv, installs dependencies, and builds the docs. Open
`docs/_build/index.html` in a browser to preview. For a live-reloading server:

```sh
make run
```

> Note: On an Ubuntu Cloud VM or container, add `export SPHINX_HOST=0.0.0.0`
> to your shell before running `make run`.

**Editing autogenerated content (CLI help text, hook command docs, controller
configuration key descriptions):** Install Go, make your changes to the Go
source, then regenerate the committed output files:

```sh
make regenerate-docs
```

Commit both the `.go` source change and the regenerated `.md` files together.
CI will fail the PR if the committed files do not match the generated output.
...
For detailed conventions — content placement, section structure, cross-
referencing patterns, MyST syntax — see `docs/agents/`. These files encode
the rules that reviewers check and can be used with Copilot to produce
convention-compliant drafts before submitting for review.
