...
func (s *stateSuite) TestTags(c *gc.C) {
	...
	err = apistate.Login(api.LoginParams{Tag: tag, Password: password})
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestLoginMacaroonInvalidId(c *gc.C) {
	...
	c.Assert(err, jc.ErrorIsNil)
	err = apistate.Login(api.LoginParams{Tag: tag, Macaroons: []macaroon.Slice{{mac}}})
...
func (s *stateSuite) TestLoginWithAccessToken(c *gc.C) {
	info := s.APIInfo(c)

	accessToken := "test-access-token"

	s.PatchValue(api.LoginWithAccessTokenAPICall, func(_ base.APICaller, request interface{}, response interface{}) error {
...
func (s *stateSuite) TestLoginClientCredentials(c *gc.C) {
	info := s.APIInfo(c)
...
func (s *stateSuite) TestLoginDevice(c *gc.C) {
	info := s.APIInfo(c)

	accessToken := "test-access-token"
	userCode := "test-user-code"
	verificationURL := "test-verification-url"

	s.PatchValue(api.LoginDeviceAPICall, func(_ base.APICaller, request interface{}, response interface{}) error {
		err := json.Unmarshal(
			[]byte(
				fmt.Sprintf(`{"user-code": %q, "verification-url": %q}`,
					userCode,
					verificationURL,
				),
			),
	...
	s.PatchValue(api.GetAccessTokenAPICall, func(_ base.APICaller, request interface{}, response interface{}) error {
		err := json.Unmarshal([]byte(fmt.Sprintf(`{"access-token": %q}`, accessToken)), response)
		if err != nil {
			return errors.Trace(err)
		}
		return nil
	...
	apiState, err := api.Open(&api.Info{
		...
		ControllerUUID: info.ControllerUUID,
		CACert:         info.CACert,
		ShowLoginDetails: func(s string) error {
			loginDetails = s
			return nil
		},
	}, api.DialOpts{})
	...
	c.Assert(
		loginDetails,
		gc.Equals,
		fmt.Sprintf("Please visit %s and enter code %s to log in.", verificationURL, userCode),
	)
}
