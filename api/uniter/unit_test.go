...
func (s *unitSuite) TestPublicAddress(c *gc.C) {
	...
	err = s.wordpressMachine.SetProviderAddresses(network.NewAddress("1.2.3.4", network.ScopePublic))
	c.Assert(err, jc.ErrorIsNil)
...
func (s *unitSuite) TestPrivateAddress(c *gc.C) {
	...
	err = s.wordpressMachine.SetProviderAddresses(network.NewAddress("1.2.3.4", network.ScopeCloudLocal))
	c.Assert(err, jc.ErrorIsNil)
...
func (s *unitSuite) TestWatchAddresses(c *gc.C) {
	...
	// Update config a couple of times, check a single event.
	err = s.wordpressMachine.SetProviderAddresses(network.NewAddress("0.1.2.3", network.ScopeUnknown))
	c.Assert(err, jc.ErrorIsNil)
	err = s.wordpressMachine.SetProviderAddresses(network.NewAddress("0.1.2.4", network.ScopeUnknown))
	c.Assert(err, jc.ErrorIsNil)
	wc.AssertOneChange()
