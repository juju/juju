...
func verifyCAMulti(ctx context.Context, addrs []*url.URL, opts *dialOpts) error {
	...
	tryRetrieveCaCertFn := func(ctx context.Context, addr *resolvedAddress) func(<-chan struct{}) (io.Closer, error) {
		...
		return func(<-chan struct{}) (io.Closer, error) {
			...
			logger.Debugf(ctx, "api ca probe succeeded: address=%s server=%s proxy=%s elapsed=%s", ipStr, addr.url.Host, debugProxy(proxyURL), time.Since(started).Round(time.Millisecond))
...
func (d dialer) dial(done <-chan struct{}) (io.Closer, error) {
	...
	for a.Next() {
		attempt++
		logger.Debugf(d.ctx, "api dial attempt starting: url=%s ip=%s attempt=%d proxy=%s", d.addr.String(), d.ipAddr, attempt, debugProxy(proxyURL))
		started := time.Now()
		...
		cancelMutex.Unlock()
		if err == nil {
			logger.Debugf(d.ctx, "api dial attempt succeeded: url=%s ip=%s attempt=%d proxy=%s elapsed=%s", d.addr.String(), d.ipAddr, attempt, debugProxy(proxyURL), time.Since(started).Round(time.Millisecond))
			return &dialResult{
		...
		ctxCancel()
		if !errors.Is(err, context.Canceled) {
			logger.Debugf(d.ctx, "api dial attempt failed: url=%s address=%s ip=%s attempt=%d proxy=%s elapsed=%s err=%v", d.addr.String(), d.controllerRoot.String(), d.ipAddr, attempt, debugProxy(proxyURL), time.Since(started).Round(time.Millisecond), err)
		}
		lastErr = err
		if isX509Error(err) {
			break
