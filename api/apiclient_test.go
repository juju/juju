...
func (s *apiclientSuite) TestVerifyCAUsesProxyForCAProbe(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, "unable to connect to API: boom")
	c.Assert(atomic.LoadInt32(&proxyConnectCount), tc.Equals, int32(1))
	c.Assert(atomic.LoadInt32(&targetConnCount), tc.Equals, int32(1))
}
