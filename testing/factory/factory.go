...
func (factory *Factory) MakeApplication(c *gc.C, params *ApplicationParams) *state.Application {
	...
	service, err := factory.st.AddApplication(state.AddApplicationArgs{
		Name:        params.Name,
...
func (factory *Factory) MakeUnitReturningPassword(c *gc.C, params *UnitParams) (*state.Unit, string) {
	...
	if params.SetCharmURL {
		serviceCharmURL, _ := params.Application.CharmURL()
...
func (factory *Factory) MakeMetric(c *gc.C, params *MetricParams) *state.MetricBatch {
	...
	if params.Unit == nil {
		...
		meteredService := factory.MakeApplication(c, &ApplicationParams{Charm: meteredCharm})
		params.Unit = factory.MakeUnit(c, &UnitParams{Application: meteredService, SetCharmURL: true})
	}
	if params.Time == nil {
		params.Time = &now
	}
