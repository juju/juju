...
func (s *MultiModelRunnerSuite) TestRunTransaction(c *gc.C) {
	for i, t := range getTestCases() {
		...
		err := s.multiModelRunner.RunTransaction(inOps, 0)
...
func (s *MultiModelRunnerSuite) TestRejectsAttemptToInsertWrongModelUUID(c *gc.C) {
	...
	err := s.multiModelRunner.RunTransaction(ops, 0)
	c.Assert(err, jc.ErrorIsNil)
...
func (r *recordingRunner) RunTransaction(ops []txn.Op, attempt int) error {
	r.seenOps = ops
	r.seenAttempt = attempt
	return nil
}
