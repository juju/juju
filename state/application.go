...
func serviceSettingsKey(applicationname string, curl *charm.URL) string {
...
func (s *Application) addServiceUnitOps(args serviceAddUnitOpsArgs) (string, []txn.Op, error) {
	names, ops, err := s.addUnitOpsWithCons(args)
	if err == nil {
...
func (s *Application) addUnitOpsWithCons(args serviceAddUnitOpsArgs) (string, []txn.Op, error) {
...
func settingsIncRefOp(st *State, applicationname string, curl *charm.URL, canCreate bool) (txn.Op, error) {
	...
	key := serviceSettingsKey(applicationname, curl)
...
func (s *Application) Status() (status.StatusInfo, error) {
	...
	if count, err := query.Count(); err != nil {
		...
		// charm's hooks exists or sets a application status*. This logic should be
...
// addApplicationOps returns the operations required to add a application to the
