...
// This is used on CAAS models.
func (a *Application) ChangeScale(scaleChange int, attachStorage []string) (int, error) {
	...
	// Convert attach storage ids to storage tags.
	storageTags := make([]names.StorageTag, len(attachStorage))
	for i, stor := range attachStorage {
		if names.IsValidStorage(stor) {
			storageTags[i] = names.NewStorageTag(stor)
		} else {
			return a.doc.DesiredScale, errors.NotValidf("storage name %q", stor)
		}
	}
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		if scaleChange > 0 && len(storageTags) > 0 {
			// Since len(storageTags) will always equal to one at this step,
			// the unit OrderedId will be equal to a.doc.UnitCount.
			unitName := a.doc.Name + "/" + strconv.Itoa(a.doc.UnitCount)

			// The operations return should equal to a.insertCAASUnitOps but
			// use different OrderedId check since the desired scale
			// number haven't been updated.
			insertUnitOps, err := a.insertCAASUnitOps(
				UpsertCAASUnitParams{
					AddUnitParams: AddUnitParams{
						UnitName:      &unitName,
				...
				func() error {
					if ps := a.ProvisioningState(); ps != nil && ps.Scaling && a.doc.UnitCount != ps.ScaleTarget+1 {
						return errors.NotAssignedf("Can not scale application because there's already a scaling operation in progress")
					}
					return nil
				},
			)
