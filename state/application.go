...
func (a *Application) validateSetCharmConfig(cfg SetCharmConfig) error {
	...
	} else if err := cfg.CharmOrigin.validate(); err != nil {
		return errors.Annotatef(err, "Charm origin validation failed")
	}
...
func checkSeriesForSetCharm(currentPlatform *Platform, charm *Charm, ForceBase bool) error {
	...
	if charm.URL().Series != "" {
		...
		if charm.URL().Schema != "ch" && charm.URL().Series != corebase.Kubernetes.String() && charm.URL().Series != curSeries {
			return errors.Errorf("cannot change an application's series")
		}
	} else if !ForceBase {
		supported := false
		for _, oneSeries := range charmSeries {
			if oneSeries == corebase.Kubernetes.String() {
				oneSeries = corebase.LegacyKubernetesSeries()
			}
