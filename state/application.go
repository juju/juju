...
func (op *DestroyApplicationOperation) Done(err error) error {
	if err == nil {
		if err := op.eraseHistory(); err != nil {
			if !op.Force {
				logger.Errorf(context.TODO(), "cannot delete history for application %q: %v", op.app, err)
			}
	...
	if op.app.doc.UnitCount == 0 && op.app.doc.RelationCount == removeCount {
		...
		if op.app.doc.HasResources && !op.CleanupIgnoringResources {
			if op.Force {
				...
				logger.Debugf(context.TODO(), "scheduling forced application %q cleanup", op.app.doc.Name)
			...
			logger.Debugf(context.TODO(), "advancing application %q to dead, waiting for cluster resources", op.app.doc.Name)
...
func (a *Application) changeCharmOps(
	...
	if oldKey != nil {
		...
		if len(op.Errors) != 0 {
			logger.Errorf(context.TODO(), "could not remove old charm references for %v:%v", oldKey, op.Errors)
		}
	}
...
func (a *Application) ClearResources() error {
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		logger.Debugf(context.TODO(), "application %q now has no cluster resources, scheduling cleanup", a.doc.Name)
		cleanupOp := newCleanupOp(
			cleanupApplication,
			a.doc.Name,
			false, // force
...
func (a *Application) LeaderSettings() (map[string]string, error) {
	...
	for escapedKey, interfaceValue := range doc.Settings {
		...
		if value, _ := interfaceValue.(string); value != "" {
			...
			logger.Warningf(context.TODO(), "unexpected leader settings value for %s: %#v", key, interfaceValue)
		}
	}
	return result, nil
}
