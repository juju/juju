...
func (s *StateSuite) TestAddService(c *gc.C) {
	...
	_, err := s.State.AddService("haha/borken", "user-admin", charm, []string{}, []string{})
	...
	_, err = s.State.AddService("umadbro", "user-admin", nil, []string{}, []string{})
	c.Assert(err, gc.ErrorMatches, `cannot add service "umadbro": charm is nil`)

	wordpress, err := s.State.AddService("wordpress", "user-admin", charm, []string{}, []string{})
	c.Assert(err, gc.IsNil)
	c.Assert(wordpress.Name(), gc.Equals, "wordpress")
	mysql, err := s.State.AddService("mysql", "user-admin", charm, []string{}, []string{})
	c.Assert(err, gc.IsNil)
...
func (s *StateSuite) TestAddServiceEnvironmentDying(c *gc.C) {
	...
	err = env.Destroy()
	c.Assert(err, gc.IsNil)
	_, err = s.State.AddService("s1", "user-admin", charm, []string{}, []string{})
...
func (s *StateSuite) TestAddServiceNoTag(c *gc.C) {
	charm := s.AddTestingCharm(c, "dummy")
	_, err := s.State.AddService("wordpress", state.AdminUser, charm, []string{}, []string{})
...
func (s *StateSuite) TestAddServiceNotUserTag(c *gc.C) {
	charm := s.AddTestingCharm(c, "dummy")
	_, err := s.State.AddService("wordpress", "machine-3", charm, []string{}, []string{})
...
func (s *StateSuite) TestAddServiceNonExistentUser(c *gc.C) {
	charm := s.AddTestingCharm(c, "dummy")
	_, err := s.State.AddService("wordpress", "user-notAuser", charm, []string{}, []string{})
...
func (s *StateSuite) TestAllServices(c *gc.C) {
	...
	_, err = s.State.AddService("wordpress", "user-admin", charm, []string{}, []string{})
	c.Assert(err, gc.IsNil)
	services, err = s.State.AllServices()
	c.Assert(err, gc.IsNil)
	c.Assert(len(services), gc.Equals, 1)

	_, err = s.State.AddService("mysql", "user-admin", charm, []string{}, []string{})
	c.Assert(err, gc.IsNil)
	services, err = s.State.AllServices()
	c.Assert(err, gc.IsNil)
...
func (s *StateSuite) TestSetEnvironAgentVersionErrors(c *gc.C) {
	...
	service, err := s.State.AddService("wordpress", "user-admin", s.AddTestingCharm(c, "wordpress"), []string{}, []string{})
	c.Assert(err, gc.IsNil)
