...
func (s *StateSuite) TestDialAgain(c *gc.C) {
	...
	for i := 0; i < 2; i++ {
		st, err := state.Open(state.TestingStateInfo(), state.TestingDialOpts(), state.PolicyBase{})
		c.Assert(err, gc.IsNil)
...
func tryOpenState(info *state.Info) error {
	st, err := state.Open(info, state.TestingDialOpts(), state.PolicyBase{})
	if err == nil {
		st.Close()
	}
	return err
...
func testSetMongoPassword(c *gc.C, getEntity func(st *state.State) (entity, error)) {
	...
	info.Password = "foo"
	st1, err := state.Open(info, state.TestingDialOpts(), state.PolicyBase{})
	c.Assert(err, gc.IsNil)
	defer st1.Close()
