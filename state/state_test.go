...
func (s *StateSuite) TestSetAPIHostPortsNoMgmtSpace(c *gc.C) {
	addrs, err := s.State.APIHostPortsForClients(s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)
	...
	err = s.State.SetAPIHostPorts(newHostPorts, s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)

	ctrlSt, err := s.StatePool.SystemState()
	c.Assert(err, jc.ErrorIsNil)
	gotHostPorts, err := ctrlSt.APIHostPortsForClients(s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(gotHostPorts, jc.DeepEquals, newHostPorts)

	gotHostPorts, err = ctrlSt.APIHostPortsForAgents(s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(gotHostPorts, jc.DeepEquals, newHostPorts)
...
func (s *StateSuite) TestSetAPIHostPortsNoMgmtSpaceConcurrentDifferent(c *gc.C) {
	...
	var prevAgentsRevno int64
	defer state.SetBeforeHooks(c, s.State, func() {
		err := s.State.SetAPIHostPorts([]network.SpaceHostPorts{hostPorts0}, s.ControllerConfig)
		c.Assert(err, jc.ErrorIsNil)
		revno, err := state.TxnRevno(s.State, ctrC, "apiHostPorts")
		c.Assert(err, jc.ErrorIsNil)
		prevRevno = revno
	...
	err := s.State.SetAPIHostPorts([]network.SpaceHostPorts{hostPorts1}, s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(prevRevno, gc.Not(gc.Equals), 0)
	...
	ctrlSt, err := s.StatePool.SystemState()
	c.Assert(err, jc.ErrorIsNil)
	hostPorts, err := ctrlSt.APIHostPortsForClients(s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(hostPorts, gc.DeepEquals, []network.SpaceHostPorts{hostPorts1})

	hostPorts, err = ctrlSt.APIHostPortsForAgents(s.ControllerConfig)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(hostPorts, gc.DeepEquals, []network.SpaceHostPorts{hostPorts1})
}
