...
func (s *StateSuite) TestAddAndGetEquivalence(c *C) {
	...
	charm1 := s.AddTestingCharm(c, "riak")
	charm2, err := s.State.Charm(charm1.URL())
	c.Assert(err, IsNil)
	c.Assert(charm1, DeepEquals, charm2)

	service1, err := s.State.AddService("riak", charm1)
	c.Assert(err, IsNil)
	service2, err := s.State.Service("riak")
	c.Assert(err, IsNil)
	c.Assert(service1, DeepEquals, service2)

	unit1, err := service1.AddUnit()
	c.Assert(err, IsNil)
	unit2, err := s.State.Unit("riak/0")
	c.Assert(err, IsNil)
	c.Assert(unit1, DeepEquals, unit2)

	ep, err := service1.Endpoint("ring")
	c.Assert(err, IsNil)
	relation1, err := s.State.AddRelation(ep)
	c.Assert(err, IsNil)
	relation2, err := s.State.EndpointsRelation(ep)
