...
func (s *VolumeStateSuite) TestAddApplicationDefaultPool(c *gc.C) {
	...
	app := s.AddTestingApplicationWithStorage(c, "storage-block", ch, testStorage, map[string]any{
...
func (s *VolumeStateSuite) TestWatchMachineVolumeAttachments(c *gc.C) {
	...
	addUnit := func(to *state.Machine) (u *state.Unit, m *state.Machine) {
		...
		c.Assert(err, jc.ErrorIsNil)
		if to != nil {
			err = u.AssignToMachine(state.StubModelConfigService(c), to)
			c.Assert(err, jc.ErrorIsNil)
			return u, to
		}
		err = s.State.AssignUnit(state.StubModelConfigService(c), u, state.AssignNew)
...
func (s *VolumeStateSuite) assertCreateVolumes(c *gc.C) (_ *state.Machine, all, persistent []names.VolumeTag) {
	machine, err := s.State.AddOneMachine(state.StubModelConfigService(c), state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
...
func (s *VolumeStateSuite) TestEnsureMachineDeadAddVolumeConcurrently(c *gc.C) {
	...
	addVolume := func() {
		...
		err := u.AssignToMachine(state.StubModelConfigService(c), machine)
		c.Assert(err, jc.ErrorIsNil)
...
func removeVolumeStorageInstance(c *gc.C, sb *state.StorageConfigBackend, volumeTag names.VolumeTag) {
...
func removeStorageInstance(c *gc.C, sb *state.StorageConfigBackend, storageTag names.StorageTag) {
