...
func (s *ModelSummariesSuite) TestContainsMachineInformation(c *gc.C) {
	...
	twocores := uint64(2)
	threecores := uint64(3)
	m0, err := shared.AddMachine(defaultInstancePrechecker, state.UbuntuBase("12.10"), status.NoopStatusHistoryRecorder, state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	...
	c.Assert(err, jc.ErrorIsNil)
	m1, err := shared.AddMachine(defaultInstancePrechecker, state.UbuntuBase("12.10"), status.NoopStatusHistoryRecorder, state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	err = m1.SetInstanceInfo("i-45678", "", "nonce", &instance.HardwareCharacteristics{
		CpuCores: &twocores,
	}, nil, nil, nil, nil, nil, status.NoopStatusHistoryRecorder)
	c.Assert(err, jc.ErrorIsNil)
	m2, err := shared.AddMachine(defaultInstancePrechecker, state.UbuntuBase("12.10"), status.NoopStatusHistoryRecorder, state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	...
	c.Assert(err, jc.ErrorIsNil)
	// No instance
	_, err = shared.AddMachine(defaultInstancePrechecker, state.UbuntuBase("12.10"), status.NoopStatusHistoryRecorder, state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	// Dying instance, should not count to Cores or Machine count
	mDying, err := shared.AddMachine(defaultInstancePrechecker, state.UbuntuBase("12.10"), status.NoopStatusHistoryRecorder, state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	err = mDying.SetInstanceInfo("i-78901", "", "nonce", &instance.HardwareCharacteristics{
		CpuCores: &threecores,
	}, nil, nil, nil, nil, nil, status.NoopStatusHistoryRecorder)
	c.Assert(err, jc.ErrorIsNil)
	err = mDying.Destroy(state.NewObjectStore(c, shared.State.ModelUUID()), status.NoopStatusHistoryRecorder)
	c.Assert(err, jc.ErrorIsNil)
	// Instance data, but no core count
	m4, err := shared.AddMachine(defaultInstancePrechecker, state.UbuntuBase("12.10"), status.NoopStatusHistoryRecorder, state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
