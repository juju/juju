...
func (s *InitializeSuite) openState(c *gc.C) {
	st, err := state.Open(state.TestingStateInfo(), state.TestingDialOpts(), policy.Policy(nil))
...
func (s *InitializeSuite) TestInitialize(c *gc.C) {
	...
	st, err := state.Initialize(state.TestingStateInfo(), cfg, state.TestingDialOpts(), policy.Policy(nil))
	c.Assert(err, gc.IsNil)
...
func (s *InitializeSuite) TestDoubleInitializeConfig(c *gc.C) {
	cfg := testing.EnvironConfig(c)
	initial := cfg.AllAttrs()
	st := state.TestingInitialize(c, cfg, policy.Policy(nil))
	st.Close()
...
func (s *InitializeSuite) TestEnvironConfigWithAdminSecret(c *gc.C) {
	...
	_, err = state.Initialize(state.TestingStateInfo(), bad, state.TestingDialOpts(), policy.Policy(nil))
	c.Assert(err, gc.ErrorMatches, "admin-secret should never be written to the state")

	// admin-secret blocks UpdateEnvironConfig.
	st := state.TestingInitialize(c, good, policy.Policy(nil))
	st.Close()
