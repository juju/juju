...
func (s *MachineSuite) SetUpTest(c *C) {
	...
	var err error
	s.machine, err = s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
}
...
func (s *MachineSuite) TestDestroyMachines(c *C) {
	...
	c.Assert(err, IsNil)
	m2, err := s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
...
func (s *MachineSuite) TestMachineRefresh(c *C) {
	m0, err := s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
...
func (s *MachineSuite) TestMachinePrincipalUnits(c *C) {
	...
	m2, err := s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
	m3, err := s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
...
func (s *MachineSuite) TestConstraintsFromEnvironment(c *C) {
	...
	c.Assert(err, IsNil)
	machine1, err := s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	machine2, err := s.State.AddMachine("series", nil, state.JobHostUnits)
	c.Assert(err, IsNil)
