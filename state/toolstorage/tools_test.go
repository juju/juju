...
func (s *ToolsSuite) TestTools(c *gc.C) {
	...
	err = s.managedStorage.PutForModel("my-uuid", "path", strings.NewReader("blah"), 4)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *ToolsSuite) TestAddToolsRemovesExisting(c *gc.C) {
	...
	_, _, err = s.managedStorage.GetForModel("my-uuid", "path")
	c.Assert(err, jc.Satisfies, errors.IsNotFound)
...
func (s *ToolsSuite) TestAddToolsRemovesExistingRemoveFails(c *gc.C) {
	...
	r, _, err := s.managedStorage.GetForModel("my-uuid", "path")
	c.Assert(err, jc.ErrorIsNil)
	r.Close()
...
func (s *ToolsSuite) TestAddToolsConcurrent(c *gc.C) {
	...
	addMetadata := func() {
		...
		c.Assert(err, jc.ErrorIsNil)
		r, _, err := s.managedStorage.GetForModel("my-uuid", fmt.Sprintf("tools/%s-0", current))
		c.Assert(err, jc.ErrorIsNil)
		r.Close()
	}
	...
	_, _, err = s.managedStorage.GetForModel("my-uuid", fmt.Sprintf("tools/%s-0", current))
	c.Assert(err, jc.Satisfies, errors.IsNotFound)
