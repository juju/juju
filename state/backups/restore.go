...
func newStateConnection(
	controllerTag names.ControllerTag,
	modelTag names.ModelTag,
	info *mongo.MongoInfo,
	providerRegistry *environs.ProviderRegistry) (*state.State, error) {
	...
	getEnviron := stateenvirons.GetNewEnvironFunc(providerRegistry.NewEnviron)
	...
	for a := attempt.Start(); a.Next(); {
		st, err = state.Open(state.OpenParams{
			...
			NewPolicy:          environsGetNewPolicyFunc(getEnviron, providerRegistry),
		})
		if err == nil {
			return st, nil
		}
