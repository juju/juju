...
func (s *mongoRestoreSuite) TestMongoRestoreArgsForVersion(c *gc.C) {
	...
	c.Assert(args[1], gc.Equals, "--journal")
	...
	c.Assert(args[0], gc.Equals, "--drop")
	c.Assert(args[1], gc.Equals, "--journal")
	c.Assert(args[2], gc.Equals, "--oplogReplay")
	c.Assert(args[3], gc.Equals, "--dbpath")
	c.Assert(args[4], jc.SamePath, dir)
	c.Assert(args[5], gc.Equals, "/some/fake/path")
