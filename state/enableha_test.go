...
func (s *EnableHASuite) TestHasVote(c *gc.C) {
	controller, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("18.04"), state.JobHostUnits, state.JobManageModel)
...
func (s *EnableHASuite) TestEnableHAFailsWithBadCount(c *gc.C) {
	for _, n := range []int{-1, 2, 6} {
		changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), n, constraints.Value{}, state.Base{}, nil)
		c.Assert(err, gc.ErrorMatches, "number of controllers must be odd and non-negative")
		c.Assert(changes.Added, gc.HasLen, 0)
		c.Assert(units, gc.HasLen, 0)
	}
	_, _, err := s.State.EnableHA(state.StubModelConfigService(c), controller.MaxPeers+2, constraints.Value{}, state.Base{}, nil)
...
func (s *EnableHASuite) TestEnableHAAddsNewMachines(c *gc.C) {
	ids := make([]string, 3)
	m0, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("18.04"), state.JobHostUnits, state.JobManageModel)
	c.Assert(err, jc.ErrorIsNil)
	ids[0] = m0.Id()

	// Add a non-controller machine just to make sure.
	_, err = s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("18.04"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	...
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 3, cons, state.UbuntuBase("18.04"), nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *EnableHASuite) TestEnableHAAddsControllerCharm(c *gc.C) {
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 3, constraints.Value{}, state.UbuntuBase("18.04"), nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *EnableHASuite) TestEnableHAAddsControllerCharmToPromoted(c *gc.C) {
	m0, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("18.04"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 3, constraints.Value{}, state.UbuntuBase("18.04"), []string{"0"})
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(changes.Added, gc.HasLen, 2)
...
func (s *EnableHASuite) TestEnableHATo(c *gc.C) {
	...
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 3, constraints.Value{}, state.UbuntuBase("18.04"), []string{"1", "2"})
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(changes.Added, gc.HasLen, 0)
...
func (s *EnableHASuite) TestEnableHAToPartial(c *gc.C) {
	...
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 3, constraints.Value{}, state.UbuntuBase("18.04"), []string{"1"})
	c.Assert(err, jc.ErrorIsNil)
...
func (s *EnableHASuite) TestEnableHASamePlacementAsNewCount(c *gc.C) {
	...
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 3, constraints.Value{}, state.UbuntuBase("18.04"), placement)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(changes.Added, gc.HasLen, 3)
...
func (s *EnableHASuite) TestEnableHADefaultsTo3(c *gc.C) {
	changes, units, err := s.State.EnableHA(state.StubModelConfigService(c), 0, constraints.Value{}, state.UbuntuBase("18.04"), nil)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(changes.Added, gc.HasLen, 3)
	s.assertControllerInfo(c, []string{"0", "1", "2"}, []string{"0", "1", "2"}, nil)
	c.Assert(units, jc.SameContents, []string{"controller/0", "controller/1", "controller/2"})
...
func (s *EnableHASuite) progressControllerToDead(c *gc.C, id string) {
	...
	c.Assert(m.Destroy(nil), jc.ErrorIsNil)
	c.Assert(node.Refresh(), jc.ErrorIsNil)
...
func (s *EnableHASuite) TestEnableHAGoesToNextOdd(c *gc.C) {
	...
	changes, _, err = s.State.EnableHA(state.StubModelConfigService(c), 0, constraints.Value{}, state.UbuntuBase("18.04"), nil)
	c.Assert(err, jc.ErrorIsNil)
	c.Check(changes.Added, gc.HasLen, 0)
	s.assertControllerInfo(c, []string{"1", "2", "3"}, []string{"1", "2", "3"}, nil)
	// Now if we go up to 5, and drop down to 4, we should again go to 5
	changes, _, err = s.State.EnableHA(state.StubModelConfigService(c), 5, constraints.Value{}, state.UbuntuBase("18.04"), nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *EnableHASuite) TestEnableHAConcurrentSame(c *gc.C) {
	defer state.SetBeforeHooks(c, s.State, func() {
		changes, _, err := s.State.EnableHA(state.StubModelConfigService(c), 3, constraints.Value{}, state.UbuntuBase("18.04"), nil)
		c.Assert(err, jc.ErrorIsNil)
...
func (s *EnableHASuite) TestWatchControllerInfo(c *gc.C) {
	_, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("18.04"), state.JobManageModel)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *EnableHASuite) TestRemoveControllerMachineOneMachine(c *gc.C) {
	m0, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("18.04"), state.JobManageModel)
	c.Assert(err, jc.ErrorIsNil)
	node, err := s.State.ControllerNode(m0.Id())
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(node.SetHasVote(true), jc.ErrorIsNil)
...
func (s *EnableHASuite) TestRemoveControllerMachine(c *gc.C) {
	...
	s.assertControllerInfo(c, []string{"0", "1", "2"}, []string{"0", "1", "2"}, nil)
	c.Assert(m0.Destroy(nil), jc.ErrorIsNil)
	c.Assert(node.SetHasVote(false), jc.ErrorIsNil)
	c.Assert(node.Refresh(), jc.ErrorIsNil)
	err = s.State.RemoveControllerReference(node)
	c.Assert(err, jc.ErrorIsNil)
