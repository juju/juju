...
func (s *TopologySuite) TestAssignUnitToMachine(c *C) {
	...
	_, err = s.t.AddUnit("s-0", "u-0", "")
	c.Assert(err, IsNil)
	err = s.t.AssignUnitToMachine("s-0", "u-0", "m-0")
	c.Assert(err, IsNil)

	// Check that we cannot assign subordinate units to machines
	err = s.t.AddMachine("m-1")
	c.Assert(err, IsNil)
	err = s.t.AddService("s-1", "rsyslog")
	c.Assert(err, IsNil)
	_, err = s.t.AddUnit("s-1", "u-1", "u-0")
	c.Assert(err, IsNil)
	err = s.t.AssignUnitToMachine("s-1", "u-1", "m-0")
...
func (s *TopologySuite) TestAddUnitSubordinate(c *C) {
	...
	err = s.t.AddService("s-1", "rsyslog")
	c.Assert(err, IsNil)
	_, err = s.t.AddUnit("s-0", "u-05", "")
	c.Assert(err, IsNil)
	_, err = s.t.UnitPrincipalKey("s-0", "u-05")
	c.Assert(err, Equals, unitNotSubordinate)
	_, err = s.t.AddUnit("s-0", "u-12", "")
	c.Assert(err, IsNil)
	_, err = s.t.AddUnit("s-1", "u-07", "u-05")
	principal, err := s.t.UnitPrincipalKey("s-1", "u-07")
	c.Assert(err, IsNil)
	c.Assert(principal, Equals, "u-05")
}
...
func (s *TopologySuite) TestRelation(c *C) {
	...
	s.t.AddRelation("r-1", &topoRelation{
		Interface: "ifce",
		Scope:     ScopeGlobal,
		Services:  map[string]*topoRelationService{"s-p": &topoRelationService{RolePeer, "cache"}},
	})
	relation, err = s.t.Relation("r-1")
	c.Assert(err, IsNil)
	c.Assert(relation, NotNil)
	c.Assert(relation.Services["s-p"].RelationRole, Equals, RolePeer)
}
...
func (s *TopologySuite) TestAddRelation(c *C) {
	...
	err = s.t.AddRelation("r-1", &topoRelation{
		...
		Services: map[string]*topoRelationService{
			"s-p": &topoRelationService{RoleProvider, "db"},
			"s-r": &topoRelationService{RoleRequirer, "db"},
		},
	})
	c.Assert(err, IsNil)
	relation, err = s.t.Relation("r-1")
	c.Assert(err, IsNil)
	c.Assert(relation, NotNil)
	c.Assert(relation.Services["s-p"].RelationRole, Equals, RoleProvider)
	c.Assert(relation.Services["s-r"].RelationRole, Equals, RoleRequirer)
	...
	err = s.t.AddRelation("r-5", &topoRelation{
		...
		Services: map[string]*topoRelationService{
			"s-p": &topoRelationService{RoleProvider, "db"},
			"s-r": &topoRelationService{RolePeer, "db"},
		},
	})
	c.Assert(err, ErrorMatches, `relation has provider but no requirer`)
	...
	err = s.t.AddRelation("r-6", &topoRelation{
		...
		Services: map[string]*topoRelationService{
			"s-p": &topoRelationService{RoleProvider, "db"},
			"s-r": &topoRelationService{RoleRequirer, "db"},
			"s-e": &topoRelationService{RolePeer, "db"},
		},
	})
...
func (s *TopologySuite) TestRelationKeys(c *C) {
	...
	s.t.AddRelation("r-1", &topoRelation{
		...
		Scope:     ScopeGlobal,
		Services: map[string]*topoRelationService{
			"s-p": &topoRelationService{RolePeer, "cache"},
		},
	})
	keys = s.t.RelationKeys()
...
func (s *TopologySuite) TestPeerRelationKeyEndpoints(c *C) {
	...
	s.t.AddRelation("r-0", &topoRelation{
		...
		Scope:     ScopeGlobal,
		Services: map[string]*topoRelationService{
			"s-p": &topoRelationService{RolePeer, "ring"},
		},
	})
	s.t.AddRelation("r-1", &topoRelation{
		Interface: "ifce2",
