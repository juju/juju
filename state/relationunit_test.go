...
func (s *RelationUnitSuite) TestSettingsAddressWithSpaces(c *gc.C) {
...
func newProReqRelationWithBindings(c *gc.C, s *ConnSuite, scope charm.RelationScope, pbindings, rbindings map[string]string) *ProReqRelation {
	papp := s.AddTestingServiceWithBindings(c, "mysql", s.AddTestingCharm(c, "mysql"), pbindings)
	var rapp *state.Application
	if scope == charm.ScopeGlobal {
		rapp = s.AddTestingServiceWithBindings(c, "wordpress", s.AddTestingCharm(c, "wordpress"), rbindings)
	} else {
		rapp = s.AddTestingServiceWithBindings(c, "logging", s.AddTestingCharm(c, "logging"), rbindings)
	}
	return newProReqRelationForApps(c, s, papp, rapp)
}
