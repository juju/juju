...
func (s *AssignSuite) TestAssignUnitToMachineAgainFails(c *gc.C) {
	...
	// machine.
	machineOne, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	machineTwo, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)

	err = unit.AssignToMachine(state.StubModelConfigService(c), machineOne)
	c.Assert(err, jc.ErrorIsNil)
	...
	err = unit.AssignToMachine(state.StubModelConfigService(c), machineTwo)
...
func (s *AssignSuite) TestAssignSubordinatesToMachine(c *gc.C) {
	...
	err = subUnit.AssignToMachine(state.StubModelConfigService(c), machine)
	c.Assert(err, gc.ErrorMatches, `cannot assign unit "logging/0" to machine 0: unit is a subordinate`)
	err = subUnit.AssignToNewMachine(state.StubModelConfigService(c))
...
func (s *AssignSuite) TestAssignBadSeries(c *gc.C) {
	machine, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("22.10"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	unit, err := s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
	err = unit.AssignToMachine(state.StubModelConfigService(c), machine)
...
func (s *AssignSuite) TestAssignMachineDifferentSeries(c *gc.C) {
	machine, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("22.04"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
	unit, err := s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
	err = unit.AssignToMachine(state.StubModelConfigService(c), machine)
...
func (s *AssignSuite) TestAssignUnitToNewMachineCleanAvailable(c *gc.C) {
	...
	// Add a clean machine.
	clean, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *AssignSuite) TestAssignUnitToNewMachineBecomesDirty(c *gc.C) {
	_, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobManageModel) // bootstrap machine
	c.Assert(err, jc.ErrorIsNil)
	...
	unit, err := s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
	anotherUnit, err := s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
	machine, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, jc.ErrorIsNil)

	makeDirty := jujutxn.TestHook{
		Before: func() { c.Assert(unit.AssignToMachine(state.StubModelConfigService(c), machine), gc.IsNil) },
	}
	defer state.SetTestHooks(c, s.State, makeDirty).Check()

	err = anotherUnit.AssignToNewMachine(state.StubModelConfigService(c))
	c.Assert(err, jc.ErrorIsNil)
	mid, err := unit.AssignedMachineId()
	c.Assert(err, jc.ErrorIsNil)
...
func (s *AssignSuite) TestAssignUnitBadPolicy(c *gc.C) {
	...
	err = s.State.AssignUnit(state.StubModelConfigService(c), unit, state.AssignmentPolicy("random"))
...
func (s *AssignSuite) TestAssignUnitLocalPolicy(c *gc.C) {
	m, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobManageModel, state.JobHostUnits) // bootstrap machine
	c.Assert(err, jc.ErrorIsNil)
	unit, err := s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)

	for i := 0; i < 2; i++ {
		err = s.State.AssignUnit(state.StubModelConfigService(c), unit, state.AssignLocal)
		c.Assert(err, jc.ErrorIsNil)
		mid, err := unit.AssignedMachineId()
		c.Assert(err, jc.ErrorIsNil)
...
func (s *AssignSuite) assertAssignUnitNewPolicyNoContainer(c *gc.C) {
	_, err := s.State.AddMachine(state.StubModelConfigService(c), state.UbuntuBase("12.10"), state.JobHostUnits) // available machine
	c.Assert(err, jc.ErrorIsNil)
	unit, err := s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
...
func (s *AssignSuite) TestAssignUnitWithSubordinate(c *gc.C) {
	...
	for _, policy := range []state.AssignmentPolicy{
		...
		err = s.State.AssignUnit(state.StubModelConfigService(c), subUnit, policy)
...
func (s *assignSuite) TestAssignToMachineErrors(c *gc.C) {
	...
	container, err := s.State.AddMachineInsideMachine(state.StubModelConfigService(c), state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
		Jobs: []state.MachineJob{state.JobHostUnits},
	}, machine.Id(), instance.LXD)
	c.Assert(err, jc.ErrorIsNil)
	err = unit.AssignToMachine(state.StubModelConfigService(c), container)
...
func (s *assignSuite) TestAssignUnitWithNonDynamicStorageAndZonePlacementDirective(c *gc.C) {
	...
	err = s.State.AssignUnitWithPlacement(state.StubModelConfigService(c), unit, placement, nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *assignSuite) TestAssignUnitPolicyConcurrently(c *gc.C) {
	...
	for i := range us {
		us[i], err = s.wordpress.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
		c.Assert(err, jc.ErrorIsNil)
	}
	type result struct {
		u   *state.Unit
