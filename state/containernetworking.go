...
func (m *Model) AutoConfigureContainerNetworking(environ environs.Environ) error {
	...
	} else if environs.SupportsContainerAddresses(createEnvironCallContext(m), environ) {
...
func createEnvironCallContext(m *Model) *callContext {
	return &callContext{m.st}
}
...
func (m *Model) discoverFan(environ environs.Environ, modelConfig *config.Config, updateAttrs map[string]interface{}) (bool, error) {
	...
	subnets, err := netEnviron.SuperSubnets(createEnvironCallContext(m))
