...
func (s *FastPeriodSuite) TestWatchBeforeKnown(c *gc.C) {
	...
	s.w.StartSync()
	assertChange(c, s.ch, watcher.Change{"test", "a", false, revno})
	assertNoChange(c, s.ch)
...
func (s *FastPeriodSuite) TestWatchOrder(c *gc.C) {
	...
	assertChange(c, s.ch, watcher.Change{"test", "b", false, revno2})
	assertChange(c, s.ch, watcher.Change{"test", "c", false, revno3})
	assertNoChange(c, s.ch)
}
...
func (s *FastPeriodSuite) TestTransactionWithMultiple(c *gc.C) {
	...
	s.w.StartSync()
	assertChange(c, s.ch, watcher.Change{"test", "a", false, revnos[0]})
	assertChange(c, s.ch, watcher.Change{"test", "b", false, revnos[1]})
	assertChange(c, s.ch, watcher.Change{"test", "c", false, revnos[2]})
	assertNoChange(c, s.ch)
}
...
func (s *FastPeriodSuite) TestWatchMultipleChannels(c *gc.C) {
	...
	assertChange(c, ch1, watcher.Change{"test1", 1, false, revno1})
	_ = revno2
	assertChange(c, ch3, watcher.Change{"test3", 3, false, revno3})
...
func (s *FastPeriodSuite) TestRemove(c *gc.C) {
	...
	s.w.StartSync()
	assertChange(c, s.ch, watcher.Change{"test", "a", true, -1})
	assertNoChange(c, s.ch)

	revno3 := s.insert(c, "test", "a")
	s.w.StartSync()
	assertChange(c, s.ch, watcher.Change{"test", "a", false, revno3})
	assertNoChange(c, s.ch)
...
func (s *FastPeriodSuite) TestWatchKnownRemove(c *gc.C) {
	...
	assertChange(c, s.ch, watcher.Change{"test", "a", true, revno2})
...
func (s *FastPeriodSuite) TestWatchAlreadyRemoved(c *gc.C) {
	...
	s.w.WatchNoRevno("test", "a", s.ch)
	assertNoChange(c, s.ch)
	// If you watch after the changes have happened, you don't get an event.
	// If you recreate the document, you see it show up again.
	revno3 := s.insert(c, "test", "a")
	assertChange(c, s.ch, watcher.Change{"test", "a", false, revno3})
	revno4 := s.remove(c, "test", "a")
	assertChange(c, s.ch, watcher.Change{"test", "a", true, revno4})
}
...
func (s *FastPeriodSuite) TestScale(c *gc.C) {
	...
	worstTimeout := time.After(worstCase)
	for i := 0; i < N; i++ {
		select {
		case change := <-s.ch:
			seen[change.Id] = true
...
Loop1:
	...
	c.Check(seen[chA1], gc.DeepEquals, []watcher.Change{{"testA", 1, false, revno1}})
	c.Check(seen[chB1], gc.DeepEquals, []watcher.Change{{"testB", 1, false, revno3}})
	c.Check(seen[chA], gc.DeepEquals, []watcher.Change{{"testA", 1, false, revno1}, {"testA", 2, false, revno2}})
	c.Check(seen[chB], gc.DeepEquals, []watcher.Change{{"testB", 1, false, revno3}, {"testB", 2, false, revno4}})
	if c.Failed() {
		return
	}
...
Loop2:
	...
	c.Check(seen[chA], gc.DeepEquals, []watcher.Change{{"testA", 1, false, revno1}})
...
func (s *FastPeriodSuite) TestUnwatchCollectionWithFilter(c *gc.C) {
	...
	assertChange(c, chA, watcher.Change{"testA", 1, false, revnoA})
	s.insert(c, "testA", 2)
	assertNoChange(c, chA)
	s.insert(c, "testA", 3)
	s.w.StartSync()
	assertChange(c, chA, watcher.Change{"testA", 3, false, revnoA})
}
...
func (s *FastPeriodSuite) TestUnwatchCollectionWithOutstandingRequest(c *gc.C) {
	...
	assertChange(c, chB, watcher.Change{"testB", 1, false, revnoB})
}
...
func (s *FastPeriodSuite) TestNonMutatingTxn(c *gc.C) {
	...
	s.w.WatchNoRevno("test", "a", chA1)
	s.w.WatchCollection("test", chA)
	revno1 := s.insert(c, "test", "a")

	assertChange(c, chA1, watcher.Change{"test", "a", false, revno1})
	assertChange(c, chA, watcher.Change{"test", "a", false, revno1})
	...
	// We track what events we have published to each consumer, so a repeat on a
	// doc is skipped
	assertNoChange(c, chA1)
	// We don't track individual document last-seen revno, so any document txn
	// shows up as a change in the collection
	assertChange(c, chA, watcher.Change{"test", "a", false, revno2})
}
...
func (s *SlowPeriodSuite) TestWatchPeriod(c *gc.C) {
	...
	case got := <-s.ch:
		...
		c.Assert(got, gc.Equals, watcher.Change{"test", "a", false, revno2})
...
func (s *SlowPeriodSuite) TestStartSyncStartsImmediately(c *gc.C) {
	...
	// StartSync after Watching should see the current state of affairs.
	s.w.WatchNoRevno("test", "a", s.ch)
	revno := s.insert(c, "test", "a")
	s.w.StartSync()
	select {
