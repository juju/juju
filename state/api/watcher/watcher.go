...
// The content of the changes is the new environment config attributes.
...
func NewEnvironConfigWatcher(caller common.Caller, result params.EnvironConfigResult) EnvironConfigWatcher {
	w := &environConfigWatcher{
		caller:    caller,
		watcherId: result.EnvironConfigWatcherId,
		out:       make(chan params.EnvironConfig),
	}
	go func() {
		defer w.tomb.Done()
		defer close(w.out)
		w.tomb.Kill(w.loop(result.Config))
	}()
	return w
}

func (w *environConfigWatcher) loop(initialConfig params.EnvironConfig) error {
	config := initialConfig
	w.newResult = func() interface{} { return new(params.EnvironConfigResult) }
	w.call = func(request string, result interface{}) error {
		return w.caller.Call("EnvironConfigWatcher", w.watcherId, request, nil, &result)
	}
	w.commonWatcher.init()
	go w.commonLoop()
	...
	for {
		...
		config = data.(*params.EnvironConfigResult).Config
	}
	return nil
}

// Changes returns a channel that receives environ config attributes.
