...
func (m *Machine) WatchUnits() (watcher.StringsWatcher, error) {
	...
	err := m.st.caller.FacadeCall("WatchUnits", args, &results)
	if err != nil {
		return nil, err
	}
	...
	w := watcher.NewStringsWatcher(m.st.caller.RawAPICaller(), result)
	return w, nil
}
...
func (m *Machine) InstanceId() (instance.Id, error) {
	...
	err := m.st.caller.FacadeCall("InstanceId", args, &results)
	if err != nil {
		return "", err
	}
	if len(results.Results) != 1 {
