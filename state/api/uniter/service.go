...
func (s *Service) Watch() (watcher.NotifyWatcher, error) {
	...
	err := s.st.FacadeCall("Watch", args, &results)
	if err != nil {
		return nil, err
	}
...
func (s *Service) WatchRelations() (watcher.StringsWatcher, error) {
	...
	err := s.st.FacadeCall("WatchServiceRelations", args, &results)
	if err != nil {
		return nil, err
	}
	if len(results.Results) != 1 {
...
func (s *Service) CharmURL() (*charm.URL, bool, error) {
	...
	err := s.st.FacadeCall("CharmURL", args, &results)
	if err != nil {
...
func (s *Service) GetOwnerTag() (string, error) {
	...
	err := s.st.FacadeCall("GetOwnerTag", args, &result)
	if err != nil {
		return "", err
	}
	if result.Error != nil {
