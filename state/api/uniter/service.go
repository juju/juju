...
func (s *Service) Watch() (watcher.NotifyWatcher, error) {
	...
	err := s.st.CallFacade("Watch", args, &results)
	if err != nil {
		return nil, err
	}
	...
	w := watcher.NewNotifyWatcher(s.st.RawAPICaller(), result)
	return w, nil
}
...
func (s *Service) WatchRelations() (watcher.StringsWatcher, error) {
	...
	err := s.st.CallFacade("WatchServiceRelations", args, &results)
	if err != nil {
		return nil, err
	}
	if len(results.Results) != 1 {
	...
	w := watcher.NewStringsWatcher(s.st.RawAPICaller(), result)
	return w, nil
}
...
func (s *Service) CharmURL() (*charm.URL, bool, error) {
	...
	err := s.st.CallFacade("CharmURL", args, &results)
	if err != nil {
...
func (s *Service) GetOwnerTag() (string, error) {
	...
	err := s.st.CallFacade("GetOwnerTag", args, &result)
	if err != nil {
		return "", err
	}
	if result.Error != nil {
