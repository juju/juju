...
func (st *State) relation(relationTag, unitTag string) (params.RelationResult, error) {
	...
	err := st.caller.FacadeCall("Relation", args, &result)
	if err != nil {
		return nothing, err
	}
...
func (st *State) ProviderType() (string, error) {
	...
	err := st.caller.FacadeCall("ProviderType", nil, &result)
	if err != nil {
		return "", err
	}
...
func (st *State) RelationById(id int) (*Relation, error) {
	...
	err := st.caller.FacadeCall("RelationById", args, &results)
	if err != nil {
		return nil, err
	}
...
func (st *State) Environment() (*Environment, error) {
	...
	err := st.caller.FacadeCall("CurrentEnvironment", nil, &result)
...
func (st *State) environment1dot16() (*Environment, error) {
	var result params.StringResult
	err := st.caller.FacadeCall("CurrentEnvironUUID", nil, &result)
	if err != nil {
		return nil, err
	}
	if err := result.Error; err != nil {
