...
func (s *MinUnitsSuite) TestMinUnitsSetDestroyEntities(c *gc.C) {
	...
	unit1, err := s.application.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
	unit2, err := s.application.AddUnit(state.StubModelConfigService(c), state.AddUnitParams{})
	c.Assert(err, jc.ErrorIsNil)
...
func (s *MinUnitsSuite) TestEnsureMinUnits(c *gc.C) {
	...
	for i, t := range []struct {
		...
		c.Assert(application.Refresh(), gc.IsNil)
		err = application.EnsureMinUnits(state.StubModelConfigService(c))
		c.Assert(err, jc.ErrorIsNil)
...
func (s *MinUnitsSuite) TestEnsureMinUnitsApplicationNotAlive(c *gc.C) {
	...
	c.Assert(s.application.EnsureMinUnits(state.StubModelConfigService(c)), gc.ErrorMatches, expectedErr)
...
func (s *MinUnitsSuite) TestEnsureMinUnitsUpdateMinUnitsRetry(c *gc.C) {
	...
	err = s.application.EnsureMinUnits(state.StubModelConfigService(c))
	c.Assert(err, jc.ErrorIsNil)
...
func (s *MinUnitsSuite) TestEnsureMinUnitsDestroyApplicationBefore(c *gc.C) {
	...
	c.Assert(s.application.EnsureMinUnits(state.StubModelConfigService(c)), gc.ErrorMatches,
