...
func (s *constraintsValidationSuite) addOneMachine(c *gc.C, cons constraints.Value) (*state.Machine, error) {
	return s.State.AddOneMachine(state.StubModelConfigService(c), state.MachineTemplate{
...
func (s *applicationConstraintsSuite) TestConstraintsRetrieval(c *gc.C) {
	...
	negApplication, err := s.State.AddApplication(state.StubModelConfigService(c), state.AddApplicationArgs{
		Name: "unimportant",
		CharmOrigin: &state.CharmOrigin{Platform: &state.Platform{
			OS:      "ubuntu",
			Channel: "20.04/stable",
