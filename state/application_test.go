...
func (s *CAASApplicationSuite) TestInvalidChangeScale(c *gc.C) {
	newScale, err := s.app.ChangeScale(-1, []string{})
...
func (s *CAASApplicationSuite) TestChangeScale(c *gc.C) {
	newScale, err := s.app.ChangeScale(5, []string{})
	c.Assert(err, jc.ErrorIsNil)
	...
	newScale, err = s.app.ChangeScale(-4, []string{})
	c.Assert(err, jc.ErrorIsNil)
...
func (s *CAASApplicationSuite) TestChangeScaleAttachStorage(c *gc.C) {
	...
	newScale, err := app.ChangeScale(1, []string{storageTags[0].Id()})
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(newScale, gc.Equals, 1)

	newScale, err = app.ChangeScale(1, []string{storageTags[1].Id()})
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(newScale, gc.Equals, 2)

	newScale, err = app.ChangeScale(1, []string{storageTags[2].Id()})
	c.Assert(err, jc.ErrorIsNil)
