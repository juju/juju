...
type GroupedPortRanges map[string]PortRanges
...
// PortRanges represents a set of sorted, non-overlapping port ranges. When
// constructing or adding to PortRanges we ensure that the ranges are merged
// when they can be, and that the order is preserved.
type PortRanges []PortRange

// NewPortRanges returns a new PortRanges with the given ranges. The ranges
// are merged as they are added to the set. This guarantees that the resulting
// set of ranges will be non-overlapping. We also sort the ranges.
func NewPortRanges(ranges ...PortRange) PortRanges {
	prs := PortRanges{}
	for _, pr := range ranges {
		prs = prs.Add(pr)
	}
...
// Normalise returns a new PortRanges with the ranges sorted and merged.
func (prs PortRanges) Normalise() PortRanges {
	return NewPortRanges(prs...)
}

// Add returns a new PortRanges with the given PortRange added. If the new port
// range can be merged with existing port ranges, they will be.
func (prs PortRanges) Add(other PortRange) PortRanges {
	res := PortRanges{}
	i := 0
	merged := false
	for i < len(prs) {
		if !canMergePortRanges(prs[i], other) {
			res = append(res, prs[i])
			i++
			continue
		}
		// It is possible that the new port range bridges across multiple existing
		// port ranges. Since port ranges are ordered, first by protocol then by
		// starting port, we can look ahead for the first non-mergeable port range
		// and then merge all the port ranges in between.
		n := i + 1
		for n < len(prs) && canMergePortRanges(prs[n], other) {
			n++
		}
		mergedPortRange := PortRange{
			FromPort: min(prs[i].FromPort, other.FromPort),
			ToPort:   max(prs[n-1].ToPort, other.ToPort),
			Protocol: other.Protocol,
		}
		res = append(res, mergedPortRange)
		i = n
		merged = true
	}
	if !merged {
...
// Remove returns a new PortRanges with the given PortRange subtracted from
// all port ranges it intersects with.
func (prs PortRanges) Remove(other PortRange) PortRanges {
	res := PortRanges{}
	for _, pr := range prs {
		res = append(res, portRangeDifference(pr, other)...)
	}
	return res
}

func (prs PortRanges) Update(open, close PortRanges) PortRanges {
	for _, pr := range open {
		prs = prs.Add(pr)
	}
	for _, pr := range close {
		prs = prs.Remove(pr)
	}
	return prs
}

// EqualTo returns true if this set of port ranges are equal to other.
// Remember, PortRanges are sorted
func (prs PortRanges) EqualTo(other PortRanges) bool {
	if len(prs) != len(other) {
		return false
	}
	for i, pr := range prs {
		if pr != other[i] {
...
// canMergePortRanges returns true if the two port ranges can be merged.
// That is, if their protocol match and their ranges overlap or are adjacent.
//
// icmp is a special case because it does not have port ranges.
func canMergePortRanges(a, b PortRange) bool {
	...
	return a.ToPort+1 >= b.FromPort && a.FromPort-1 <= b.ToPort
}

// portRangeDifference returns the port ranges that result from removing b from a.
func portRangeDifference(a, b PortRange) PortRanges {
	if !a.ConflictsWith(b) {
		return PortRanges{a}
	}

	if a.Protocol == b.Protocol && b.Protocol == "icmp" {
		return PortRanges{}
	}

	if a.FromPort >= b.FromPort && a.ToPort <= b.ToPort {
		return PortRanges{}
	}

	if a.FromPort < b.FromPort && a.ToPort > b.ToPort {
		return PortRanges{
			{FromPort: a.FromPort, ToPort: b.FromPort - 1, Protocol: a.Protocol},
			{FromPort: b.ToPort + 1, ToPort: a.ToPort, Protocol: a.Protocol},
		}
	}

	if a.FromPort < b.FromPort {
		return PortRanges{{FromPort: a.FromPort, ToPort: b.FromPort - 1, Protocol: a.Protocol}}
	}

	return PortRanges{{FromPort: b.ToPort + 1, ToPort: a.ToPort, Protocol: a.Protocol}}
}
