...
func (*computeBaseChannelSuite) TestComputeBaseChannel(c *gc.C) {
	tests := []struct {
		...
		platform: charm.Platform{OS: "centos", Channel: "centos7"},
		result:   "7",
	}, {
		platform: charm.Platform{OS: "centos", Channel: "centos8"},
		result:   "8",
	}, {
		platform: charm.Platform{OS: "ubuntu", Channel: "20.04"},
		result:   "20.04",
	}}

	for i, test := range tests {
		c.Logf("test %d - %s", i, test.platform)
		got := charm.ComputeBaseChannel(test.platform).Channel
		c.Assert(got, gc.Equals, test.result)
	}
}
...
func (*normalisePlatformSeriesSuite) TestNormalisePlatformSeries(c *gc.C) {
	tests := []struct {
		platform charm.Platform
		result   string
	}{{
		...
		platform: charm.Platform{OS: "centos", Channel: "7"},
		result:   "centos7",
	}, {
		platform: charm.Platform{OS: "ubuntu", Channel: "20.04"},
		result:   "20.04",
	...
	for i, test := range tests {
		c.Logf("test %d - %s", i, test.platform)
		got := charm.NormalisePlatformSeries(test.platform).Channel
		c.Assert(got, gc.Equals, test.result)
	}
}
