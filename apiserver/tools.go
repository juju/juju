...
func (h *toolsDownloadHandler) processGet(r *http.Request, st *state.State) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Annotate(err, "error getting agents storage")
	}
	defer storage.Close()
	...
	if errors.IsNotFound(err) {
		...
		logger.Infof("%v agents not found locally, fetching", version)
		reader, err = h.fetchAndCacheTools(version, storage, st)
		if err != nil {
			err = errors.Annotate(err, "error fetching agents")
		}
	}
	if err != nil {
		return nil, err
	...
	if err != nil {
		return nil, errors.Annotate(err, "failed to read agents tarball")
	}
	return data, nil
}
...
func (h *toolsDownloadHandler) fetchAndCacheTools(v version.Binary, stor binarystorage.Storage, st *state.State) (io.ReadCloser, error) {
	...
	logger.Infof("fetching %v agents from %v", v, tools.URL)
	...
	if err := stor.Add(bytes.NewReader(data), metadata); err != nil {
		return nil, errors.Annotate(err, "error caching agents")
	}
...
func (h *toolsDownloadHandler) sendTools(w http.ResponseWriter, statusCode int, tarball []byte) error {
	...
	if _, err := w.Write(tarball); err != nil {
		return errors.Trace(sendError(
			w,
			errors.NewBadRequest(errors.Annotatef(err, "failed to write agents"), ""),
		))
	}
	return nil
}
...
func (h *toolsUploadHandler) processPost(r *http.Request, st *state.State) (*tools.Tools, error) {
	...
	if err != nil {
		return nil, errors.NewBadRequest(err, fmt.Sprintf("invalid agents version %q", binaryVersionParam))
	}
	...
	logger.Debugf("request to upload agents: %s", toolsVersion)
...
func (h *toolsUploadHandler) handleUpload(r io.Reader, toolsVersions []version.Binary, serverRoot string, st *state.State) (*tools.Tools, error) {
	...
	if len(data) == 0 {
		return nil, errors.BadRequestf("no agents uploaded")
	}
	...
	for _, v := range toolsVersions {
		...
		logger.Debugf("uploading agents %+v to storage", metadata)
