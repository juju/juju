...
func (s *withoutControllerSuite) TestSetStatus(c *gc.C) {
	...
	err := s.machines[0].SetStatus(sInfo, status.NoopStatusHistoryRecorder)
	c.Assert(err, jc.ErrorIsNil)
	sInfo = status.StatusInfo{
	...
	err = s.machines[1].SetStatus(sInfo, status.NoopStatusHistoryRecorder)
	c.Assert(err, jc.ErrorIsNil)
	sInfo = status.StatusInfo{
		Status:  status.Error,
		Message: "not really",
	...
	err = s.machines[2].SetStatus(sInfo, status.NoopStatusHistoryRecorder)
	c.Assert(err, jc.ErrorIsNil)
