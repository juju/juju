...
func (s *uniterSuite) TestWorkloadVersion(c *gc.C) {
	...
	c.Assert(result, gc.DeepEquals, params.StringResults{
		Results: []params.StringResult{
			...
			{Result: "capulet"},
			{Error: apiservertesting.ErrUnauthorized},
			{Error: commonerrors.ServerError(errors.New(`"application-wordpress" is not a valid unit tag`))},
			{Error: commonerrors.ServerError(errors.New(`"just-foo" is not a valid tag`))},
		},
	})
}
...
func (s *uniterSuite) TestFinishActionsAuthAccess(c *gc.C) {
	...
	var tests = []struct {
		...
		{actionTag: bad.ActionTag(), err: commonerrors.ErrPerm},
	}
...
func (s *uniterSuite) TestReadLocalApplicationSettingsForAnotherApplicationAsAnOperator(c *gc.C) {
	...
	c.Assert(errors.Cause(err), gc.Equals, commonerrors.ErrPerm, gc.Commentf("expected ErrPerm due to mismatch in logged in app and inferred app from provided unit name"))
}
