...
func (s *uniterSuite) expectGetCharmLocatorByApplicationName(c *gc.C, appName string, charmLocator domaincharm.CharmLocator, err error) {
	s.applicationService.EXPECT().GetCharmLocatorByApplicationName(gomock.Any(), appName).Return(charmLocator, err)
}

func (s *uniterSuite) expectShouldAllowCharmUpgradeOnError(c *gc.C, appName string, v bool, err error) {
...
func (s *uniterSuite) TestConfigSettings(c *gc.C) {
	...
	// Assert:
	c.Assert(err, jc.ErrorIsNil)
	c.Check(result, jc.DeepEquals, params.ConfigSettingsResults{
