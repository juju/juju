...
func (s *commitHookChangesSuite) TestSetUnitRelationNetworks(c *tc.C) {
	...
	s.expectedSetRelationApplicationAndUnitSettings(unitName, relUUID1, nil, map[string]string{
	...
	s.expectedSetRelationApplicationAndUnitSettings(unitName, relUUID2, nil, map[string]string{
...
func (s *commitHookChangesSuite) expectGetRelationUnitUUID(relUUID corerelation.UUID, uuid corerelation.UnitUUID, unitTagID string) {
	s.relationService.EXPECT().GetRelationUnitUUID(gomock.Any(), relUUID, coreunit.Name(unitTagID)).Return(uuid, nil)
}
