...
func (s *uniterRelationSuite) TestWatchUnitRelations(c *gc.C) {
	...
	s.expectWatcherRegistry(watcherID, nil)
...
func (s *uniterRelationSuite) expectWatcherRegistry(watchID string, err error) {
	s.watcherRegistry.EXPECT().Register(gomock.Any()).Return(watchID, nil).AnyTimes()
}
