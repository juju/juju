...
func (uf UnitFacade) Track(ctx context.Context, args params.TrackPayloadArgs) (params.PayloadResults, error) {
	uf.logger.Debugf(ctx, "tracking %d payloads from API", len(args.Payloads))
	...
	for _, apiPayload := range args.Payloads {
		...
		uf.logger.Debugf(ctx, "tracking payload from API: %#v", pl)
...
func (uf UnitFacade) listAll(ctx context.Context) (params.PayloadResults, error) {
	var r params.PayloadResults
	...
	for _, result := range results {
		...
		id, err := uf.backend.LookUp(pl.Name, pl.ID)
		if err != nil {
			uf.logger.Errorf(ctx, "failed to look up ID for %q: %v", pl.FullID(), err)
			id = ""
		}
