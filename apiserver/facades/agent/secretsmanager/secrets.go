...
func (s *SecretsManagerAPI) updateAppOwnedOrUnitOwnedSecretLabel(ctx context.Context, uri *coresecrets.URI, label, owner string) error {
	if uri == nil || label == "" {
	...
	// Update the label.
	_, err = s.secretService.UpdateSecret(ctx, uri, secretservice.UpdateSecretParams{
		LeaderToken: token,
		Label:       &label,
	})
	return errors.Trace(err)
...
func (s *SecretsManagerAPI) getAppOwnedOrUnitOwnedSecretMetadata(ctx context.Context, uri *coresecrets.URI, label string) (*coresecrets.SecretMetadata, error) {
	...
	metadata, _, err := s.secretService.ListCharmSecrets(ctx, owner)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func (s *SecretsManagerAPI) getSecretContent(ctx context.Context, arg params.GetSecretContentArg) (
	*secrets.ContentParams, *secretsprovider.ModelBackendConfig, bool, error,
	...
	if uri == nil || uri.IsLocal(s.modelUUID) {
		md, err := s.getAppOwnedOrUnitOwnedSecretMetadata(ctx, uri, arg.Label)
		...
		if md != nil {
			...
			if labelToUpdate != nil && *labelToUpdate != md.Label {
				...
				if isOwner {
					err = s.updateAppOwnedOrUnitOwnedSecretLabel(ctx, uri, *labelToUpdate, md.OwnerTag)
					if err != nil {
						return nil, nil, false, errors.Trace(err)
					}
				}
	...
	if uri == nil {
		var err error
		uri, err = s.secretsConsumer.GetURIByConsumerLabel(ctx, arg.Label, consumer)
		if errors.Is(err, errors.NotFound) {
