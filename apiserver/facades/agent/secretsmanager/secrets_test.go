...
type SecretsManagerSuite struct {
	...
	modelState                *mocks.MockModelState
...
func (s *SecretsManagerSuite) setup(c *gc.C) *gomock.Controller {
	...
	s.modelState = mocks.NewMockModelState(ctrl)
	...
	s.facade, err = secretsmanager.NewTestAPI(
		s.authorizer, s.resources, s.leadership, s.secretsState, s.modelState, s.secretsConsumer, s.secretTriggers,
...
func (s *SecretsManagerSuite) assertGetSecretsToMigrate(
	...
	c.Assert(err, jc.ErrorIsNil)
	s.modelState.EXPECT().ModelConfig().Return(cfg, nil)
	s.modelState.EXPECT().Type().Return(modelType)

	s.modelState.EXPECT().ControllerUUID().Return(coretesting.ControllerTag.Id())
	s.modelState.EXPECT().UUID().Return(coretesting.ModelTag.Id())
...
func (s *SecretsManagerSuite) TestChangeSecretBackend(c *gc.C) {
	defer s.setup(c).Finish()
...
func (s *SecretsManagerSuite) TestWatchSecretBackendChanged(c *gc.C) {
	...
	cfg, err := config.New(config.NoDefaults, configAttrs)
	c.Assert(err, jc.ErrorIsNil)
	s.modelState.EXPECT().ModelConfig().Return(cfg, nil).Times(2)
