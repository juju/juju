...
func (s *AgentToolsSuite) TestCheckTools(c *tc.C) {
	...
	api, err := NewAgentToolsAPI(fakeToolFinder, nil, loggertesting.WrapCheckLog(c), s.machineService, s.modelConfigService, s.modelAgentService, s.modelInfoService)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *AgentToolsSuite) TestUpdateToolsAvailability(c *tc.C) {
	...
	s.modelInfoService.EXPECT().UpdateLatestAgentVersion(gomock.Any(), semversion.Number{Major: 2, Minor: 5, Patch: 2}).Return(nil)
