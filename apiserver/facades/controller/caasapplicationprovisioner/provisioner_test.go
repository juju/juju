...
func (s *CAASProvisionerSuite) SetUpTest(c *gc.C) {
	...
	api, err := caasapplicationprovisioner.NewCAASOperatorProvisionerAPI(s.st, s.resources, s.authorizer, s.storagePoolManager, s.registry)
...
func (s *CAASProvisionerSuite) TestPermission(c *gc.C) {
	...
	_, err := caasapplicationprovisioner.NewCAASOperatorProvisionerAPI(s.st, s.resources, s.authorizer, s.storagePoolManager, s.registry)
