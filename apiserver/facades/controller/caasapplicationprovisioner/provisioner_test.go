...
func (s *CAASApplicationProvisionerSuite) TestProvisioningInfo(c *gc.C) {
	...
	s.applicationService.EXPECT().GetScale(gomock.Any(), "gitlab").Return(3, nil)
...
func (s *CAASApplicationProvisionerSuite) TestProvisioningState(c *gc.C) {
	...
	s.applicationService.EXPECT().SetScalingState(gomock.Any(), "gitlab", 10, true)
	s.applicationService.EXPECT().GetScalingState(gomock.Any(), "gitlab").Return(service.ScalingState{
		Scaling:     true,
		ScaleTarget: 10,
	}, nil)
