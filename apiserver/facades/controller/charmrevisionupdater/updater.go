	...
	for _, application := range applications {
		...
		case charm.CharmHub.Matches(curl.Schema):
			...
			if origin == nil {
				...
				api.logger.Errorf(context.TODO(), "charm %s has no origin, skipping", curl)
				continue
			}
			if origin.ID == "" || origin.Revision == nil || origin.Channel == nil || origin.Platform == nil {
				api.logger.Errorf(context.TODO(), "charm %s has missing id(%s), revision (%p), channel (%p), or platform (%p), skipping",
...
func (api *CharmRevisionUpdaterAPI) fetchCharmhubInfos(ctx context.Context, cfg *config.Config, ids []charmhubID, appInfos []appInfo) ([]latestCharmInfo, error) {
	...
	for i, result := range results {
		if i >= len(appInfos) {
			api.logger.Errorf(context.TODO(), "retrieved more results (%d) than charmhub applications (%d)",
				i, len(appInfos))
			break
		}
		if result.error != nil {
			api.logger.Errorf(context.TODO(), "retrieving charm info for ID %s: %v", ids[i].id, result.error)
			continue
		}
		appInfo := appInfos[i]
