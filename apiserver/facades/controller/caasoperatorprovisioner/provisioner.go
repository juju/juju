		...
		if storageRequired {
			if storageClassName == "" {
				return params.OperatorProvisioningInfo{
					Error: commonerrors.ServerError(errors.New("no operator storage defined")),
				}
			} else {
				...
				if err != nil {
					return params.OperatorProvisioningInfo{
						Error: commonerrors.ServerError(errors.Annotatef(err, "getting operator storage parameters")),
					}
				}
