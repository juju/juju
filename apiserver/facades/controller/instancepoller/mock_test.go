...
func (m *mockMachine) SetInstanceStatus(instanceStatus status.StatusInfo, recorder status.StatusHistoryRecorder) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	m.MethodCall(m, "SetInstanceStatus", instanceStatus, recorder)
	if err := m.NextErr(); err != nil {
		return err
	}
