...
type ApplicationService interface {
	GetScale(ctx context.Context, appName string) (int, error)
	SetScale(ctx context.Context, appName string, scale int, force bool) error
...
func (f *Facade) applicationScale(ctx context.Context, tagString string) (int, error) {
	...
	scale, err := f.applicationService.GetScale(ctx, appTag.Id())
	if err != nil {
		return 0, errors.Trace(err)
	}
	return scale, nil
...
func (f *Facade) UpdateApplicationsService(ctx context.Context, args params.UpdateApplicationServiceArgs) (params.ErrorResults, error) {
	...
	for i, appUpdate := range args.Args {
		...
		if appUpdate.Scale != nil {
			if err := f.applicationService.SetScale(ctx, appName, *appUpdate.Scale, false); err != nil {
				result.Results[i].Error = apiservererrors.ServerError(err)
			}
		}
	}
