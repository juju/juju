...
func (s *facadeSuite) TestGetVirtualHostnameForEntity(c *gc.C) {
	...
	authorizer.EXPECT().AuthClient().Return(true).MinTimes(1)
	authorizer.EXPECT().HasPermission(permission.SuperuserAccess, testing.ControllerTag).Return(authentication.ErrorEntityMissingPermission).MinTimes(1)
	authorizer.EXPECT().HasPermission(permission.AdminAccess, testing.ModelTag).Return(nil).MinTimes(1)
	...
	for _, t := range tests {
		...
		res, err := facade.VirtualHostname(params.VirtualHostnameTargetArg{
			Entity:    params.Entity{Tag: t.tag},
