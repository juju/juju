...
func newOpFactory(st *state.State, ctrlConfigService ControllerConfigGetter) OpFactory {
	return &opFactory{st: st, ctrlConfigService: ctrlConfigService}
}
...
func (f *opFactory) NewRenameSpaceOp(fromName, toName string) (state.ModelOperation, error) {
	...
	return NewRenameSpaceOp(
		f.st.IsController(), f.st.NewControllerSettings(), &renameSpaceState{f.st}, space, toName, f.ctrlConfigService), nil
}
