...
type SpaceRenameSuite struct {
	...
	cc       *spaces.MockControllerConfigGetter
}
...
func (s *SpaceRenameSuite) TestBuildSuccess(c *gc.C) {
	...
	op := spaces.NewRenameSpaceOp(true, s.settings, s.state, s.space, toName, s.cc)
	ops, err := op.Build(0)
...
func (s *SpaceRenameSuite) TestBuildNotControllerModelSuccess(c *gc.C) {
	...
	op := spaces.NewRenameSpaceOp(false, s.settings, s.state, s.space, toName, s.cc)
	ops, err := op.Build(0)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *SpaceRenameSuite) setupMocks(c *gc.C) *gomock.Controller {
	...
	s.cc = spaces.NewMockControllerConfigGetter(ctrl)
