...
func (api *KeyManagerAPI) checkCanRead(sshUser string) error {
	if err := api.checkCanWrite(sshUser); err == nil {
		return nil
	} else if err != commonerrors.ErrPerm {
...
func (api *KeyManagerAPI) ListKeys(arg params.ListSSHKeys) (params.StringsResults, error) {
	...
	for i, entity := range arg.Entities.Entities {
		...
		results[i].Error = commonerrors.ServerError(configErr)
	}
...
func (api *KeyManagerAPI) AddKeys(arg params.ModifyUserSSHKeys) (params.ErrorResults, error) {
	...
	if err := api.checkCanWrite(arg.User); err != nil {
		return params.ErrorResults{}, commonerrors.ServerError(err)
	}

	// For now, authorised keys are global, common to all users.
	sshKeys, currentFingerprints, err := api.currentKeyDataForAdd()
	if err != nil {
		return params.ErrorResults{}, commonerrors.ServerError(fmt.Errorf("reading current key data: %v", err))
	}
	...
	for i, key := range arg.Keys {
		fingerprint, _, err := ssh.KeyFingerprint(key)
		if err != nil {
			result.Results[i].Error = commonerrors.ServerError(fmt.Errorf("invalid ssh key: %s", key))
			continue
		}
		if currentFingerprints.Contains(fingerprint) {
			result.Results[i].Error = commonerrors.ServerError(fmt.Errorf("duplicate ssh key: %s", key))
			continue
		}
...
func (api *KeyManagerAPI) ImportKeys(arg params.ModifyUserSSHKeys) (params.ErrorResults, error) {
	...
	for i, key := range arg.Keys {
		...
		if compoundErr != "" {
			result.Results[i].Error = commonerrors.ServerError(errors.Errorf(strings.TrimSuffix(compoundErr, "\n")))
		}
...
func (api *KeyManagerAPI) DeleteKeys(arg params.ModifyUserSSHKeys) (params.ErrorResults, error) {
	...
	for i, keyId := range arg.Keys {
		...
		if ok {
			if internalComments.Contains(keyId) {
				result.Results[i].Error = commonerrors.ServerError(fmt.Errorf("may not delete internal key: %s", keyId))
				continue
			}
			keysToDelete.Add(key)
			continue
		}
		result.Results[i].Error = commonerrors.ServerError(fmt.Errorf("invalid ssh key: %s", keyId))
	}
	...
	if len(keysToWrite) == 0 {
		return params.ErrorResults{}, commonerrors.ServerError(fmt.Errorf("cannot delete all keys"))
	}
