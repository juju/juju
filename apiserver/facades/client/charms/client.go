...
func (a *API) resolveOneCharm(arg params.ResolveCharmWithChannel) params.ResolveCharmWithChannelResult {
	...
	resultURL, origin, resolvableBases, err := repo.ResolveWithPreferredChannel(curl, requestedOrigin)
	if err != nil {
		result.Error = apiservererrors.ServerError(err)
		return result
	}
	...
	switch {
	case resultURL.Series != "" && len(resolvableBases) == 0:
		base, err := series.GetBaseFromSeries(resultURL.Series)
		if err != nil {
			result.Error = apiservererrors.ServerError(err)
			return result
		}
		result.SupportedBases = []params.CharmBase{{Name: base.OS, Channel: base.Channel.String()}}
	default:
...
func (a *APIv6) ResolveCharms(args params.ResolveCharmsWithChannel) (params.ResolveCharmWithChannelResults, error) {
	res, err := a.API.ResolveCharms(args)
	if err != nil {
		return params.ResolveCharmWithChannelResults{}, errors.Trace(err)
	}
	res.Results, err = transform.SliceOrErr(res.Results, func(result params.ResolveCharmWithChannelResult) (params.ResolveCharmWithChannelResult, error) {
		result.SupportedSeries, err = transform.SliceOrErr(result.SupportedBases, func(pBase params.CharmBase) (string, error) {
...
func convertCharmBase(in corecharm.Platform) params.CharmBase {
	return params.CharmBase{
		Name:    in.OS,
		Channel: in.Channel,
	}
}
