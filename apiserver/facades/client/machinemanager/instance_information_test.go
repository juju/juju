...
func (s *instanceTypesSuite) TestInstanceTypes(c *gc.C) {
	...
	providerGetter := func(_ context.Context) (environs.InstanceTypesFetcher, error) {
		return s.instanceTypesFetcher, nil
	}

	r, err := instanceTypes(envcontext.WithoutCredentialInvalidator(context.Background()), providerGetter, cons)
