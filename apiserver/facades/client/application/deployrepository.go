	...
	if addApplicationErr != nil {
		...
		if len(pendingIDs) != 0 {
			...
			if removeResourcesErr != nil {
				api.logger.Errorf(context.TODO(), "unable to remove pending resources for %q", dt.applicationName)
			}
		}
...
func (api *DeployFromRepositoryAPI) addPendingResources(appName string, resources []resource.Resource) (map[string]string, []error) {
	...
	for _, r := range resources {
		...
		if err != nil {
			api.logger.Errorf(context.TODO(), "Unable to add pending resource %v for application %v: %v", r.Name, appName, err)
