...
func (s *modelUpgradeSuite) assertUpgradeModelForControllerModelJuju3(c *gc.C, dryRun bool) {
	...
	s.ctrlConfigService.EXPECT().ControllerConfig(gomock.Any()).Return(controllerCfg, nil).AnyTimes()
	...
	s.toolsFinder.EXPECT().FindAgents(common.FindAgentsParams{
		...
		ControllerCfg: controllerCfg, ModelType: state.ModelTypeIAAS}, s.ctrlConfigService).Return(
		[]*coretools.Tools{
...
func (s *modelUpgradeSuite) TestFindToolsIAAS(c *gc.C) {
	...
	s.toolsFinder.EXPECT().FindAgents(common.FindAgentsParams{
		MajorVersion: 2, ModelType: state.ModelTypeIAAS}, s.ctrlConfigService).Return(simpleStreams, nil)
...
func (s *modelUpgradeSuite) TestFindToolsCAASReleased(c *gc.C) {
	...
	gomock.InOrder(
		...
		}, s.ctrlConfigService).Return(simpleStreams, nil),
...
func (s *modelUpgradeSuite) TestDecideVersionFindToolUseAgentVersionMajorMinor(c *gc.C) {
	...
	}, s.ctrlConfigService).Return(nil, errors.New(`fail to exit early`))
