	...
	t := revokeCredentialData{
		...
		results: params.ErrorResults{
			Results: []params.ErrorResult{
				{commonerrors.ServerError(errors.New("no niet nope"))},
			},
		},
		expectedLog: []string{},
	}
...
func (s *cloudSuite) TestRevokeCredentialsHasModel(c *gc.C) {
	t := revokeCredentialData{
		...
		results: params.ErrorResults{
			Results: []params.ErrorResult{
				{commonerrors.ServerError(errors.New("cannot revoke credential cloudcred-meep_julia_three: it is still used by 1 model"))},
			},
		},
...
func (s *cloudSuite) TestRevokeCredentialsHasModels(c *gc.C) {
	t := revokeCredentialData{
		...
		results: params.ErrorResults{
			Results: []params.ErrorResult{
				{commonerrors.ServerError(errors.New("cannot revoke credential cloudcred-meep_julia_three: it is still used by 2 models"))},
			},
		},
...
func (s *cloudSuite) TestRevokeCredentialsForceMany(c *gc.C) {
	t := revokeCredentialData{
		...
		results: params.ErrorResults{
			Results: []params.ErrorResult{
				{},
				{commonerrors.ServerError(errors.New("cannot revoke credential cloudcred-meep_bruce_three: it is still used by 1 model"))},
			},
		},
		expectedLog: []string{
