...
// RelatedEndpoints returns the endpoints in the relation status that are related to the specified application.
// It filters endpoints based on the counterpart role of the specified application's endpoint role.
...
func (context *statusContext) processRelations(ctx context.Context) []params.RelationStatus {
	...
	for _, relation := range context.relationsByID {
		...
		relStatus := params.RelationStatus{
			Id:        relation.ID,
			Key:       relation.UUID.String(),
			Interface: relationInterface,
			Scope:     string(scope),
			Endpoints: eps,
		}
		populateStatusFromStatusInfoAndErr(&relStatus.Status, relation.Status, nil)
