...
func (s *modelManagerSuite) TestDumpModel(c *tc.C) {
	...
	expected := &domainexport.ModelExport{
		...
		Payload: &v4_0_16.ModelExport{
			AgentBinaryStore: []v4_0_16.AgentBinaryStore{{
				Version:         "4.0.4",
				ArchitectureID:  1,
	...
	var wireExport struct {
		...
		Payload v4_0_16.ModelExport `yaml:"payload"`
	}
	err := yaml.Unmarshal([]byte(results.Results[0].Result), &wireExport)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(wireExport.Version, tc.Equals, "4.0.16")

	expectedPayload, ok := expected.Payload.(*v4_0_16.ModelExport)
...
func (s *modelManagerSuite) TestDumpModelUsers(c *tc.C) {
	...
	c.Check(results.Results[0].Result, tc.Matches, "(?s).*version: 4.0.16.*")
}
