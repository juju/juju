...
func (s *uniterBaseSuite) testSetStatus(
	...
	err = s.mysqlUnit.SetStatus(state.StatusInstalling, "foo", nil)
	c.Assert(err, jc.ErrorIsNil)

	args := params.SetStatus{
		Entities: []params.EntityStatus{
			{Tag: "unit-mysql-0", Status: params.StatusError, Info: "not really"},
			{Tag: "unit-wordpress-0", Status: params.StatusInstalling, Info: "foobar"},
	...
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(status, gc.Equals, state.StatusInstalling)
