...
func (s *clientAuthRootSuite) TestNormalUser(c *gc.C) {
	...
	s.AssertCallGood(c, client, "Client", 0, "ServiceDeploy")
	s.AssertCallGood(c, client, "UserManager", 0, "UserInfo")
	s.AssertCallNotImplemented(c, client, "Client", 0, "Unknown")
	s.AssertCallNotImplemented(c, client, "Unknown", 1, "Method")
}

func (s *clientAuthRootSuite) TestReadOnlyUser(c *gc.C) {
	envUser := s.Factory.MakeEnvUser(c, &factory.EnvUserParams{ReadOnly: true})
	client := newClientAuthRoot(&fakeFinder{}, envUser)
	// deploys are bad
	s.AssertCallErrPerm(c, client, "Client", 0, "ServiceDeploy")
	// read only commands are fine
	s.AssertCallGood(c, client, "Client", 0, "FullStatus")
	// calls on the restricted root is also fine
	s.AssertCallGood(c, client, "UserManager", 0, "AddUser")
	s.AssertCallNotImplemented(c, client, "Client", 0, "Unknown")
	s.AssertCallNotImplemented(c, client, "Unknown", 1, "Method")
}
