...
func (h *UnitResourcesHandler) serveGet(resp http.ResponseWriter, req *http.Request) error {
	...
	if err != nil {
		h.logger.Errorf("cannot fetch resource reader: %v", err)
		return err
	}
	defer opened.Close()
	...
	if bytesWritten, err = io.Copy(resp, opened); err != nil {
		...
		h.logger.Warningf("unable to complete stream for resource, %d bytes streamed: %v out of %v",
			bytesWritten, opened.Size, err)
		return nil
	}

	if bytesWritten != opened.Size {
		h.logger.Warningf("resource streamed to unit had unexpected size: got %v, expected %v",
	...
	if err != nil {
		h.logger.Warningf("setting resource %s as in use: %w", name, err)
	}
