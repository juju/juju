...
func (h *ResourceHandler) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		if err := internalhttp.SendError(resp, err, h.logger); err != nil {
			h.logger.Errorf("returning error to user: %v", err)
		}
		return
	}
	...
	case "GET":
		...
		if bytesWritten, err = io.Copy(resp, reader); err != nil {
			if err := internalhttp.SendError(resp, errors.Errorf("resource download failed: %w", err), h.logger); err != nil {
				h.logger.Errorf("returning error to user: %v", err)
			}
			return
		}
		if bytesWritten != size {
			h.logger.Warningf("resource download size does not match expected resource size: %d != %d", bytesWritten, size)
		}
	case "PUT":
		...
		if err := internalhttp.SendStatusAndJSON(resp, http.StatusOK, &response); err != nil {
			h.logger.Errorf("sending response: %v", err)
		}
	default:
...
func (h *ResourceHandler) readResource(resourceService ResourceService, req *http.Request) (io.ReadCloser, *uploadedResource, error) {
	...
	reader, err := h.validator.Validate(req.Body, uReq.Fingerprint.String(), uReq.Size)
	if err != nil {
