...
func (v *Downloader) Download(
	...
	defer func() {
		...
		if err == nil {
			if cErr != nil {
				v.logger.Warningf(ctx, "closing temporary file: %v", cErr)
			}
			return
		}
		...
		if removeErr != nil {
			v.logger.Warningf(ctx, "failed to remove temporary file %q: %v", tmpFile.Name(), removeErr)
		}
	}()
	...
	if sha384 != expectedSHA384 {
		return nil, errors.Errorf(
			"downloaded resource has unexpected hash: got %q, expected %q", sha384, expectedSHA384,
		)
	}
...
func (v *Downloader) newTmpFileReader(file string) (*tmpFileReader, error) {
	...
	return &tmpFileReader{
		File:       f,
		logger:     v.logger.Child("tmpfilereader"),
