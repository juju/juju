...
func (t testingAPIRootHandler) WatcherRegistry() facade.WatcherRegistry {
	return t.watcherRegistry
}
...
func TestingAPIRoot(c *gc.C, facades *facade.Registry) rpc.Root {
	watcherRegistry, err := registry.NewRegistry(clock.WallClock)
	c.Assert(err, jc.ErrorIsNil)

	root, err := newAPIRoot(clock.WallClock, facades, testingAPIRootHandler{watcherRegistry: watcherRegistry}, nil)
	c.Assert(err, jc.ErrorIsNil)
	return root
}
...
func TestingAPIHandler(c *gc.C, pool *state.StatePool, st *state.State) (*apiHandler, facade.WatcherRegistry) {
...
// entity.
func TestingAPIHandlerWithEntity(c *gc.C, pool *state.StatePool, st *state.State, entity state.Entity) (*apiHandler, facade.WatcherRegistry) {
...
) (*apiHandler, facade.WatcherRegistry) {
	h, hr := TestingAPIHandler(c, pool, st)
...
// scenario.
func TestingUpgradingRoot(c *gc.C) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
	return restrictRoot(r, upgradeMethodsOnly)
}

// TestingMigratingRoot returns a resricted srvRoot in a migration
// scenario.
func TestingMigratingRoot(c *gc.C) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
...
func TestingAnonymousRoot(c *gc.C) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
...
func TestingControllerOnlyRoot(c *gc.C) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
...
func TestingModelOnlyRoot(c *gc.C) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
...
func TestingCAASModelOnlyRoot(c *gc.C) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
	return restrictRoot(r, caasModelFacadesOnly)
}

// TestingRestrictedRoot returns a restricted srvRoot.
func TestingRestrictedRoot(c *gc.C, check func(string, string) error) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
...
func TestingUpgradeOrMigrationOnlyRoot(c *gc.C, userLogin bool, clientVersion version.Number) rpc.Root {
	r := TestingAPIRoot(c, AllFacades())
