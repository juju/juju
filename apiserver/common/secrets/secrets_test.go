...
func (s *secretsSuite) TestRemoveSecretsForModelAdminDuringBackendMigration(c *gc.C) {
	...
	mockprovider.EXPECT().NewBackend(backendBeforeMigrationCfg).Times(2).Return(backendBeforeMigration, nil)
	mockprovider.EXPECT().NewBackend(backendAfterMigrationCfg).Times(1).Return(backendAfterMigration, nil)
...
func (s *secretsSuite) TestRemoveSecretNotFoundForModelAdmin(c *gc.C) {
	...
	// Mock the package GetProvider function to return the mockprovider
	s.PatchValue(&secrets.GetProvider, func(string) (provider.SecretBackendProvider, error) { return mockprovider, nil })
	// Uses NewBackend.AnyTimes as the number of calls to this is an implementation detail of the function under test
	mockprovider.EXPECT().NewBackend(backendCfg).Times(2).Return(backend, nil)
