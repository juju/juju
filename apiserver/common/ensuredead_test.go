...
func (*deadEnsurerSuite) TestEnsureDead(c *gc.C) {
	...
	defer ctrl.Finish()
	d := common.NewDeadEnsurer(st, afterDead, getCanModify, mocks.NewMockMachineDeadEnsurerService(ctrl))
...
func (*deadEnsurerSuite) TestEnsureDeadError(c *gc.C) {
	...
	ctrl := gomock.NewController(c)
	defer ctrl.Finish()
	d := common.NewDeadEnsurer(&fakeState{}, nil, getCanModify, mocks.NewMockMachineDeadEnsurerService(ctrl))
