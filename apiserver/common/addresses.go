...
func (a *APIAddresser) APIHostPorts(ctx context.Context) (params.APIHostPortsResult, error) {
	srvs, err := a.getter.GetAllAPIAddressesForAgents(ctx)
	if err != nil {
		return params.APIHostPortsResult{}, err
	}

	// Convert the strings to the HostPorts indirection.
	serverResults := make([][]params.HostPort, 0)
	for _, addrs := range srvs {
		var err error
		out := transform.Slice(addrs, func(in string) params.HostPort {
			if err != nil {
				return params.HostPort{}
			}
			addrPort, localErr := netip.ParseAddrPort(in)
			if localErr != nil {
				err = localErr
				return params.HostPort{}
			}
...
func (a *APIAddresser) WatchAPIHostPorts(ctx context.Context) (params.NotifyWatchResult, error) {
	...
	notifyWatcher, err := a.getter.WatchControllerAPIAddresses(ctx)
	if err != nil {
		return result, errors.Trace(err)
	}
	result.NotifyWatcherId, _, err = internal.EnsureRegisterWatcher[struct{}](ctx, a.watcherRegistry, notifyWatcher)
	return result, err
}
