...
func (s *storageRemovalSuite) TestClassifyStorageRemovalDetachesDetachable(c *tc.C) {
	...
	detachable := s.newInstance(c, "lxd-fs/0", true)
	nonDetachable := s.newInstance(c, "loop-vol/0", false)
	...
	c.Check(err, tc.ErrorIsNil)
	c.Check(destroyed, tc.DeepEquals, []params.Entity{{Tag: "storage-loop-vol-0"}})
	c.Check(detached, tc.DeepEquals, []params.Entity{{Tag: "storage-lxd-fs-0"}})
}
