...
func (*statusSetterSuite) TestSetStatus(c *gc.C) {
	st := &fakeState{
		entities: map[names.Tag]entityWithError{
			...
			u("x/1"): &fakeStatusSetter{status: state.StatusActive, info: "foo"},
			u("x/2"): &fakeStatusSetter{status: state.StatusError, info: "some info"},
			u("x/3"): &fakeStatusSetter{fetchError: "x3 error"},
			u("x/4"): &fakeStatusSetter{status: state.StatusInstalling, info: "blah"},
		},
	}
	...
	args := params.SetStatus{
		Entities: []params.EntityStatus{
			...
			{"unit-x-1", params.StatusActive, "", nil},
			{"unit-x-2", params.StatusAllocating, "not really", nil},
			{"unit-x-3", params.StatusStopped, "", nil},
			{"unit-x-4", params.StatusError, "blarg", nil},
			{"unit-x-5", params.StatusActive, "42", nil},
			{"unit-x-6", params.StatusActive, "bar", nil},
		},
	}
	...
	c.Assert(get(u("x/2")).status, gc.Equals, state.StatusAllocating)
	c.Assert(get(u("x/2")).info, gc.Equals, "not really")
	c.Assert(get(u("x/4")).status, gc.Equals, state.StatusError)
	c.Assert(get(u("x/4")).info, gc.Equals, "blarg")
}
...
func (*statusSetterSuite) TestUpdateStatus(c *gc.C) {
	st := &fakeState{
		entities: map[names.Tag]entityWithError{
			...
			m("5"): &fakeStatusSetter{status: state.StatusFailed, info: ""},
		},
	}
	getCanModify := func() (common.AuthFunc, error) {
		x0 := m("0")
