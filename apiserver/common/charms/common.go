...
func convertCharmDeployment(deployment *charm.Deployment) *params.CharmDeployment {
	...
	servicePorts := []params.CharmServicePort(nil)
	for _, v := range deployment.ServicePorts {
		servicePorts = append(servicePorts, convertCharmServicePort(v))
	}
	return &params.CharmDeployment{
		...
		ContainerImageName: deployment.ContainerImageName,
...
func convertCharmServicePort(sp charm.ServicePort) params.CharmServicePort {
	return params.CharmServicePort{
		Name:        sp.Name,
		Port:        sp.Port,
