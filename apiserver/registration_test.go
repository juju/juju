...
func (s *registrationSuite) SetUpTest(c *tc.C) {
	...
	s.activationKey, err = s.accessService.ResetPassword(c.Context(), user.GenName(c, "bob"))
	c.Assert(err, tc.ErrorIsNil)
...
func (s *registrationSuite) assertRegisterNoProxy(c *tc.C, hasProxy bool) {
	...
	// now.
	_, err := s.accessService.GetUserByAuth(c.Context(), user.GenName(c, "bob"), auth.NewPassword(password))
	...
	user, err := s.accessService.GetUserByAuth(c.Context(), user.GenName(c, "bob"), auth.NewPassword(password))
	c.Assert(err, tc.ErrorIsNil)
...
func (s *registrationSuite) TestRegisterNoSecretKey(c *tc.C) {
	err := s.accessService.SetPassword(c.Context(), user.GenName(c, "bob"), auth.NewPassword("anything"))
	c.Assert(err, tc.ErrorIsNil)
