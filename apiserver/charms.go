...
func repackageCharmWithRevision(ctx context.Context, archive *charm.CharmArchive, charmRevision int) (*charm.CharmArchive, bytes.Buffer, string, error) {
	...
	repackagedArchive, err := charm.ReadCharmArchiveBytes(repackagedArchiveBuf.Bytes())
	if err != nil {
		return archive, bytes.Buffer{}, "", errors.Annotate(err, "cannot read repackaged charm")
	}

	return repackagedArchive, repackagedArchiveBuf, repackagedSHA256, nil
}
