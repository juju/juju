...
func (cfg *InstanceConfig) SetTools(toolsList coretools.List) error {
	if len(toolsList) == 0 {
		return errors.New("need at least 1 agent")
	}
	var tools *coretools.Tools
	for _, listed := range toolsList {
		if listed == nil {
			return errors.New("nil entry in agents list")
		}
		info := *listed
		info.URL = ""
		if tools == nil {
		...
		if !reflect.DeepEqual(info, *tools) {
			return errors.Errorf("agents info mismatch (%v, %v)", *tools, info)
		}
	}
...
func (cfg *InstanceConfig) VerifyConfig() (err error) {
	...
	if cfg.tools == nil {
		...
		return errors.New("missing agents")
	}
