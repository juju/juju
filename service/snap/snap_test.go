...
func (*serviceSuite) TestInstallCommands(c *gc.C) {
	...
	service, err := NewService("juju-db", "juju-db", conf, Command, "4.4.30/stable", "", backgroundServices, prerequisites)
	c.Assert(err, jc.ErrorIsNil)
	...
	c.Assert(commands, gc.DeepEquals, []string{
		"snap install core",
		"snap install --channel=4.4.30/stable juju-db",
	})
}
...
func (*serviceSuite) TestInstallCommandsWithConfinementPolicy(c *gc.C) {
	...
	service, err := NewService("juju-db", "juju-db", conf, Command, "4.4.30/stable", "classic", backgroundServices, prerequisites)
	c.Assert(err, jc.ErrorIsNil)
	...
	c.Assert(commands, gc.DeepEquals, []string{
		"snap install core",
		"snap install --channel=4.4.30/stable --classic juju-db",
	})
}
...
func (*serviceSuite) TestInstall(c *gc.C) {
	...
	runnable.EXPECT().Execute("snap", []string{"install", "--channel=4.4.30/stable", "juju-db"}).Return("", nil)
