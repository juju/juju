...
In Juju, a **user** is any person who can interact with a Juju controller or its models. Traditionally, all users must log in to the controller. However, when a controller is configured with `allow-model-access=true`, users can be granted access to specific models without having any controller-level permissions, enabling model-only workflows.

## User types

**Local users** are managed directly by Juju and authenticate with controller-local credentials:

- The **admin user** is created implicitly when bootstrapping a controller. This user has the username `admin` and is prompted to create a password on first login.
- **Additional local users** are created explicitly via `juju add-user`. They receive the username assigned during creation and set their password when registering the controller with their Juju client.

**External users** authenticate via an external identity provider when the controller is configured with `identity-url`. They can be granted controller access or, when `allow-model-access=true` is enabled, model-only access without any controller-level permissions.
