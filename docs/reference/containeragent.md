...
```
*Where `containeragent` runs: one binary per unit pod, containing the
unit agent. The charm container holds the agent plus the charm code;
the workload container holds Pebble and the workload. The controller
pod's agent (`jujud`) runs the controller instead.*
