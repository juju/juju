...
| `--exclude-label` |  | Excludes log messages for the specified logging labels. |
| `--exclude-module` |  | Specifies the logging modules to exclude log messages from. |
| `-i`, `--include` |  | Specifies the entities to restrict log messages to. |
| `--include-label` |  | Specifies the logging labels to restrict log messages to. |
| `--include-module` |  | Specifies the logging modules to restrict log messages to. |
| `-l`, `--level` |  | Specifies the log level to show, one of `[TRACE, DEBUG, INFO, WARNING, ERROR]`. |
| `--limit` | 0 | Shows the specified number of the most recent logs and then exits. |
| `--location` | false | Shows filename and line numbers. |
| `-m`, `--model` |  | Specifies the model to operate in. Accepts `[<controller name>:]<model name>|<model UUID>`. |
| `--ms` | false | Shows times to millisecond precision. |
| `-n`, `--lines` | 0 | Shows the specified number of the most recent lines and continues to append new ones. |
| `--no-tail` | false | Shows existing log messages and then exits. |
| `--replay` | false | Shows the entire log and continues to append new ones. |
| `--retry` | false | Retries connection on failure. |
| `--retry-delay` | 1s | Specifies the retry delay between connection failure retries. |
| `--tail` | false | Shows existing log messages and continues to append new ones. |
| `--utc` | false | Shows times in UTC. |
| `-x`, `--exclude` |  | Specifies the entities to exclude log messages from. |
