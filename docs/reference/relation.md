...
:no-legend:
:caption: The relation connects applications THROUGH their units, and it happens in the controller's model database: each unit's agent reads and writes databag records there over the API -- its own unit databag (reads + writes; the relation_unit row stores the writing unit), every other unit's bag (reads), the application databag (the leader reads + writes it; writes are leader-gated). A remote application's units read MIRRORED copies of the local bags -- the offering model streams the application and unit settings over the cross-model facade into the remote model's own database -- and can never write the local bags. In a PEER relation everything lives in the one model database and the permissions turn inward: every unit reads every bag, the application databag included, leader or not. Users have read-only visibility via the API (juju show-unit), never write.
:alt: Two application panels plus a peer panel: unit agents above, each model database container below holding the databag records; arrows from the unit agents into their own unit databags, from the leader into the application databag, peer reads across; the local database mirrors its bags into the remote model's database, whose unit agents read the copies.
```
