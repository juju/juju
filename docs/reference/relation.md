...
:no-legend:
:caption: The databags are records in the host model's database, and the connection between applications runs through their units: each unit's agent reads and writes them over the API (relation-get/relation-set via the hook tools -> the unit agent's uniter facade). Each application has ONE application databag (per relation endpoint) and unit databags (one per unit). The databag records are labelled for the application (and unit) that owns them, so the arrows reduce to the verb: red arrows read + write, green arrows read only. Regular case (two sets): each unit reads + writes its OWN unit bag; the leader reads + writes its application's databag; ALL units of an application read all of the OTHER application's databags. Peer case (app A related to itself): ONE set, and the permissions turn inward -- the non-leader unit reads the application databag too. Users have read-only visibility via the API (juju show-unit), never write. Users have read-only visibility via the API (juju show-unit), never write.
