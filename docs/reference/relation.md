...
:no-legend:
:caption: The databags are records in the host model's database, and the connection between applications runs through their units: each unit's agent reads and writes them over the API (relation-get/relation-set via the hook tools -> the unit agent's uniter facade). Each application has ONE application databag (per relation endpoint) and unit databags (one per unit). The units (not their agents -- leadership is a property of the unit; the unit agent is the machinery doing the API work) read and write the databags, which are records in the host model's database: each unit's scope chip names its model. Red arrows read + write, green arrows read only, and each unit reads + writes ONLY the unit databag associated with its own unit -- the leader unit also reads + writes its application's application databag. Every unit reads the other units' unit databags and the other application's databags; the permission story is the same whether the applications share a model or not. Peer case (app A related to itself): the permissions turn inward -- the non-leader unit reads the application databag too. Users have read-only visibility via the API (juju show-unit), never write. Users have read-only visibility via the API (juju show-unit), never write.
