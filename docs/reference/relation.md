...
:no-legend:
:caption: The databags are records in the host model's database, and the connection between applications runs through their units: each unit's agent reads and writes them over the API (relation-get/relation-set via the hook tools -> the unit agent's uniter facade). Each application has ONE application databag (per relation endpoint) and unit databags (one per unit). The databag records are labelled for the application (and unit) that owns them, so the arrows reduce to the verb: red arrows read + write, green arrows read only. Each unit agent reads + writes ONLY the unit databag associated with its own unit -- the leader unit agent additionally reads + writes its application's application databag. Every unit agent reads the OTHER units' unit databags and (in the regular case) the other application's databags. Peer case (app A related to itself): the permissions turn inward -- the non-leader unit reads the application databag too. Users have read-only visibility via the API (juju show-unit), never write. Users have read-only visibility via the API (juju show-unit), never write.
:alt: Two unit-agent panels above one model-database container holding two sets of databags (application A's and application B's application databag and unit databags); red arrows reading and writing the own set, green arrows reading across to the other set; below, the peer panel: one set of databags with red own/leader arrows and a green unit read of the application databag.
```
