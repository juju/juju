...
:no-legend:
:caption: The relation connects applications THROUGH their units, and it happens in each model's database: each unit's agent reads and writes databag records in its own model's database over the API -- its own unit databag (reads + writes; the relation_unit row stores the writing unit), every other unit's bag (reads), the application databag (the leader reads + writes it; writes are leader-gated). In a cross-model relation EACH model database also holds MIRRORED copies of the other application's bags -- the facades stream the settings both ways (watchOneRelationChanges down, PublishRelationChanges back) -- and the units read those copies from their own database, never writing across. In a PEER relation everything lives in the one model database and the permissions turn inward: every unit reads every bag, the application databag included, leader or not. Red arrows read + write; green arrows read only. Users have read-only visibility via the API (juju show-unit), never write.
:alt: Two model-database panels plus a peer panel: unit agents above each database container; red arrows reading and writing their own unit databags and (as leader) the application databag; green arrows reading peer bags and the mirrored copies of the other application's bags that each database holds; a bidirectional streaming arrow between the two databases.
```
