...
Note: While the various clients generally aim for feature parity, there are still differences between them coming from the nature of the client (e.g., interactive vs. declarative vs. programmatic). Also, the only client that can currently bootstrap a Juju controller is the `juju` CLI client.