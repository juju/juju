...
A script is not a record of its own: what Juju persists is the
**execution**. A script run creates an operation whose parameters
carry the command and its timeout, plus one {ref}`task <task>` per
target -- the same records an action run creates (see
{ref}`the action in the data model
<the-action-in-the-data-model>`). An exec run is modelled as the
predefined `juju-exec` action; there is no separate script table.
