...
:caption: On a machine cloud, `jujud` runs every agent role. The machine agent's workers manage the machine itself; the unit agent runs the charm; a separate controller agent (not shown here) carries the model and controller roles with the embedded Dqlite database.
```
