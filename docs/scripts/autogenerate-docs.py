...
def generate_model_config_docs(force=False):
    config_reference_dir = 'reference/configuration/'
    ...
    if not force and os.path.exists(model_config_file):
        print("model config docs already present, skipping generation. "
              "Run 'make regenerate-docs' to regenerate after changing Go source.")
        return

    # Generate the model config using script. The script prints the generated
    # Markdown to stdout, so no root directory argument is needed.
...
generate_model_config_docs(args.force)
