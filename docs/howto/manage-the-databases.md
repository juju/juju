...
:caption: Topology: The cluster you are inspecting: every controller node runs Dqlite embedded in its agent process, and the three databases raft-replicate each other. `juju-db-repl` attaches to one of them with the controller or model namespace.
```
