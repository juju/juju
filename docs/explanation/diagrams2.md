...
:no-legend:
:caption: The execution chain as a worker tree — the controller drives the machine agent, which hosts the unit agent, which runs the uniter, which dispatches the charm. The controller-side manifold tree (the full dependency engine) remains unillustrated.
