...
:no-legend:
:caption: The execution chain as a worker tree — the controller drives the machine agent, which hosts the unit agent, which runs the uniter, which dispatches the charm. See [Worker tree (controller)](#worker-tree-controller) for the controller-side dependency engine.
