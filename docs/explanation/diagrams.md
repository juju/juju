...
:no-legend:
:caption: The relation databags are records in each model's database (on the controller), and the connection between applications runs through their units: each unit's AGENT reads and writes those records over the API -- its own unit databag (reads + writes; the relation_unit row stores the writing unit), every other unit's bag (reads), the application databag (leader reads + writes; writes leader-gated). The cross-model facade streams the bags into the REMOTE model's database (watchOneRelationChanges -> SetRelationRemoteApplicationAndUnitSettings) and the remote units read those mirrored copies. Peer panel: one model database, and the watcher watches the OWN endpoint's app settings and all peers -- every unit reads every bag, the application databag included, leader or not. Users have read-only visibility via the API (juju show-unit), never write.
:alt: Two application panels plus a peer panel: unit agents above, their controller's model database container below holding the application and unit databag records; arrows from the unit agents into the bags; the local database mirroring its bags into the remote application's database.
```
