...
:no-legend:
:caption: The databags are records in the host model's database (on the controller), and the connection between applications runs through their units: each unit's AGENT reads and writes them over the API (relation-get/relation-set via the hook tools -> the unit agent's uniter facade). Each application has ONE application databag (per relation endpoint) and unit databags (one per unit). The databag records are labelled for the application (and unit) that owns them, so the arrows reduce to the verb: red = reads + writes, green = reads. Regular panel: two sets in the one database -- each unit reads + writes its OWN unit bag, the leader reads + writes its application's databag, and ALL units of an application read all of the OTHER application's databags. Peer panel (app A related to itself): ONE set, permissions turned inward -- every unit reads every bag and the application databag (red own/leader writes; green reads). Users have read-only visibility via the API (juju show-unit), never write.
