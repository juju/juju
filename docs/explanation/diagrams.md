...
:no-legend:
:caption: The databags are records in the host model's database (on the controller), and the connection between applications runs through their units: each unit's AGENT reads and writes them over the API (relation-get/relation-set via the hook tools -> the unit agent's uniter facade). Each application has ONE application databag (per relation endpoint) and unit databags (one per unit). The databag records are labelled for the application (and unit) that owns them, so the arrows reduce to the verb: red = reads + writes, green = reads. Each unit agent reads + writes ONLY the unit databag associated with its own unit; the leader unit agent additionally reads + writes its application's application databag. Every unit agent reads the other units' unit databags and, in the regular case, the other application's databags. Peer panel (app A related to itself): the permissions turn inward -- the non-leader unit reads the application databag too. Users have read-only visibility via the API (juju show-unit), never write.
