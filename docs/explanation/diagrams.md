...
:no-legend:
:caption: The databags are records in each model's database, and the connection between applications runs through their units: each unit's AGENT reads and writes the records in its own model's database over the API (juju relation-get/relation-set via the hook tools -> the unit agent's uniter facade) -- its own unit databag (reads + writes; the relation_unit row stores the writing unit), every other unit's bag (reads), the application databag (leader reads + writes; writes leader-gated). Red arrows read + write, green arrows read only. In a cross-model relation EACH database also holds mirrored copies of the other side's bags -- the facades stream both ways (watchOneRelationChanges; PublishRelationChanges) -- and the units read the copies from their own database, never writing across. Peer panel: one model database, the watcher watches the OWN endpoint's app settings and all peers -- every unit reads every bag, the application databag included, leader or not. Users have read-only visibility via the API (juju show-unit), never write.
:alt: Two model-database panels plus a peer panel: unit agents above their databases; red arrows reading and writing their own unit databags and the application databag (as leader); green arrows reading peer bags and the mirrored copies of the other application's bags; a bidirectional streaming arrow between the two databases.
```
