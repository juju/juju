...
func (s *CachingAPIHostPortsSetter) SetAPIHostPorts(apiServers []network.SpaceHostPorts, config controller.Config) error {
	if len(apiServers) == 0 {
	...
	if err := s.APIHostPortsSetter.SetAPIHostPorts(sorted, config); err != nil {
