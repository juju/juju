...
func (config ManifoldConfig) newWorker(a agent.Agent, apiCaller base.APICaller) (worker.Worker, error) {
	...
	// Curry the NewFacade method and pass it as a factory.
	...
	// Curry the Upgrader factory function so we only need to request using the
	// getter for the target OS series.
	newUpgrader := func(targetSeries func() (string, error)) (Upgrader, error) {
