...
// DefaultDialOpts returns a set of dial options for use when connecting.
func DefaultDialOpts() api.DialOpts {
	return api.DialOpts{
...
// password.
func (s *ConnectStrategy) Connect(info *api.Info, fallbackPassword string) (api.Connection, bool, error) {
	var (
		err         error
		conn        api.Connection
		useFallback bool
	...
	if info.Password != "" {
		...
		} else if params.IsCodeUnauthorized(err) || errors.Cause(err) == common.ErrBadCreds {
			useFallback = true
		}
	} else {
		useFallback = true
