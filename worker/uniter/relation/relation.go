...
func (s *State) Validate(hi hook.Info) (err error) {
	defer errors.Contextf(&err, "inappropriate %q for %q", hi.Kind, hi.RemoteUnit)
...
func ReadStateDir(dirPath string, relationId int) (d *StateDir, err error) {
	...
	defer errors.Contextf(&err, "cannot load relation state from %q", d.path)
...
func ReadAllStateDirs(dirPath string) (dirs map[int]*StateDir, err error) {
	defer errors.Contextf(&err, "cannot load relations state from %q", dirPath)
...
func (d *StateDir) Write(hi hook.Info) (err error) {
	defer errors.Contextf(&err, "failed to write %q hook info for %q on state directory", hi.Kind, hi.RemoteUnit)
