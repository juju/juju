...
func (s *HookSenderSuite) TestHandlesUpdatesFullQueue(c *gc.C) {
	source := &updateSource{
		changes: make(chan juju.RelationUnitsChange),
		updates: make(chan juju.RelationUnitsChange),
	}
	out := make(chan hook.Info)
	sender := relation.NewHookSender(out, source)
	defer statetesting.AssertStop(c, sender)
	...
	sent := juju.RelationUnitsChange{Departed: []string{"sent"}}
	select {
	...
	notSent := juju.RelationUnitsChange{Departed: []string{"notSent"}}
	select {
...
func (s *HookSenderSuite) TestHandlesUpdatesFullQueueSpam(c *gc.C) {
	...
	for i := 0; i < 100; i++ {
		...
		case hi, ok := <-out:
			...
			hookCount++
		case source.changes <- juju.RelationUnitsChange{}:
			changeCount++
		case update, ok := <-source.updates:
			c.Assert(ok, jc.IsTrue)
			c.Assert(update, gc.DeepEquals, juju.RelationUnitsChange{})
			updateCount++
		case <-timeout:
...
type updateSource struct {
	...
	updates chan juju.RelationUnitsChange
}
...
func (source *updateSource) Changes() <-chan juju.RelationUnitsChange {
	return source.changes
}

func (source *updateSource) Update(change juju.RelationUnitsChange) error {
	select {
	case <-time.After(coretesting.LongWait):
