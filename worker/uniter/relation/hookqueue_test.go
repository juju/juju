...
func (d send) check(c *gc.C, in chan multiwatcher.RelationUnitsChange, out chan hook.Info) {
	in <- d.event()
}
...
func (d advance) check(c *gc.C, in chan multiwatcher.RelationUnitsChange, out chan hook.Info) {
	for i := 0; i < d.count; i++ {
		select {
		case <-out:
...
func (d expect) check(c *gc.C, in chan multiwatcher.RelationUnitsChange, out chan hook.Info) {
	if d.hook == "" {
		select {
		case unexpected := <-out:
