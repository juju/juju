...
func (runner *runner) RunCommands(ctx stdcontext.Context, commands string, runLocation RunLocation) (*utilexec.ExecResponse, error) {
	...
	result, err := runner.runCommandsWithTimeout(ctx, commands, 0, clock.WallClock, rMode, nil)
...
func (runner *runner) runCommandsWithTimeout(ctx stdcontext.Context, commands string, timeout time.Duration, clock clock.Clock, rMode runMode, abort <-chan struct{}) (*utilexec.ExecResponse, error) {
	var err error
	token := ""
...
func (runner *runner) runJujuExecAction(ctx stdcontext.Context) (err error) {
	...
	results, err := runner.runCommandsWithTimeout(ctx, command, time.Duration(timeout), clock.WallClock, rMode, data.Cancel)
	if results != nil {
