...
func (s *containerWorkerSuite) stubOutProvisioner(ctrl *gomock.Controller) {
	...
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "WatchForModelConfigChanges", nil, gomock.Any()).SetArg(5, notifySource).Return(nil).AnyTimes()
	...
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "ModelConfig", nil, gomock.Any()).SetArg(5, modelCfgSource).Return(nil).AnyTimes()

	addrSource := params.StringsResult{Result: []string{"0.0.0.0"}}
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "StateAddresses", nil, gomock.Any()).SetArg(5, addrSource).Return(nil).AnyTimes()
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "APIAddresses", nil, gomock.Any()).SetArg(5, addrSource).Return(nil).AnyTimes()

	certSource := params.BytesResult{Result: []byte(coretesting.CACert)}
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "CACert", nil, gomock.Any()).SetArg(5, certSource).Return(nil).AnyTimes()

	uuidSource := params.StringResult{Result: s.modelUUID.String()}
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "ModelUUID", nil, gomock.Any()).SetArg(5, uuidSource).Return(nil).AnyTimes()

	lifeSource := params.LifeResults{Results: []params.LifeResult{{Life: life.Alive}}}
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "Life", gomock.Any(), gomock.Any()).SetArg(5, lifeSource).Return(nil).AnyTimes()

	watchSource := params.StringsWatchResults{Results: []params.StringsWatchResult{{
		StringsWatcherId: "whatever",
		Changes:          []string{},
	}}}
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "WatchContainers", gomock.Any(), gomock.Any()).SetArg(5, watchSource).Return(nil).AnyTimes()
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "WatchContainersCharmProfiles", gomock.Any(), gomock.Any()).SetArg(5, watchSource).Return(nil).AnyTimes()

	controllerCfgSource := params.ControllerConfigResult{
		Config: map[string]interface{}{"controller-uuid": s.controllerUUID.String()},
	}
	fExp.APICall(gomock.Any(), "Provisioner", 0, "", "ControllerConfig", nil, gomock.Any()).SetArg(5, controllerCfgSource).Return(nil).AnyTimes()
}
