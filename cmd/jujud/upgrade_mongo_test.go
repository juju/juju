...
func (s *UpgradeMongoCommandSuite) TestRun(c *gc.C) {
	...
	expectedCommands := [][]string{
		{"getenv", "UPSTART_JOB"},
		{"service.DiscoverService", "bogus_daemon"},
		{"CreateTempDir"},
		{"SatisfyPrerequisites"},
		{"CreateTempDir"},
		{"mongo.StopService"},
		{"stat", "/var/lib/juju/db"},
		{"mkdir", "/fake/temp/dir/24"},
		{"fs.Copy", "/var/lib/juju/db", "/fake/temp/dir/24/db"},
		{"mongo.StartService"},
		{"mongo.StopService"},
		{"/usr/lib/juju/bin/mongod", "--dbpath", "/var/lib/juju/db", "--replSet", "juju", "--upgrade"},
		{"mongo.EnsureServiceInstalled", testDir, "69", "0", "false", "2.6/mmapv1", "true", "low"},
		{"mongo.StartService"},
		{"DialAndlogin"},
		...
		{"mongo.StopService"},
		{"mongo.EnsureServiceInstalled", testDir, "69", "0", "false", "3.2/mmapv1", "true", "low"},
		{"mongo.StartService"},
		{"/usr/lib/juju/mongo3.2/bin/mongodump", "--ssl", "-u", "admin", "-p", "sekrit", "--port", "69", "--host", "localhost", "--out", "/fake/temp/dir/migrateToTigerdump"},
		{"mongo.StopService"},
		{"stat", dbDir},
		{"remove", dbDir},
		{"mkdir", dbDir},
		{"mongo.EnsureServiceInstalled", testDir, "69", "0", "false", "3.2/wiredTiger", "false", "low"},
		{"mongo.DialInfo"},
		{"mongo.StartService"},
		{"peergrouper.InitiateMongoServer"},
		{"/usr/lib/juju/mongo3.2/bin/mongorestore", "--ssl", "--port", "69", "--host", "localhost", "--sslAllowInvalidCertificates", "--batchSize", "100", "/fake/temp/dir/migrateToTigerdump"},
		{"mongo.EnsureServiceInstalled", testDir, "69", "0", "false", "3.2/wiredTiger", "true", "low"},
		{"mongo.ReStartService"},
	}
...
func (s *UpgradeMongoCommandSuite) TestRunRollback(c *gc.C) {
	...
	expectedCommands := [][]string{
		...
		{"mongo.StopService"},
		{"/usr/lib/juju/bin/mongod", "--dbpath", "/var/lib/juju/db", "--replSet", "juju", "--upgrade"},
		{"mongo.EnsureServiceInstalled", tempDir, "69", "0", "false", "2.6/mmapv1", "true", "low"},
		{"mongo.StartService"},
		{"DialAndlogin"},
		{"mongo.ReStartServiceFail"},
		{"mongo.StopService"},
		{"remove", "/var/lib/juju/db"},
		{"mongo.StartService"},
	}
