...
func (c *imageMetadataCommand) setParams(cmdContext *cmd.Context, base corebase.Base) (corebase.Base, error) {
	...
	if err == nil {
		if environ, err := prepare(cmdContext, controllerName, c.ClientStore()); err == nil {
			logger.Infof(context.TODO(), "creating image metadata for model %q", environ.Config().Name())
			...
			logger.Warningf(context.TODO(), "bootstrap parameters could not be opened: %v", err)
		}
	}
	if environ == nil {
		logger.Infof(context.TODO(), "no model found, creating image metadata using user supplied data")
	}
	if c.ImageId == "" {
	...
	if c.Dir == "" {
		logger.Infof(context.TODO(), "no destination directory specified, using current directory")
		var err error
