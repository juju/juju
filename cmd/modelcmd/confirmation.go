...
func (c *DestroyConfirmationCommandBase) SetFlags(f *gnuflag.FlagSet) {
	f.BoolVar(&c.assumeNoPrompt, "no-prompt", false, "Specifies whether to skip confirmation prompt.")
}
...
func (c *RemoveConfirmationCommandBase) SetFlags(f *gnuflag.FlagSet) {
	f.BoolVar(&c.assumeNoPrompt, "no-prompt", false, "Specifies whether to skip confirmation prompt. Overrides `mode` model config setting.")
}
