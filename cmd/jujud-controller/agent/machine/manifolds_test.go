...
func (s *ManifoldsSuite) TestObjectStoreGuardsUsed(c *tc.C) {
	exempt := set.NewStrings(
		"agent",
		"agent-config-updater",
		"api-address-updater",
	...
	for name, manifold := range manifolds {
		c.Logf(name)
		if !exempt.Contains(name) {
			checkContains(c, manifold.Inputs, "object-store-fortress")
			checkContains(c, manifold.Inputs, "object-store-draining-flag")
		}
	}
}
