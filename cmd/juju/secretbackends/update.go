...
Updates an existing secret backend for storing secret content.
...
func (c *updateSecretBackendCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.BoolVar(&c.Force, "force", false, "Specifies whether to force update even if the backend is unreachable.")
}
