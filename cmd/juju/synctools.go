...
func (c *SyncToolsCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.StringVar(&c.source, "source", "", "choose a location on the file system as source")
	f.StringVar(&c.destination, "destination", "", "choose a location on the file system as destination")
