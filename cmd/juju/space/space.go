...
type SpaceAPI interface {
	...
	MoveToSpace(name string, subnetIds []string, force bool) ([]network.MovedSpace, error)
...
func (m *APIShim) MoveToSpace(name string, subnetIds []string, force bool) ([]network.MovedSpace, error) {
	return m.facade.MoveToSpace(name, subnetIds, force)
}
