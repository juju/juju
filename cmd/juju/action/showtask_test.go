...
func (s *ShowTaskSuite) SetUpTest(c *gc.C) {
...
func (s *ShowTaskSuite) TestInit(c *gc.C) {
	...
	for i, t := range tests {
		for _, modelFlag := range s.modelFlags {
			...
			cmd, _ := action.NewShowTaskCommandForTest(s.store, nil)
...
//		expectedErr:       `tasks for id "` + validActionId + `" not found`,
//	}, {
...
//		withTags:          tagsForIdPrefix(validActionId, validActionTagString),
//		withAPIResponse:   []params.ActionResult{},
//		expectedErr:       "no results for task " + validActionId,
//	}, {
...
func testRunHelper(c *gc.C, s *ShowTaskSuite, client *fakeAPIClient,
	...
	cmd, _ := action.NewShowTaskCommandForTest(s.store, func(_ *cmd.Context, msg string) {
