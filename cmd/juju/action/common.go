...
func formatActionResult(id string, result actionapi.ActionResult, utc bool) (map[string]interface{}, int) {
	...
	if len(result.Log) > 0 {
		...
		for _, msg := range result.Log {
			logs = append(logs, formatLogMessage(action.ActionMessage{
...
func decodeLogMessage(encodedMessage string, utc bool) (string, error) {
	var actionMessage action.ActionMessage
...
func formatLogMessage(actionMessage action.ActionMessage, progressFormat, utc, plain bool) string {
