...
func (c *runCommandBase) setNonFormatFlags(f *gnuflag.FlagSet) {
	...
	f.BoolVar(&c.background, "background", false, "Specifies whether to run the task in the background.")
	f.DurationVar(&c.wait, "wait", 0, "Specifies the maximum wait time for a task to complete.")
	f.BoolVar(&c.noColor, "no-color", false, "Specifies whether to disable ANSI color codes in output.")
