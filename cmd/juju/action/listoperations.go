...
func (c *listOperationsCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.Var(cmd.NewStringsValue(nil, &c.actionNames), "actions", "Specifies a comma-separated list of action names to filter on.")
	...
	f.UintVar(&c.offset, "offset", 0, "Returns operations from offset onwards.")
}
