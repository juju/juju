...
func (c *UpdateCAASCommand) Run(ctx *cmd.Context) (err error) {
	...
	if newCloud.Type != caasconstants.CAASProviderType {
		ctx.Infof("The %q cloud is a %q cloud and not a %q cloud.", c.caasName, newCloud.Type, caasconstants.CAASProviderType)
		return cmd.ErrSilent
	}
	...
	if c.ControllerName != "" {
		...
		if existing.Type != caasconstants.CAASProviderType {
			ctx.Infof("The %q cloud on the controller is a %q cloud and not a %q cloud.", c.caasName, existing.Type, caasconstants.CAASProviderType)
			return cmd.ErrSilent
		}
