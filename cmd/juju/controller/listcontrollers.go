...
func (c *listControllersCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.BoolVar(&c.refresh, "refresh", false, "Specifies whether to connect to each controller to download the latest details.")
	f.BoolVar(&c.managed, "managed", false, "Specifies whether to show controllers managed by JAAS.")
