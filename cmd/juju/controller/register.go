...
func (c *registerCommand) ensureNotKnownEndpoint(user string, controllerAddrs []string) error {
	...
	return errors.Errorf(`A controller with the same API address has already been registered on this client as %q.
To login run 'juju login -c %s'.`, existingName, existingName)
}
...
A controller with the same API address has already been registered on this client as "{{.ControllerName}}".
