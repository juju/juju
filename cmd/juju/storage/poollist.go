...
func (c *poolListCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.Var(cmd.NewAppendStringsValue(&c.Providers), "provider", "Only shows pools of these provider types.")
	f.Var(cmd.NewAppendStringsValue(&c.Names), "name", "Only shows pools with these names.")
