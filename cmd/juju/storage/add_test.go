		...
		addToUnitFunc: func(storages []params.StorageAddParams) ([]params.AddStorageResult, error) {
			...
			for i, one := range storages {
				if strings.HasPrefix(one.StorageName, "err") {
					result[i].Error = commonerrors.ServerError(errors.Errorf("test failure"))
					continue
				}
	...
	s.mockAPI.addToUnitFunc = func(storages []params.StorageAddParams) ([]params.AddStorageResult, error) {
		...
		for i, one := range storages {
			if one.StorageName == "storage2" {
				result[i].Error = commonerrors.ServerError(errors.Errorf(`storage pool "barf" not found`))
			}
			if one.StorageName == "storage42" {
				result[i].Error = commonerrors.ServerError(errors.Errorf(`storage "storage42" not found`))
			}
		}
		return result, nil
	}
	...
	s.mockAPI.addToUnitFunc = func(storages []params.StorageAddParams) ([]params.AddStorageResult, error) {
		...
		for i, one := range storages {
			if one.StorageName == "storage42" || one.StorageName == "storage2" {
				result[i].Error = commonerrors.ServerError(errors.Errorf(`storage "%v" not found`, one.StorageName))
			}
		}
		return result, nil
	}
...
`[1:]
	...
	s.mockAPI.addToUnitFunc = func(storages []params.StorageAddParams) ([]params.AddStorageResult, error) {
		...
		for i, one := range storages {
			if one.StorageName == "storage42" || one.StorageName == "storage2" {
				result[i].Error = commonerrors.ServerError(errors.New(unitErr))
			} else {
				result[i].Error = commonerrors.ServerError(errors.Errorf(`storage "%v" not found`, one.StorageName))
			}
		}
...
func (s *addSuite) TestCollapseUnitErrors(c *gc.C) {
	...
	s.mockAPI.addToUnitFunc = func(storages []params.StorageAddParams) ([]params.AddStorageResult, error) {
		...
		for i := range storages {
			result[i].Error = commonerrors.ServerError(errors.New(expectedErr))
		}
		return result, nil
	}
