...
func (c *downloadCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.BoolVar(&c.resources, "resources", false, "download the resources associated with the charm")
}
...
Calculated: %s`, c.charmOrBundle, entitySHA, calculatedHash)
	...
	if c.resources {
		...
		for _, resource := range entity.Resources {
			rscPath := filepath.Join(dir, fmt.Sprintf("resource_%s_r%d_%s", resource.Name, resource.Revision, resource.Filename))
