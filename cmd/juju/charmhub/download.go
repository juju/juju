...
func (c *downloadCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.StringVar(&c.series, "series", SeriesAll, "(DEPRECATED) Specifies a series. Deprecated: use `--base`.")
	...
	f.BoolVar(&c.noProgress, "no-progress", false, "Specifies whether to disable the progress bar.")
	f.BoolVar(&c.resources, "resources", false, "(DEPRECATED IN JUJU 4.0 BECAUSE DEFAULT BEHAVIOR) Specifies whether to download the resources associated with the charm (will be default behaviour in 4.0).")
}
