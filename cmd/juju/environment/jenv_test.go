...
func (*jenvSuite) TestConfigStoreError(c *gc.C) {
	...
	home := gitjujutesting.JujuDataPath()
	err := os.Chmod(home, 0)
	c.Assert(err, jc.ErrorIsNil)
...
func (*jenvSuite) TestWriteError(c *gc.C) {
	...
	envsDir := gitjujutesting.JujuDataPath("environments")
...
func (*jenvSuite) TestSwitchErrorCannotWriteCurrentEnvironment(c *gc.C) {
	...
	currentEnvPath := gitjujutesting.JujuDataPath(envcmd.CurrentEnvironmentFilename)
...
func assertJenvContents(c *gc.C, contents []byte, envName string) {
	path := gitjujutesting.JujuDataPath("environments", envName+".jenv")
