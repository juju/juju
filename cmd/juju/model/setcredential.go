...
func (c *modelCredentialCommand) Info() *cmd.Info {
	return &cmd.Info{
		...
		Purpose: "Links a remote credential to a model.",
...
This command links a credential cached on a controller to a specific model. It
does not change/update the contents of an existing active credential. See
command ` + "`update-credential`" + ` for that.

The credential specified may exist locally (on the client), remotely (on the
controller), or both. The command will error out if the credential is neither
remotely nor locally stored.

When remote, the credential will be linked to the specified model.

When local and not remote, the credential will first be uploaded to the
controller and then linked.

Examples:

For cloud 'aws', link remote credential 'bob' to model 'trinity':
