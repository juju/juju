...
Firewall rules have been moved to model-config settings "ssh-allowlist" and
"application-offer-ingress-allowlist". This command is deprectaed in favour of
...
func (c *setFirewallRuleCommand) Run(ctx *cmd.Context) error {
	...
	case firewall.JujuApplicationOfferRule:
		err = client.ModelSet(map[string]interface{}{config.ApplicationOfferIngressAllowListKey: c.allowlist})
	default:
