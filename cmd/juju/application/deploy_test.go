...
type fakeCharmstoreClientShim struct {
	*apiservertesting.Client
}

func (c *fakeCharmstoreClientShim) WithChannel(channel csparams.Channel) charmstoreCommunicator {
...
func (s *DeploySuiteBase) SetUpTest(c *gc.C) {
	...
	charmrepo := apiservertesting.NewRepository()
	fakeClient := apiservertesting.NewClient(*charmrepo)
	s.client = fakeClient
	s.charmstore = &fakeCharmstoreClientShim{fakeClient}
	s.charmrepo = charmrepo
}
...
func runDeployWithOutput(c *gc.C, charmstore charmstoreCommunicator, charmrepo charmRepository, args ...string) (string, string, error) {
	ctx, err := cmdtesting.RunCommand(c, NewDeployCommandForTest2(charmstore, charmrepo), args...)
...
// runDeploy executes the deploy command in order to deploy the given
// charm or bundle. The deployment stderr output and error are returned.
func runDeploy(c *gc.C, charmStore charmstoreCommunicator, charmrepo charmRepository, args ...string) error {
	_, _, err := runDeployWithOutput(c, charmStore, charmrepo, args...)
	return err
}
...
func (s *DeploySuite) runDeploy(c *gc.C, args ...string) error {
	return runDeploy(c, s.charmstore, s.charmrepo, args...)
}

func (s *DeploySuite) runDeployWithOutput(c *gc.C, args ...string) (string, string, error) {
...
func (s *CAASDeploySuiteBase) SetUpTest(c *gc.C) {
	...
	s.api = s.fakeAPI()
	//	s.CleanupSuite.AddCleanup(func(*gc.C) { s.server.Close() })
...
func (s *CAASDeploySuite) runDeploy(c *gc.C, args ...string) (*cmd.Context, error) {
...
func (s *CAASDeploySuite) TestLocalCharmNeedsResources(c *gc.C) {
	...
	// TODO(tsm)
	//   This isn't quite right. We need to alter the with* functions
	//   to set things up for what CAAS expects
	charmDir := repo.ClonedDir(s.CharmsPath, "mariadb")
	charmId := charm.MustParseURL("local:mariadb")
	withLocalCharmDeployable(s.api, charmId, charmDir, false)
	withCharmDeployable(s.api,
		charmId,
		"",
		charmDir.Meta(),
		charmDir.Metrics(),
	...
	_, err = s.runDeploy(c, charmDir.Path, "-m", m.Name(), "--resource", "mysql_image=abc")
	c.Assert(err, gc.ErrorMatches, "local charm missing OCI images for: another_image")

	_, err = s.runDeploy(c, charmDir.Path, "-m", m.Name(), "--resource", "mysql_image=abc", "--resource", "another_image=zxc")
	c.Assert(err, jc.ErrorIsNil)
}
...
func (s *CAASDeploySuite) TestDevices(c *gc.C) {
	...
	_, err = s.runDeploy(c, "bitcoin-miner", "-m", m.Name(), "--device", "bitcoinminer=10,nvidia.com/gpu", "--series", "kubernetes")
	c.Assert(err, jc.ErrorIsNil)
...
func (s *DeployCharmStoreSuite) TestDeployWithTermsSuccess(c *gc.C) {
	...
	_, stdErr, err := s.runDeployWithOutput(c, "trusty/terms1")
	c.Assert(err, jc.ErrorIsNil)
	expectedOutput := `
...
func (s *DeployCharmStoreSuite) TestDeployWithTermsNotSigned(c *gc.C) {
	...
	// err := s.runDeploy(c, "quantal/terms1")
...
func (s *DeployCharmStoreSuite) TestDeployWithChannel(c *gc.C) {
	...
	err = s.runDeploy(c, "--channel", "edge", "~client-username/wordpress")
	c.Assert(err, gc.IsNil)
...
type charmStoreSuite struct {
	...
	client     testcharms.Charmstore // TODO(tsm) use interface
	charmstore charmstoreCommunicator
}
...
func (s *charmStoreSuite) SetUpTest(c *gc.C) {
	...
	s.client = apiservertesting.NewClient(*s.charmrepo.(*apiservertesting.Repository))
}
...
func (s *DeployCharmStoreSuite) TestDeployCharmWithSomeEndpointBindingsSpecifiedSuccess(c *gc.C) {
	...
	charmName := "cs:quantal/wordpress-extra-bindings-1"
	url := charm.MustParseURL(charmName)
	_, ch := testcharms.UploadCharm(c, s.client, charmName, "wordpress-extra-bindings")
	fakeAPI := vanillaFakeModelAPI(cfgAttrs)
	fakeAPI.planURL = server.URL
	withCharmDeployable(fakeAPI, url, "quantal", ch.Meta(), ch.Metrics(), false, false, 1, nil, nil)
	withCharmRepoResolvable(fakeAPI, url)
	...
	fakeAPI.Call("Deploy", application.DeployArgs{
		CharmID:          jjcharmstore.CharmID{URL: url, Channel: ""},
		ApplicationName:  "wordpress-extra-bindings",
		Series:           "quantal",
		NumUnits:         1,
		EndpointBindings: map[string]string{"": "public", "admin-api": "public", "db": "db", "db-client": "db"},
	}).Returns(error(nil))

	args := []string{url.String(), "--bind", "db=db db-client=db public admin-api=public"}
	_, err = cmdtesting.RunCommand(c, modelcmd.Wrap(deploy), args...)
	c.Assert(err, jc.ErrorIsNil)
	// TODO(tsm) fix test assertions
	//
	// s.assertApplicationsDeployed(c, map[string]applicationInfo{
