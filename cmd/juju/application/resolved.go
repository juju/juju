...
func (c *resolvedCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.BoolVar(&c.NoRetry, "no-retry", false, "Specifies whether to skip re-execution of failed hooks on the unit.")
	f.BoolVar(&c.All, "all", false, "Specifies whether to mark all units in error as resolved.")
}
