...
between units when direct connectivity is restricted by firewall rules. Charms define the logic for
...
Applications can be integrated across different models, even when those models
are managed by different controllers or hosted on different clouds. This
functionality is known as 'cross-model relation' (CMR).

Cross-model relations always require an offer. Use ` + "`juju offer`" + ` to create an offer
for the remote application, then integrate using the offer URL:

    juju integrate <application>[:<endpoint>] <offer-url>[:<endpoint>]

The offer URL takes the form:

    [[<controller>:]<user>/]<model-name>.<application-name>

The ` + "`<controller>:`" + ` part is only required when models are on different controllers.

Example with controller specified (different controllers):

    juju integrate <application>[:<endpoint>] <controller>:<user>/<model-name>.<application-name>[:<endpoint>]

You can also use an offer alias created with ` + "`juju consume`" + `:

    juju integrate <application>[:<endpoint>] <offer-alias>

When the local application is behind a firewall or NAT, use the ` + "`--via`" + ` option
to specify egress subnets so the remote application can open the necessary ports.

For detailed instructions, see the how-to guide on cross-model relations.
