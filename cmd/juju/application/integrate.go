...
A CMR always involves an offer. The admin of the offering model uses
` + "`juju offer`" + ` to make an application available, then the consuming side
integrates with it via an offer endpoint:

    juju integrate <application>[:<endpoint>] <offer-endpoint>

` + "`<offer-endpoint>`" + ` takes one of two forms:

    <offer-alias>
    <offer-url>[:<endpoint>]

` + "`<offer-alias>`" + ` is an alias defined by the ` + "`juju consume`" + ` command.

` + "`<offer-url>`" + ` identifies the offer across models and controllers:
...
To consume an offer on another controller, name the controller explicitly
in the offer path; the offering controller must be registered locally.
