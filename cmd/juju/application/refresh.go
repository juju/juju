...
func (c *refreshCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.BoolVar(&c.Force, "force", false, "Allows a charm to be refreshed bypassing LXD profile allow list.")
	...
	f.BoolVar(&c.ForceBase, "force-series", false, "(DEPRECATED) Refreshes even if the series of the deployed application is not supported by the new charm. Deprecated: Use `--force-base` instead.")
	...
	f.IntVar(&c.Revision, "revision", -1, "Specifies the explicit revision of current charm.")
	f.Var(stringMap{&c.Resources}, "resource", "Specifies a resource to be uploaded to the controller.")
	f.Var(storageFlag{&c.Storage, nil}, "storage", "Specifies charm storage constraints.")
