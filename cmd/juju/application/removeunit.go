...
func (c *removeUnitCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.IntVar(&c.NumUnits, "num-units", 0, "Specifies the number of units to remove (Kubernetes models only).")
	f.BoolVar(&c.DestroyStorage, "destroy-storage", false, "Destroys storage attached to the unit.")
	f.BoolVar(&c.Force, "force", false, "Completely removes an unit and all its dependencies.")
	f.BoolVar(&c.NoWait, "no-wait", false, "Rushes through unit removal without waiting for each individual step to complete.")
	c.fs = f
}
