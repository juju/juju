...
func (c *debugLogCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.Var(cmd.NewAppendStringsValue(&c.params.IncludeEntity), "i", "Specifies the entities to include.")
	f.Var(cmd.NewAppendStringsValue(&c.params.IncludeEntity), "include", "Specifies the entities to include.")
	f.Var(cmd.NewAppendStringsValue(&c.params.ExcludeEntity), "x", "Specifies the entities to exclude.")
	f.Var(cmd.NewAppendStringsValue(&c.params.ExcludeEntity), "exclude", "Specifies the entities to exclude.")
	f.Var(cmd.NewAppendStringsValue(&c.params.IncludeModule), "include-module", "Specifies the logging modules to include.")
	f.Var(cmd.NewAppendStringsValue(&c.params.ExcludeModule), "exclude-module", "Specifies the logging modules to exclude.")
	f.Var(cmd.NewAppendStringsValue(&c.params.IncludeLabel), "include-label", "Specifies the logging labels to include.")
	f.Var(cmd.NewAppendStringsValue(&c.params.ExcludeLabel), "exclude-label", "Specifies the logging labels to exclude.")
	...
	f.Var(c.backLogFlag, "n", "Specifies the number of recent lines to show, and continue to append new ones.")
	f.Var(c.backLogFlag, "lines", "")

	c.limitFlag = newIntValue(&c.params.Limit)
	f.Var(c.limitFlag, "limit", "Specifies the number of recent logs to show, then exit.")

	f.BoolVar(&c.params.Replay, "replay", false, "Shows the entire log and continue to append new lines.")

	f.BoolVar(&c.noTail, "no-tail", false, "Shows existing log messages, then exit.")
	f.BoolVar(&c.tail, "tail", false, "Shows existing log messages and continue to append new ones.")
