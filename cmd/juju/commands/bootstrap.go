...
func (c *bootstrapCommand) SetFlags(f *gnuflag.FlagSet) {
	...
	f.StringVar(&c.BootstrapSeries, "bootstrap-series", "", "(DEPRECATED)Specifies the series of the bootstrap machine. Deprecated: Use `bootstrap-base`.")
	...
	f.BoolVar(&c.BuildAgent, "build-agent", false, "Specifies whether to build local version of agent binary before bootstrapping.")
	f.StringVar(&c.JujuDbSnapPath, "db-snap", "",
		"Specifies the path to a locally built `.snap` to use as the internal `juju-db` service.")
	f.StringVar(&c.JujuDbSnapAssertionsPath, "db-snap-asserts", "", "Specifies the path to a local `.assert` file. Requires `--db-snap`.")
	f.StringVar(&c.MetadataSource, "metadata-source", "", "Specifies the local path to use as agent and/or image metadata source.")
	f.StringVar(&c.Placement, "to", "", "Specifies the placement directive indicating an instance to bootstrap to.")
	f.BoolVar(&c.KeepBrokenEnvironment, "keep-broken", false,
		"Specifies whether to preserve the provisioned controller instance if bootstrap fails.")
	f.BoolVar(&c.AutoUpgrade, "auto-upgrade", false, "Specifies whether to upgrade to the latest patch release after bootstrap.")
	...
	f.BoolVar(&c.showClouds, "clouds", false,
		"Specifies whether to print the available clouds which can be used to bootstrap a Juju environment.")
	f.StringVar(&c.showRegionsForCloud, "regions", "", "Prints the available regions for the specified cloud.")
	f.BoolVar(&c.noSwitch, "no-switch", false, "Specifies whether to skip switch to the newly created controller.")
	f.BoolVar(&c.Force, "force", false, "Specifies whether to bypass checks such as supported series.")
	f.StringVar(&c.ControllerCharmPath, "controller-charm-path", "", "Specifies the path to a locally built controller charm.")
