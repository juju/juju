...
func (s *BootstrapSuite) run(c *gc.C, test bootstrapTest) testing.Restorer {
	...
	info, err := store.ReadInfo(expectedBootstrappedControllerName)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *BootstrapSuite) TestBootstrapSetsCurrentEnvironment(c *gc.C) {
	...
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(coretesting.Stderr(ctx), jc.Contains, "-> local.devenv")
	currentEnv, err := modelcmd.ReadCurrentModel()
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(currentEnv, gc.Equals, bootstrappedControllerName("devenv"))
}
...
func (s *BootstrapSuite) TestBootstrapPropagatesEnvErrors(c *gc.C) {
	...
	expectedBootstrappedControllerName := bootstrappedControllerName(envName)
	s.patchVersionAndSeries(c, "raring")
	...
	c.Assert(err, gc.ErrorMatches, `error reading controller "local.devenv" info:.*\n.*cannot unmarshal.*nonsense.*`)
}
...
func (s *BootstrapSuite) TestAutoSyncLocalSource(c *gc.C) {
	...
	info, err := store.ReadInfo(bootstrappedControllerName("devenv"))
	c.Assert(err, jc.ErrorIsNil)
	cfg, err := config.New(config.NoDefaults, info.BootstrapConfig())
	c.Assert(err, jc.ErrorIsNil)
...
Creating Juju controller "local.devenv" on dummy-cloud/region-1
