...
func (c *AddMachineCommand) Init(args []string) error {
	...
	if strings.HasPrefix(containerSpec, sshHostPrefix) {
		...
		if c.ContainerType, err = instance.ParseAllowedContainerType(containerSpec); err != nil {
			...
			c.ContainerType, err = instance.ParseAllowedContainerType(containerSpec[:sep])
		}
	}
	return err
}
