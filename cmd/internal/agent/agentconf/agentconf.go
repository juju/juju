...
func SetupAgentLogging(loggerContext corelogger.LoggerContext, config agent.Config) {
	...
	if loggingOverride := config.Value(agent.LoggingOverride); loggingOverride != "" {
		logger.Infof(context.Background(), "logging override set for this agent: %q", loggingOverride)
		loggerContext.ResetLoggerLevels()
		err := loggerContext.ConfigureLoggers(loggingOverride)
		if err != nil {
			logger.Errorf(context.Background(), "setting logging override %v", err)
		}
	} else if loggingConfig := config.LoggingConfig(); loggingConfig != "" {
		logger.Infof(context.Background(), "setting logging config to %q", loggingConfig)
		...
		if err != nil {
			logger.Errorf(context.Background(), "problem setting logging config %v", err)
		}
		mgo.ConfigureMgoLogging()
	}

	if flags := featureflag.String(); flags != "" {
		logger.Warningf(context.Background(), "developer feature flags enabled: %s", flags)
	}
}
