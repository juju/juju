...
func (s *initialisationSuite) TestDetectSeries(c *gc.C) {
	...
	defer installFakeSSH(c, linux.DetectionScript, response, 0)()
	_, series, err := linux.DetectSeriesAndHardwareCharacteristics("whatever")
...
func (s *initialisationSuite) TestDetectionError(c *gc.C) {
	...
	defer installFakeSSH(c, linux.DetectionScript, []string{scriptResponse, "oh noes"}, 33)()
	hc, _, err := linux.DetectSeriesAndHardwareCharacteristics("hostname")
	...
	defer installFakeSSH(c, linux.DetectionScript, []string{scriptResponse, "non-empty-stderr"}, 0)()
	hc, _, err = linux.DetectSeriesAndHardwareCharacteristics("hostname")
	c.Assert(err, jc.ErrorIsNil)
...
func (s *initialisationSuite) TestDetectHardwareCharacteristics(c *gc.C) {
	...
	for i, test := range tests {
		...
		defer installFakeSSH(c, linux.DetectionScript, scriptResponse, 0)()
		hc, _, err := linux.DetectSeriesAndHardwareCharacteristics("hostname")
		c.Assert(err, jc.ErrorIsNil)
...
func (s *initialisationSuite) TestCheckProvisioned(c *gc.C) {
	...
	provisioned, err := linux.CheckProvisioned("example.com")
	c.Assert(err, jc.ErrorIsNil)
	...
	_, err = linux.CheckProvisioned("example.com")
...
func (s *initialisationSuite) TestInitUbuntuUserNonExisting(c *gc.C) {
	...
	err := linux.InitUbuntuUser("testhost", "testuser", "", nil, nil)
	c.Assert(err, jc.ErrorIsNil)
}

func (s *initialisationSuite) TestInitUbuntuUserExisting(c *gc.C) {
	defer installFakeSSH(c, "", nil, 0)()
	linux.InitUbuntuUser("testhost", "testuser", "", nil, nil)
}
