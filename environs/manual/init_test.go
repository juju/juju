...
func (s *initialisationSuite) TestDetectSeries(c *gc.C) {
	...
	defer sshtesting.InstallFakeSSH(c, ssh.DetectionScript, response, 0)()
...
func (s *initialisationSuite) TestDetectionError(c *gc.C) {
	...
	defer sshtesting.InstallFakeSSH(c, ssh.DetectionScript, []string{scriptResponse, "oh noes"}, 33)()
	...
	defer sshtesting.InstallFakeSSH(c, ssh.DetectionScript, []string{scriptResponse, "non-empty-stderr"}, 0)()
	hc, _, err = DetectSeriesAndHardwareCharacteristics("hostname")
	c.Assert(err, gc.IsNil)
...
func (s *initialisationSuite) TestDetectHardwareCharacteristics(c *gc.C) {
	...
	for i, test := range tests {
		...
		defer sshtesting.InstallFakeSSH(c, ssh.DetectionScript, scriptResponse, 0)()
		hc, _, err := DetectSeriesAndHardwareCharacteristics("hostname")
		c.Assert(err, gc.IsNil)
...
func (s *initialisationSuite) TestCheckProvisioned(c *gc.C) {
	defer sshtesting.InstallFakeSSH(c, ssh.CheckProvisionedScript, "", 0)()
	provisioned, err := checkProvisioned("example.com")
	c.Assert(err, gc.IsNil)
	c.Assert(provisioned, jc.IsFalse)

	defer sshtesting.InstallFakeSSH(c, ssh.CheckProvisionedScript, "non-empty", 0)()
	provisioned, err = checkProvisioned("example.com")
	c.Assert(err, gc.IsNil)
	c.Assert(provisioned, jc.IsTrue)

	// stderr should not affect result.
	defer sshtesting.InstallFakeSSH(c, ssh.CheckProvisionedScript, []string{"", "non-empty-stderr"}, 0)()
	provisioned, err = checkProvisioned("example.com")
	c.Assert(err, gc.IsNil)
	c.Assert(provisioned, jc.IsFalse)
	...
	// if the script fails for whatever reason, then checkProvisioned
	// will return an error. stderr will be included in the error message.
	defer sshtesting.InstallFakeSSH(c, ssh.CheckProvisionedScript, []string{"non-empty-stdout", "non-empty-stderr"}, 255)()
