...
func buildJujus(dir string) error {
	...
	cmds := [][]string{
		...
		//       -ldflags "-s -w"
		{"go", "build", modArg, "-ldflags", "-extldflags \"-static\"", "-o", filepath.Join(dir, names.Jujud), "github.com/juju/juju/v2/cmd/jujud"},
		{"go", "build", modArg, "-ldflags", "-extldflags \"-static\"", "-o", filepath.Join(dir, names.Jujuc), "github.com/juju/juju/v2/cmd/jujuc"},
	}
