...
// JujudCompatShim is a small shell shim named "jujud" that forwards to
// "jujuagentd". It is included in agent binary tarballs so that init
// scripts generated by versions of Juju before 4.1 (which referenced the
// "jujud" binary) keep working after the agent binary was renamed to
// "jujuagentd". The content is stored in jujud-shim.sh to avoid
// duplication with the Makefile simplestreams target.
//
//go:embed jujud-shim.sh
var JujudCompatShim []byte
...
func bundleTools(
	...
	if err := os.WriteFile(filepath.Join(dir, names.JujuController), JujudCompatShim, 0755); err != nil {
		return semversion.Binary{}, semversion.Number{}, false, "", errors.Annotate(err, "writing jujud compatibility shim")
	}
