...
func FindExactTools(env environs.Environ, vers version.Number, series string, arch string) (_ *coretools.Tools, err error) {
	...
	logger.Debugf("looking for agents in stream %q", stream)
	availableTools, err := FindTools(env, vers.Major, vers.Minor, stream, filter)
	if err != nil {
		return nil, err
	}
	if len(availableTools) != 1 {
		return nil, fmt.Errorf("expected one agent, got %d agents", len(availableTools))
	}
...
func checkToolsSeries(toolsList coretools.List, series string) error {
	...
	if toolsSeries[0] != series {
		return fmt.Errorf("agent mismatch: expected series %v, got %v", series, toolsSeries[0])
	}
	return nil
}
