...
func (*bootstrapAddressSuite) TestInstanceAddresses(c *tc.C) {
	...
	got, err := finder.BootstrapControllerAddresses(c.Context(), "bootstrap")
	c.Assert(err, tc.ErrorIsNil)
...
func (*bootstrapAddressSuite) TestInstanceLookupError(c *tc.C) {
	...
	finder := instanceBootstrapAddressFinder{lister: stubInstanceLister{err: boom}}

	_, err := finder.BootstrapControllerAddresses(c.Context(), "bootstrap")
...
func (*bootstrapAddressSuite) TestUnsupportedEnviron(c *tc.C) {
	_, err := NewBootstrapAddressFinder(unsupportedBootstrapEnviron{})
