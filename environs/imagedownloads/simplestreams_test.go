...
func (Suite) TestOnePpc64elXenialImg(c *gc.C) {
	...
	ts := httptest.NewServer(&sstreamsHandler{})
	defer ts.Close()
	got, err := One(ss, "ppc64el", "16.04", "", "disk1.img", newTestDataSourceFunc(ts.URL))
	c.Check(err, jc.ErrorIsNil)
	c.Assert(got, jc.DeepEquals, &Metadata{
		Arch:    "ppc64el",
		Release: "16.04",
...
func (Suite) TestOneErrors(c *gc.C) {
	...
	table := []struct {
		...
		{"stream not found", "amd64", "22.04", "hourly", "disk1.img", `no results for "amd64", "jammy", "hourly", "disk1.img"`},
	}
	ts := httptest.NewServer(&sstreamsHandler{})
	defer ts.Close()
