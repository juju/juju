...
func (t *Tests) TestBootstrapWithoutAdminSecret(c *C) {
	...
	c.Assert(err, IsNil)
	err = environs.Bootstrap(env, false, []byte(coretesting.CACert+coretesting.CAKey))
...
func (t *Tests) TestBootstrap(c *C) {
	...
	e := t.Open(c)
	err := environs.Bootstrap(e, false, []byte(coretesting.CACert+coretesting.CAKey))
	c.Assert(err, IsNil)
	...
	e2 := t.Open(c)
	err = environs.Bootstrap(e2, false, []byte(coretesting.CACert+coretesting.CAKey))
	c.Assert(err, ErrorMatches, "environment is already bootstrapped")
	...
	err = environs.Bootstrap(e3, false, []byte(coretesting.CACert+coretesting.CAKey))
	c.Assert(err, IsNil)
