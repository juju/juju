...
func SyncTools(syncContext *SyncContext) error {
	...
	for _, tool := range sourceTools {
		logger.Debugf("found source agent: %v", tool)
	}
...
func selectSourceDatasource(syncContext *SyncContext) (simplestreams.DataSource, error) {
	...
	return simplestreams.NewURLSignedDataSource("sync agents source", sourceURL, keys.JujuPublicKey, utils.VerifySSLHostnames, simplestreams.CUSTOM_CLOUD_DATA, false), nil
}
...
func buildAgentTarball(build bool, forceVersion *version.Number, stream string) (_ *BuiltAgent, err error) {
	...
	fileInfo, err := f.Stat()
	if err != nil {
		return nil, errors.Errorf("cannot stat newly made agents archive: %v", err)
	}
	size := fileInfo.Size()
