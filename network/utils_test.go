...
; comment; ignored
 ; not a comment, but still ignored

;;;; whitespace before 'nameserver' causes it to be ignored
 nameserver       127.0.1.1
  # empty lines and comments ignored

nameserver 8.8.8.8 # not-a-comment-but-still-ignored--single-value-only-allowed

# no value below, so will be ignored
nameserver
search

# same for 'search' - must not begin with whitespace 
  search   example.org

nameserver 2001:db8::1
# the second value below is ignored
nameserver 8.8.4.4 127.0.100.100
search example.com example.org foo bar
# only the first valid 'search' is processed
search bad
