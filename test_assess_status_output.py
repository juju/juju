...
"""
SAMPLE_JSON_OUTPUT="""{"environment":"perritoec2","machines":{"0":{"agent-state":"started","agent-version":"1.25-alpha1","dns-name":"54.82.51.4","instance-id":"i-c0dadd10","instance-state":"running","series":"trusty","hardware":"arch=amd64 cpu-cores=1 cpu-power=100 mem=1740M root-disk=8192M availability-zone=us-east-1c","state-server-member-status":"has-vote"},"1":{"agent-state":"started","agent-version":"1.25-alpha1","dns-name":"54.162.95.230","instance-id":"i-a7a2b377","instance-state":"running","series":"trusty","hardware":"arch=amd64 cpu-cores=1 cpu-power=300 mem=3840M root-disk=8192M availability-zone=us-east-1c"}},"services":{"statusstresser":{"charm":"local:trusty/statusstresser-1","exposed":false,"service-status":{"current":"active","message":"called in config-changed hook","since":"15 Jun 2015 20:56:29-03:00"},"units":{"statusstresser/0":{"workload-status":{"current":"active","message":"called in config-changed hook","since":"15 Jun 2015 20:56:29-03:00"},"agent-status":{"current":"idle","since":"15 Jun 2015 20:56:41-03:00","version":"1.25-alpha1"},"agent-state":"started","agent-version":"1.25-alpha1","machine":"1","public-address":"54.162.95.230"}}}}}
"""
...
statusstresser/0 active         idle        1.25-alpha1 1             54.162.95.230  called in config-changed hook

[Machines]
ID         STATE   VERSION     DNS            INS-ID     SERIES HARDWARE
0          started 1.25-alpha1 54.82.51.4     i-c0dadd10 trusty arch=amd64 cpu-cores=1 cpu-power=100 mem=1740M root-disk=8192M availability-zone=us-east-1c
1          started 1.25-alpha1 54.162.95.230  i-39280ac6 trusty arch=amd64 cpu-cores=1 cpu-power=100 mem=1740M root-disk=8192M availability-zone=us-east-1d
"""

class ReducedTestStatus(object):
    ...
    def test_assert_machine_hardware(self):
        self.parser.assert_machine_hardware("0", "arch=amd64 cpu-cores=1 cpu-power=100 mem=1740M root-disk=8192M availability-zone=us-east-1c")
    def test_assert_machine_hardware_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_machine_hardware("0", "arch=arm cpu-cores=1 cpu-power=100 mem=1740M root-disk=8192M availability-zone=us-east-1c")
    def test_assert_machine_hardware_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_machine_hardware("3", "arch=amd64 cpu-cores=1 cpu-power=100 mem=1740M root-disk=8192M availability-zone=us-east-1c")

    def test_assert_service_charm(self):
        self.parser.assert_service_charm("statusstresser", "local:trusty/statusstresser-1")
    def test_assert_service_charm_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_service_charm("statusstresser", "local:trusty/statusstresser-2")
    def test_assert_service_charm_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_service_charm("statusrelaxer", "local:trusty/statusstresser-1")
    ...
    def test_assert_unit_public_address(self):
        self.parser.assert_unit_public_address("statusstresser/0", "54.162.95.230")
    def test_assert_unit_public_address_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_unit_public_address("statusstresser/0", "54.162.95.231")
    def test_assert_unit_public_address_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_unit_public_address("statusrelaxer/0", "54.162.95.230")
...
class BaseTestStatus(ReducedTestStatus):
    ...
    def test_assert_service_service_status(self):
        self.parser.assert_service_service_status("statusstresser", {"current":"active", "message":"called in config-changed hook"})
    def test_assert_service_service_status_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_service_service_status("statusstresser", {"current":"active", "message":"another message"})
    def test_assert_service_service_status_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_service_service_status("statusrelaxer", {"current":"active", "message":"called in config-changed hook"})


    def test_assert_unit_workload_status(self):
        self.parser.assert_unit_workload_status("statusstresser/0", {"current":"active", "message":"called in config-changed hook"})
    def test_assert_unit_workload_status_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_unit_workload_status("statusstresser/0", {"current":"active", "message":"another message"})
    def test_assert_unit_workload_status_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_unit_workload_status("statusrelaxer/0", {"current":"active", "message":"called in config-changed hook"})

    def test_assert_unit_agent_status(self):
        self.parser.assert_unit_agent_status("statusstresser/0", {"current":"idle", "message": ""})
    def test_assert_unit_agent_status_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_unit_agent_status("statusstresser/0", {"current":"idle", "message": "an unexpected message"})
    def test_assert_unit_agent_status_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_unit_agent_status("statusrelaxer/0", {"current":"idle", "message": ""})
    ...
    def test_assert_unit_agent_version(self):
        self.parser.assert_unit_agent_version("statusstresser/0", "1.25-alpha1")
    def test_assert_unit_agent_version_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_unit_agent_version("statusstresser/0", "1.25-alpha2")
    def test_assert_unit_agent_version_error(self):
        with self.assertRaises(AssertError):
            self.parser.assert_unit_agent_version("statusrelaxer/0", "1.25-alpha1")
...
class TestStatusTabular(TestCase, ReducedTestStatus):
    ...
    def test_assert_service_service_status(self):
        self.parser.assert_service_service_status("statusstresser", {"current":"active", "message":""})
    def test_assert_service_service_status_failed(self):
        with self.assertRaises(AssertFailed):
            self.parser.assert_service_service_status("statusstresser", {"current":"active", "message":"another message"})
    def test_assert_service_service_status_error(self):
