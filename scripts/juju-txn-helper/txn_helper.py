...
UUID_REGEX = re.compile(r"^[\dA-Fa-f]{8}-[\dA-Fa-f]{4}-[\dA-Fa-f]{4}-[\dA-Fa-f]{4}-[\dA-Fa-f]{12}$")
...
def parse_args():
    ...
    ap.add_argument(
        "--auth-database", default="admin", help="Mongo auth database to use, if required.  (Default: %(default)s)"
    )
    ap.add_argument("-c", "--count", type=int, help="Count of entries to examine")
    ap.add_argument(
        "-d", "--dump-transaction", action="store_true", help="Additionally pretty-print entire transactions to stdout"
    )
...
def walk_transaction_queue(client, model_uuid, dump_transaction, include_passes, max_transaction_count):
    ...
    for txn_index, txn in enumerate(txn_queue):
        ...
        for match_index, match in enumerate(matches):
            ...
            for op_index, op in enumerate(match["o"]):
                _print_op_details(db_client, op_index, op, include_passes)
            print()


def _print_op_details(db_client, op_index, op, include_passes):
