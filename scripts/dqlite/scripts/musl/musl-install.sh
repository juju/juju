...
musl_install_system() {
	sudo ./configure || {
		echo "Failed to configure musl"
		exit 1
	}
	sudo make install || {
		echo "Failed to install musl"
		exit 1
	}
	...
	mkdir -p ${LOCAL_PATH} || {
		echo "Failed to create ${MUSL_BIN_PATH}"
		exit 1
	}
	sudo ln -s ${MUSL_BIN_PATH}/musl-gcc ${LOCAL_PATH}/musl-gcc || {
		echo "Failed to link musl-gcc"
		exit 1
	}

	sudo ln -s /usr/include/${BUILD_MACHINE}-linux-gnu/asm ${MUSL_PATH}/include/asm || {
		echo "Failed to link ${BUILD_MACHINE}-linux-gnu/asm headers"
		exit 1
	}
	sudo ln -s /usr/include/asm-generic ${MUSL_PATH}/include/asm-generic || {
		echo "Failed to link asm-generic headers"
		exit 1
	}
	sudo ln -s /usr/include/linux ${MUSL_PATH}/include/linux || {
		echo "Failed to link linux header"
		exit 1
	}
}
...
musl_install_local() {
	...
	mkdir -p ${MUSL_BIN_PATH} || {
		echo "Failed to create ${MUSL_BIN_PATH}"
		exit 1
	}
	ln -s ${MUSL_PATH}/bin/musl-gcc ${MUSL_BIN_PATH}/musl-gcc || {
		echo "Failed to link musl-gcc"
		exit 1
	}

	cd ${PROJECT_DIR}
	ln -s /usr/include/${BUILD_MACHINE}-linux-gnu/asm ${MUSL_PATH}/include/asm || {
		echo "Failed to link ${BUILD_MACHINE}-linux-gnu/asm headers"
		exit 1
	}
	ln -s /usr/include/asm-generic ${MUSL_PATH}/include/asm-generic || {
		echo "Failed to link asm-generic headers"
		exit 1
	}
	ln -s /usr/include/linux ${MUSL_PATH}/include/linux || {
		echo "Failed to link linux header"
		exit 1
	}
}
...
post_musl_install_cross_darwin() {
	...
	case ${BUILD_ARCH} in
	amd64) ln -s "${BREW_BIN_PATH}/x86_64-linux-musl-gcc" ${MUSL_LOCAL_PATH}/output/bin/musl-gcc || {
		echo "Failed to link musl-gcc"
		exit 1
	} ;;
	arm64) ln -s "${BREW_BIN_PATH}/aarch64-linux-musl-gcc" ${MUSL_LOCAL_PATH}/output/bin/musl-gcc || {
		echo "Failed to link musl-gcc"
		exit 1
	} ;;
	*) {
...
musl_install_cross_darwin() {
	...
	brew install -q filosottile/musl-cross/musl-cross --with-aarch64 --with-x86_64 || {
		echo "Failed to install musl-cross"
		exit 1
	}
