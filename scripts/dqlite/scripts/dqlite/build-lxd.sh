...
lxc exec ${BUILD_CONTAINER} -- bash -c 'mkdir -p /root/scripts'
lxc file push $(dirname $0)/../env.sh ${BUILD_CONTAINER}/root/env.sh
lxc file push $(dirname $0)/build.sh ${BUILD_CONTAINER}/root/scripts/build.sh
lxc file push $(dirname $0)/dqlite-build.sh ${BUILD_CONTAINER}/root/scripts/dqlite-build.sh

lxc exec -t ${BUILD_CONTAINER} bash /root/scripts/build.sh
