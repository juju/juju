...
func (s *ApiServerSuite) NewFactory(c *gc.C, modelUUID string) (*factory.Factory, func() bool) {
	...
	modelDomainServices, err := s.DomainServicesGetter(c, s.NoopObjectStore(c), servicefactorytesting.TestingLeaseManager{}).
		ServicesForModel(context.Background(), coremodel.UUID(modelUUID))
	c.Assert(err, jc.ErrorIsNil)
