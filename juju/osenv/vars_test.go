...
func (s *varsSuite) TestJujuDataEnvVar(c *gc.C) {
	path := "/foo/bar/baz"
	s.PatchEnvironment(osenv.JujuDataEnvKey, path)
	c.Assert(osenv.JujuDataDir(), gc.Equals, path)
}

func (s *varsSuite) TestBlankJujuDataEnvVar(c *gc.C) {
	s.PatchEnvironment(osenv.JujuDataEnvKey, "")
	...
	c.Assert(osenv.JujuDataDir(), gc.Not(gc.Equals), "")

	if runtime.GOOS == "windows" {
		c.Assert(osenv.JujuDataDir(), gc.Equals, osenv.JujuDataWin())
	} else {
		c.Assert(osenv.JujuDataDir(), gc.Equals, osenv.JujuDataLinux())
	}
}
