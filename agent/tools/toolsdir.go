...
func UnpackTools(dataDir string, tools *coretools.Tools, r io.Reader) (err error) {
	...
	for {
		...
		if strings.ContainsAny(hdr.Name, "/\\") {
			return fmt.Errorf("bad name %q in agents archive", hdr.Name)
		}
		if hdr.Typeflag != tar.TypeReg {
			return fmt.Errorf("bad file type %c in file %q in agents archive", hdr.Typeflag, hdr.Name)
		}
...
func ReadTools(dataDir string, vers version.Binary) (*coretools.Tools, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cannot read agents metadata in tools directory: %v", err)
	}
	var tools coretools.Tools
	if err := json.Unmarshal(toolsData, &tools); err != nil {
		return nil, fmt.Errorf("invalid agents metadata in tools directory %q: %v", dir, err)
	}
	return &tools, nil
}
