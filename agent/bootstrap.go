...
type StateInitializer interface {
	InitializeState(envCfg *config.Config, machineCfg BootstrapMachineConfig, timeout mongo.DialOpts) (*state.State, *state.Machine, error)
}
...
func InitializeState(c ConfigSetter, envCfg *config.Config, machineCfg BootstrapMachineConfig, timeout mongo.DialOpts) (_ *state.State, _ *state.Machine, resultErr error) {
...
func initBootstrapMachine(c ConfigSetter, st *state.State, cfg BootstrapMachineConfig) (*state.Machine, error) {
	...
	m, err := st.EnvironmentDeployer.AddOneMachine(state.MachineTemplate{
