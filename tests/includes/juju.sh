...
bootstrap() {
	...
	*)
	...
	if [[ ${BOOTSTRAP_REUSE} == "true" ]]; then
		...
		BOOTSTRAPPED_CLOUD=$(juju show-model controller | yq -o=json -I=0 | jq -r '.[] | .cloud')
		export BOOTSTRAPPED_CLOUD
		BOOTSTRAPPED_CLOUD_REGION=$(juju show-model controller | yq -o=json -I=0 | jq -r '.[] | (.cloud + "/" + .region)')
...
remove_controller_offers() {
	...
	OUT=$(timeout 30 juju models -c "${name}" --format=json | jq -r '.["models"] | .[] | select(.["is-controller"] == false) | .name' || true)
	if [[ -n ${OUT} ]]; then
