...
add_client_ssh_key_to_juju_model() {
	local model_name="$1"
	local ssh_key_file="${HOME}/.local/share/juju/ssh/juju_id_ed25519.pub"
	...
	# Check if there is an error message when adding SSH key to model
	if [[ -n "$output" ]]; then
		echo "Error: Failed to add SSH key to model '${model_name}'."
		return
	fi
