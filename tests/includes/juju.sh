  ...
  "unmanaged")
	...
	*)
	...
	if [[ ${BOOTSTRAP_REUSE} == "true" && ${BOOTSTRAP_PROVIDER} != "k8s" ]]; then
		...
		OUT=$(juju show-machine -m "${bootstrapped_name}":controller --format=json | yq -r '.machines | .[] | .base | (.name + "@" + .channel)')
		if [[ -n ${OUT} ]]; then
...
post_bootstrap() {
	...
	add_client_ssh_key_to_juju_model "controller"
...
post_add_model() {
	...
	# Ensure that `juju ssh` works in tests for newly created models.
