...
run_deploy_unmanaged_aws() {
	...
	OUT=$(aws ec2 describe-vpcs | yq '.Vpcs[] | select(.Tags[]? | select((.Key=="Name") and (.Value=="unmanaged-deploy")))' || true)
	if [[ -z ${OUT} ]]; then
		vpc_id=$(create_vpc)
	...
	OUT=$(aws ec2 describe-security-groups | yq ".SecurityGroups[] | select(.VpcId==\"${vpc_id}\" and .GroupName==\"ci-unmanaged-deploy\")" || true)
	if [[ -z ${OUT} ]]; then
