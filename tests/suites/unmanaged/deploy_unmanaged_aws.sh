create_vpc() {
	set_verbosity
	vpc_id=$(aws ec2 create-vpc --cidr-block 10.0.0.0/28 --query 'Vpc.VpcId' --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=unmanaged-deploy}]' --output text)
...
create_secgroup() {
	set_verbosity
	sg_id=$(aws ec2 create-security-group --group-name "ci-unmanaged-deploy" --description "run_deploy_unmanaged_aws" --vpc-id "${vpc_id}" --query 'GroupId' --output text)
...
run_deploy_unmanaged_aws() {
	...
	OUT=$(aws ec2 describe-vpcs | jq '.Vpcs[] | select(.Tags[]? | select((.Key=="Name") and (.Value=="unmanaged-deploy")))' || true)
	if [[ -z ${OUT} ]]; then
		vpc_id=$(create_vpc)
	...
	OUT=$(aws ec2 describe-security-groups | jq ".SecurityGroups[] | select(.VpcId==\"${vpc_id}\" and .GroupName==\"ci-unmanaged-deploy\")" || true)
	if [[ -z ${OUT} ]]; then
