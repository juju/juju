test_spaces_unmanaged() {
	if [ "$(skip 'test_spaces_unmanaged')" ]; then
		echo "==> TEST SKIPPED: spaces unmanaged"
		return
	fi
	...
	(
		...
		"ec2")
			run "run_spaces_unmanaged_aws"
			;;
		*)
			echo "==> TEST SKIPPED: spaces unmanaged - tests AWS only"
			;;
		esac
	)
}

run_spaces_unmanaged_aws() {
	echo
	...
	add_clean_func "run_cleanup_deploy_unmanaged_aws"
	...
	OUT=$(aws ec2 describe-security-groups | jq '.SecurityGroups[] | select(.GroupName=="ci-spaces-unmanaged-ssh")' || true)
	if [[ -z ${OUT} ]]; then
		sg_id=$(aws ec2 create-security-group --group-name "ci-spaces-unmanaged-ssh" --description "SSH access for unmanaged spaces test" --query 'GroupId' --output text)
