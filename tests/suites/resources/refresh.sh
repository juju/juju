...
run_resource_refresh_no_new_charm_rev() {
	...
	juju resources juju-qa-test --format json | yq -S '.resources[0] | .[ "revision"] == "2"'
...
run_resource_refresh_no_new_charm_rev_supply_res_rev() {
	...
	juju config juju-qa-test foo-file=true
	wait_for "resource line one: testing one plus one." "$(workload_status juju-qa-test 0).message"
	juju resources juju-qa-test --format json | yq -S '.resources[0] | .[ "revision"] == "3"'
...
run_resource_no_upgrade_after_upload() {
	...
	juju resources juju-qa-test --format json | yq -S '.resources[0] | .[ "revision"] == "0"'
	juju resources juju-qa-test --format json | yq -S '.resources[0] | .[ "origin"] == "upload"'
