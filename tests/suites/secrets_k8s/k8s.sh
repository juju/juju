run_secrets() {
	...
	check_contains "$(juju_exec_output --unit world/0 -- secret-get "$app_owned_full_uri" 2>&1)" 'is not allowed to read this secret'

	echo "Checking: secret-revoke by app name"
	juju exec --unit hello/0 -- secret-revoke "$unit_owned_short_uri" --app world
	check_contains "$(juju_exec_output --unit world/0 -- secret-get "$unit_owned_short_uri" 2>&1)" 'is not allowed to read this secret'
...
run_user_secrets() {
	...
	check_contains "$(juju_exec_output --unit snappass-test/0 -- secret-get "$secret_uri" 2>&1)" 'is not allowed to read this secret'
