run_network_health() {
	...
	charm build ./tests/suites/network/charms/network-health --output-dir /tmp --force
	...
	juju deploy /tmp/network-health network-health-bionic --series bionic
	juju deploy /tmp/network-health network-health-focal --series focal
	juju deploy /tmp/network-health network-health-jammy --series jammy
	...
	wait_for "ubuntu-bionic" "$(idle_condition "ubuntu-bionic" 0)"
	wait_for "ubuntu-focal" "$(idle_condition "ubuntu-focal" 1)"
	wait_for "ubuntu-jammy" "$(idle_condition "ubuntu-jammy" 2)"
