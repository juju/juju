...
EOF
	...
	EXPECTED=$(echo "${CLOUDS}" | yq -o=json -I=0 | jq -S '.[] | del(.clouds) | .[] |= ({endpoint} as $endpoint | .[] |= walk(
	...
	CLOUD_LIST=$(JUJU_DATA="${TEST_DIR}/juju" juju show-cloud finfolk-vmaas --format yaml --client | yq -o=json -I=0 | jq -S 'with_entries((select(.value!= null)))')
	EXPECTED=$(
		cat <<'EOF' | jq -S
	{
    "auth-types": [
