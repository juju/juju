...
class TestMain(TestCase):
    ...
    def test_main(self):
        ...
        with patch("assess_NAMEHERE.configure_logging",
                   autospec=True) as mock_cl:
            with patch("assess_NAMEHERE.BootstrapManager.booted_context",
                       autospec=True) as mock_bc:
                with patch("jujupy.SimpleEnvironment.from_config",
                           return_value=env) as mock_e:
                    with patch("jujupy.EnvJujuClient.by_version",
                               return_value=client) as mock_c:
                        with patch("assess_NAMEHERE.assess_NAMEHERE",
...
class TestAssess(TestCase):
    ...
    def test_NAMEHERE(self):
        mock_client = Mock(spec=["juju", "wait_for_started"])
        assess_NAMEHERE(mock_client)
