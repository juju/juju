...
func (s *maas2StorageSuite) TestGetSuccess(c *gc.C) {
	...
	c.Assert(getController(c, storage).getFileFilename, gc.Equals, "prefix-grasshopper.avi")
}
...
func (s *maas2StorageSuite) TestListSuccess(c *gc.C) {
	...
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(result, gc.DeepEquals, []string{"frank", "julianna"})
	c.Assert(getController(c, storage).filesPrefix, gc.Equals, "prefix-grasshopper")
}
