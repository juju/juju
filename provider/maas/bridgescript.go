...
class LogicalInterface(object):
    ...
    def _routes_for_device(self, device):
        routes = []
        for r in check_shell_cmd('ip route show dev {}'.format(device)).splitlines():
    ...
    def _move_addresses_and_routes_to_bridge(self, args, addresses, prev_routes):
        ...
        curr_routes = self._routes_for_device(args['bridge'])

        for old_route in prev_routes:
            if old_route not in curr_routes:
                print_shell_cmd('ip route replace {} dev {bridge}'.format(old_route, **args))
