     ...
     <description>CPU</description>
      <node id="pci:2" claimed="true" class="bridge" handle="PCIBUS:0000:03">
        <node id="network" claimed="true" class="network" handle="PCI:0000:04:00.0">
         ...
         <serial>aa:bb:cc:dd:ee:f0</serial>
        </node>
      </node>
    </node>
  </node>
...
func (suite *environSuite) TestStartInstanceStartsInstance(c *gc.C) {
	...
	suite.testMAASObject.TestServer.AddNodeDetails("node0", lshwXMLNode0)
	...
	suite.testMAASObject.TestServer.AddNodeDetails("node1", lshwXMLNode1)
...
// vlanTag is either number (e.g. "1") or "null"
func (suite *environSuite) getNetwork(name string, id int, vlanTag string) *gomaasapi.MAASObject {
	var input string
	input = fmt.Sprintf(`{"name": %q, "ip":"192.168.%d.1", "netmask": "255.255.255.0",`+
		`"vlan_tag": %s, "description": "%s_%d_%s" }`, name, id, vlanTag, name, id, vlanTag)
...
func (suite *environSuite) TestBootstrapSucceeds(c *gc.C) {
	...
	suite.testMAASObject.TestServer.AddNodeDetails("thenode", lshwXMLNode0)
	err := bootstrap.Bootstrap(coretesting.Context(c), env, constraints.Value{})
	c.Assert(err, gc.IsNil)
}
...
func (suite *environSuite) TestGetNetworkMACs(c *gc.C) {
	...
	c.Check(networks, gc.DeepEquals, []string{"aa:bb:cc:dd:ee:11", "aa:bb:cc:dd:ee:21"})
	networks, err = env.getNetworkMACs("net_2")
	c.Assert(err, gc.IsNil)
	sort.Strings(networks)
	c.Check(networks, gc.DeepEquals, []string{"aa:bb:cc:dd:ee:12", "aa:bb:cc:dd:ee:22"})
...
func (suite *environSuite) TestGetInstanceNetworkInterfaces(c *gc.C) {
	inst := suite.getInstance("testInstance")
	suite.testMAASObject.TestServer.AddNodeDetails("testInstance", lshwXMLTestExtractInterfaces)
...
func (suite *environSuite) TestSetupNetworks(c *gc.C) {
	...
	c.Check(networkInfo, jc.DeepEquals, []environs.NetworkInfo{
...
func (suite *environSuite) TestSupportNetworks(c *gc.C) {
	env := suite.makeEnviron()
	// check for jc.IsTrue, because gomaasapi testing server now supports networks
