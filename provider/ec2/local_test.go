...
func (t *localServerSuite) TestAddresses(c *gc.C) {
	...
	expected := network.ProviderAddresses{
		network.NewProviderAddress("8.0.0.*", network.WithScope(network.ScopePublic)),
		network.NewProviderAddress("127.0.0.*", network.WithScope(network.ScopeCloudLocal)),
	}
...
func (t *localServerSuite) assertInterfaceLooksValid(c *gc.C, expIfaceID, expDevIndex int, iface network.InterfaceInfo) {
	...
	expectedInterface := network.InterfaceInfo{
		...
		Addresses: network.ProviderAddresses{network.NewProviderAddress(
			addr, network.WithScope(network.ScopeCloudLocal), network.WithCIDR(cidr)),
		},
