...
func (s *environPolSuite) TestPrecheckInstanceDefaults(c *gc.C) {
	err := s.Env.PrecheckInstance(s.CallCtx, environs.PrecheckInstanceParams{Series: version.SupportedLTS()})
...
func (s *environPolSuite) TestPrecheckInstanceAvailZone(c *gc.C) {
	...
	err := s.Env.PrecheckInstance(s.CallCtx, environs.PrecheckInstanceParams{Series: version.SupportedLTS(), Placement: placement})
