...
func (s *stateSuite) TestGetStorageAttachmentIDsForUnit(c *tc.C) {
	...
	storageInstanceUUID, _ := s.newStorageInstanceForCharmWithProviderType(c, charmUUID, "", "mystorage")
...
func (s *stateSuite) TestGetStorageIDsForUnit(c *tc.C) {
	...
	storageInstanceUUID, storageID := s.newStorageInstanceWithCharmUUID(c, charmUUID, "mystorage-1")
	storageInstanceUUID2, _ := s.newStorageInstanceWithCharmUUID(c, charmUUID, "mystorage-2")
	storageInstanceUUID3, _ := s.newStorageInstanceWithCharmUUID(c, charmUUID, "mystorage-3")
...
func (s *stateSuite) TestInitialWatchStatementForUnitStorageAttachments(c *tc.C) {
	...
	s.newCharmStorage(c, charmUUID, "mystorage-3", "filesystem", false, "")
	storageInstanceUUID, storageID := s.newStorageInstanceWithCharmUUID(c, charmUUID, "mystorage-1")
	_, _ = s.newStorageInstanceWithCharmUUID(c, charmUUID, "mystorage-2")
	_, _ = s.newStorageInstanceWithCharmUUID(c, charmUUID, "mystorage-3")
	s.newStorageAttachment(c, storageInstanceUUID, unitUUID, domainlife.Alive)
