...
// WatchAttachmentPlans returns a watcher that emits volume attachment plan UUIDs,
// whenever a volume attachment plan's life changes for the given machine.
func (s *Service) WatchAttachmentPlans(
	ctx context.Context, machineUUID machine.UUID,
) (watcher.StringsWatcher, error) {
	if err := machineUUID.Validate(); err != nil {
		return nil, errors.Capture(err)
