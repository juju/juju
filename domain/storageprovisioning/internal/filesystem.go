...
type FilesystemTemplate struct {
	...
	// CharmLocationHint is a path to hint where the filesystem(s) should be
	// mounted for the charm to access. It is not the exact path that
	// filesystem(s) will be mounted, but a suggestion from the charm.
	CharmLocationHint string
