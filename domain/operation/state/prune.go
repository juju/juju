...
func (st *State) PruneOperations(ctx context.Context, maxAge time.Duration, maxSizeMB int) error {
	...
	// Prune by size
	err = st.pruneOperationsToKeepSizeMiBUnder(ctx, maxSizeMB)
	if err != nil {
...
// pruneOperationsToKeepSizeMiBUnder prunes operations to ensure the total size of
...
func (st *State) pruneOperationsToKeepSizeMiBUnder(ctx context.Context, maxSizeMiB int) error {
	if maxSizeMiB <= 0 {
		// size shouldn't be negative, but zero size is valid. In any case, we ignore
		// the prune by size as done in 3.6
		st.logger.Infof(ctx, "Ignoring pruning by age ignored: zero or negative size (size(MB): %s)", maxSizeMiB)
		return nil
	}
