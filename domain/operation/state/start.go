...
func (s *State) createExecOperation(
	...
	// Tasks targetting units.
	// Before this, we get all units from the targetted applications, and add
	// them to the originally targetted units.
...
func (s *State) createUnitTask(ctx context.Context, tx *sqlair.TX, operationUUID string, taskUUID string, unitName unit.Name) operation.UnitTaskResult {
	taskID, err := sequencestate.NextValue(ctx, s, tx, operation.OperationSequenceNamespace)
	if err != nil {
...
func (s *State) createUnitTaskWithID(ctx context.Context, tx *sqlair.TX, taskID string, taskUUID string, operationUUID string, unitName unit.Name) operation.UnitTaskResult {
	now := time.Now().UTC()
...
func (s *State) getUnitsForApplication(ctx context.Context, tx *sqlair.TX, appName string) ([]unit.Name, error) {
...
`
	...
	units := make([]unit.Name, len(results))
	for i, result := range results {
		units[i] = unit.Name(result.Name)
	}
...
func (s *State) getUnitUUID(ctx context.Context, tx *sqlair.TX, unitName unit.Name) (string, error) {
