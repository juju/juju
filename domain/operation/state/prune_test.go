...
// TestPruneOperationsToKeepSizeUnderIgnoresNonPositive ensures no pruning occurs when size limit is zero or negative.
func (s *pruneBySizeSuite) TestPruneOperationsToKeepSizeUnderIgnoresNonPositive(c *tc.C) {
	// Arrange: add some operations to ensure there is data.
	op1 := s.addOperation(c)
	op2 := s.addOperation(c)

	// Act: call with zero and negative; both should be no-ops and return nil.
	err := s.state.pruneOperationsToKeepSizeMiBUnder(c.Context(), 0)
	c.Assert(err, tc.ErrorIsNil)
	err = s.state.pruneOperationsToKeepSizeMiBUnder(c.Context(), -5)
	c.Assert(err, tc.ErrorIsNil)
...
// TestPruneOperationsToKeepSizeUnderNoPruneWhenUnderLimit verifies no deletions when total size <= 1 MiB.
func (s *pruneBySizeSuite) TestPruneOperationsToKeepSizeUnderNoPruneWhenUnderLimit(c *tc.C) {
	// Arrange: 2 operations, minimal data so total size is very small (<< 1 MiB).
	op1 := s.addOperation(c)
	op2 := s.addOperation(c)

	// Act
	err := s.state.pruneOperationsToKeepSizeMiBUnder(c.Context(), 1)
	c.Assert(err, tc.ErrorIsNil)
...
// TestPruneOperationsToKeepSizeUnderPrunesExpected ensures pruning deletes the correct number of oldest/prioritized operations.
func (s *pruneBySizeSuite) TestPruneOperationsToKeepSizeUnderPrunesExpected(c *tc.C) {
	// Arrange:
