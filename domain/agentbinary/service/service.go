...
type AgentBinaryModelState interface {
	...
	// GetAgentBinarySHA256 retrieves the agent binary matching the given version
	// and returns its reader, size, and SHA256 checksum. If not found, an error
	// is returned.
...
type ModelStore interface {
	...
	//
	// The caller is responsible for closing the returned reader.
	GetAgentBinary(ctx context.Context, ver coreagentbinary.Version) (io.ReadCloser, int64, string, error)
...
func (s *AgentBinaryService) HasAgentBinary(ctx context.Context, ver coreagentbinary.Version) (bool, error) {
	binariesMetadata, err := s.ListAgentBinaries(ctx)
	if err != nil {
		return false, err
	}
	sha256Sum := ""
	for _, metadata := range binariesMetadata {
		...
		// agent binaries.
		if metadata.Version == ver.Number.String() && metadata.Arch == ver.Arch {
...
func (s *AgentBinaryService) GetAgentBinary(ctx context.Context, ver coreagentbinary.Version) (io.ReadCloser, int64, error) {
	reader, size, sha256sum, err := s.modelStore.GetAgentBinary(ctx, ver)

	if err != nil {
		return nil, 0, errors.Errorf("getting agent binary from model store: %w", err)
	}
	if reader == nil {
