...
type agentObjectQuerierSuite struct {
...
func (s *agentObjectQuerierSuite) TestGetAvailableForVersionInStream(c *tc.C) {
	...
	stExp.GetAllAgentStoreBinariesForStream(
	...
	svc := NewAgentObjectQuerierStore(s.state)
	agentBinaries, err := svc.GetAvailableForVersionInStream(
		c.Context(),
		tc.Must1(c, semversion.Parse, "4.0.0"),
...
func (s *agentObjectQuerierSuite) TestGetAvailablePatchVersionsInStream(c *tc.C) {
	defer s.setupMocks(c).Finish()
	...
	stExp.GetAllAgentStoreBinariesForStream(
		...
		},
		{
			...
			SHA256:       "sha256-1",
			Stream:       domainagentbinary.AgentStreamReleased,
			Version:      tc.Must1(c, semversion.Parse, "4.0.8"),
		},
	}, nil)

	svc := NewAgentObjectQuerierStore(s.state)
	agentBinaries, err := svc.GetAvailablePatchVersionsInStream(
		c.Context(),
		tc.Must1(c, semversion.Parse, "4.0.0"),
		domainagentbinary.AgentStreamReleased,
	)
