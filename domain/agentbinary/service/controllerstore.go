...
func (s *ControllerAgentBinaryStore) GetAgentBinary(
	...
	s.logger.Debugf(ctx, "retrieving agent binary from controller agent binary store for ver %q and stream %q", ver.String(), stream.String())
...
func (s *ControllerAgentBinaryStore) add(
	...
	s.logger.Debugf(
		ctx,
		"adding agent binary %q with arch %q to controller agent binary store",
		version.Number,
		version.Arch,
	)
