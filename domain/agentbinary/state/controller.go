...
// It returns false if no matching record exists.
func (s *ControllerState) GetAgentBinarySHA256(ctx context.Context, version coreagentbinary.Version, stream modelagent.AgentStream) (bool, string, error) {
	db, err := s.DB(ctx)
	if err != nil {
		return false, "", errors.Capture(err)
	}
...
SELECT &metadataRecord.*
FROM   v_agent_binary_store
WHERE version = $metadataRecord.version`, metadataRecord{})
	if err != nil {
		return false, "", errors.Capture(err)
	}
	...
	exists := false
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		err := tx.Query(ctx, stmt, record).Get(&record)
		if errors.Is(err, sqlair.ErrNoRows) {
			return nil
		} else if err != nil {
			return errors.Errorf(
