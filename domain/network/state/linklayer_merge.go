...
// Updates can be ignored if the provider subnet id doesn't exist or if
...
func (st *State) updateSubnetFromProviderID(ctx context.Context, tx *sqlair.TX, address mergeAddress) error {
	...
	if err != nil {
		st.logger.Warningf(ctx, "ignoring subnet update for address %q (%s)",
			address.Value, address.UUID)
		return nil
	}
...
func (st *State) validateSubnetUpdate(ctx context.Context, tx *sqlair.TX, address mergeAddress) (string, error) {
	...
	if err != nil {
		return "", errors.Errorf("failed to get subnet for provider id %q: %w", address.ProviderSubnetID, err)
	}
	cidr, err := candidateSubnet.ParsedCIDRNetwork()
	if err != nil {
		return "", errors.Errorf("failed to parse subnet cidr %q: %w", candidateSubnet.CIDR, err)
	}
