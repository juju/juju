...
func (st *State) ImportLinkLayerDevices(ctx context.Context, input []internal.ImportLinkLayerDevice) error {
	...
	return db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		addressTypeMap, err := typeNameToIDMap[network.AddressType](ctx, st, tx, "ip_address_type")
		if err != nil {
			return errors.Capture(err)
		}
		configTypeMap, err := typeNameToIDMap[network.AddressConfigType](ctx, st, tx, "ip_address_config_type")
		if err != nil {
			return errors.Capture(err)
		}
		originTypeMap, err := typeNameToIDMap[network.Origin](ctx, st, tx, "ip_address_origin")
		if err != nil {
			return errors.Capture(err)
		}
		scopeTypeMap, err := typeNameToIDMap[network.Scope](ctx, st, tx, "ip_address_scope")
		if err != nil {
			return errors.Capture(err)
		}
		if err != nil {
		...
		addresses, providerAddresses, err := transformAddressesFromImportData(input,
			addressTypeMap, configTypeMap, originTypeMap, scopeTypeMap)
		if err != nil {
			return errors.Capture(err)
		}
