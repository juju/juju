...
func (s *controllerNodeSuite) TestDeleteDqliteNode(c *tc.C) {
	...
	_, err = db.Exec("INSERT INTO api_address_by_controller (controller_id, address, is_agent_only, scope) VALUES ('99', '10.0.0.1:17070', false, 'local-cloud')")
	c.Assert(err, tc.ErrorIsNil)
	...
	for _, q := range []struct {
		...
		{"SELECT COUNT(*) FROM api_address_by_controller WHERE controller_id = ?", []any{"99"}},
...
func (s *controllerNodeSuite) TestDeleteDqliteNodeIdempotent(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	_, err = db.Exec("INSERT INTO api_address_by_controller (controller_id, address, is_agent_only, scope) VALUES ('98', '10.0.0.1:17070', false, 'local-cloud')")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *controllerNodeSuite) TestDeleteDqliteNodePreservesOtherNodes(c *tc.C) {
	...
	for _, cID := range []string{"96", "97"} {
		...
		c.Assert(err, tc.ErrorIsNil)
		_, err = db.Exec("INSERT INTO api_address_by_controller (controller_id, address, is_agent_only, scope) VALUES (?, '10.0.0.' || ? || ':17070', false, 'local-cloud')", cID, cID)
		c.Assert(err, tc.ErrorIsNil)
	}
	...
	err = db.QueryRow("SELECT COUNT(*) FROM api_address_by_controller WHERE controller_id = '96'").Scan(&count)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(count, tc.Equals, 0)

	err = db.QueryRow("SELECT COUNT(*) FROM api_address_by_controller WHERE controller_id = '97'").Scan(&count)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(count, tc.Equals, 1)
}
