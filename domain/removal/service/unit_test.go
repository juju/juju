...
func (s *unitSuite) TestExecuteJobForUnitScheduleCharmRemovalFails(c *tc.C) {
	defer s.setupMocks(c).Finish()
	...
	// unit removal job still completes.
	exp.CharmScheduleRemoval(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
