...
// node.
func (st *State) GetAPIAddresses(ctx context.Context, ctrlID string) ([]controllernode.APIAddress, error) {
	db, err := st.DB()
	if err != nil {
...
func decodeAPIAddresses(addrs []controllerAPIAddress) []controllernode.APIAddress {
	result := make([]controllernode.APIAddress, 0, len(addrs))
	for _, addr := range addrs {
		result = append(result, controllernode.APIAddress{
			Address: addr.Address,
			IsAgent: addr.IsAgent,
		})
	}
