...
// agents.
func (st *State) GetAllAPIAddressesWithScopeForAgents(ctx context.Context) (controllernode.APIAddresses, error) {
	db, err := st.DB()
	if err != nil {
		return nil, errors.Capture(err)
	}
	...
	addrs := make(controllernode.APIAddresses, len(result))
	for i, res := range result {
		addrs[i] = controllernode.APIAddress{
			Address: res.Address,
			IsAgent: res.IsAgent,
			Scope:   network.Scope(res.Scope),
		}
	}
	return addrs, nil
}
