...
func (s *serviceSuite) TestGetAllAPIAddressesWithScopeForAgents(c *tc.C) {
	defer s.setupMocks(c).Finish()
	svc := NewService(s.state, loggertesting.WrapCheckLog(c))
	...
	// Act
	apiAddrs, err := svc.GetAllAPIAddressesWithScopeForAgents(c.Context())

	// Assert
	c.Assert(err, tc.ErrorIsNil)
	c.Check(apiAddrs, tc.DeepEquals, args)
}

func (s *serviceSuite) TestGetAllAPIAddressesWithScopeForAgentsError(c *tc.C) {
	defer s.setupMocks(c).Finish()
	svc := NewService(s.state, loggertesting.WrapCheckLog(c))

	s.state.EXPECT().GetAllAPIAddressesWithScopeForAgents(gomock.Any()).Return(nil, internalerrors.Errorf("boom"))

	_, err := svc.GetAllAPIAddressesWithScopeForAgents(c.Context())
	c.Assert(err, tc.ErrorMatches, "boom")
}
