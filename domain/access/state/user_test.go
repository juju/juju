...
// TestGetUsernamesForIds is testing the happy path of mapping to user id's to
// their corresponding [user.Name] type.
func (s *userStateSuite) TestGetUsernamesForIds(c *gc.C) {
	st := NewUserState(s.TxnRunnerFactory())

	uuid1 := usertesting.GenUserUUID(c)
	uuid2 := usertesting.GenUserUUID(c)

	name1 := usertesting.GenNewName(c, "tlm")
	name2 := usertesting.GenNewName(c, "wallyworld")
	...
	userIdToName, err := st.GetUsernamesForIds(context.Background(), []user.UUID{
		uuid1,
		uuid2,
	})
	c.Check(err, jc.ErrorIsNil)
	c.Check(userIdToName, jc.DeepEquals, map[user.UUID]user.Name{
		uuid1: name1,
		uuid2: name2,
	})
}

// TestGetUsernamesForIdsNotFound is asserting that if we ask for the mapping
// of a user id that doesn't exist in the system we get back a
// [errors.UserNotFound] error.
func (s *userStateSuite) TestGetUsernamesForIdsNotFound(c *gc.C) {
	st := NewUserState(s.TxnRunnerFactory())
	...
	_, err = st.GetUsernamesForIds(context.Background(), []user.UUID{
		uuid1,
		uuid2,
	})
	c.Check(err, jc.ErrorIs, errors.UserNotFound)
}

// TestGetUsernamesForIdsZero is asserting that if we ask for the mapping of
// zero ids we get back an empty map with no errors.
func (s *userStateSuite) TestGetUsernamesForIdsZero(c *gc.C) {
	st := NewUserState(s.TxnRunnerFactory())

	m, err := st.GetUsernamesForIds(context.Background(), []user.UUID{})
	c.Check(err, jc.ErrorIsNil)
	c.Check(len(m), gc.Equals, 0)
}
