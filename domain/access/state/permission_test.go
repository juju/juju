...
func (s *permissionStateSuite) SetUpTest(c *gc.C) {
	...
	s.modelUUID = modeltesting.CreateTestIAASModel(c, s.TxnRunnerFactory(), "test-model")
	s.defaultModelUUID = modeltesting.CreateTestIAASModel(c, s.TxnRunnerFactory(), "default-model")
...
func (s *permissionStateSuite) TestModelAccessForCloudCredential(c *gc.C) {
	...
	modeltesting.CreateTestIAASModel(c, s.TxnRunnerFactory(), "model-access")
	key := credential.Key{
		Cloud: "model-access",
		Owner: "model-access",
		Name:  "foobar",
