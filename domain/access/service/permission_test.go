...
func (s *serviceSuite) TestCreatePermission(c *tc.C) {
	...
	spec := corepermission.UserAccessSpec{
		User: coreuser.GenName(c, "testme"),
...
func (s *serviceSuite) TestDeletePermission(c *tc.C) {
	defer s.setupMocks(c).Finish()
	s.state.EXPECT().DeletePermission(gomock.Any(), coreuser.GenName(c, "testme"), gomock.AssignableToTypeOf(corepermission.ID{})).Return(nil)
	err := NewService(s.state).DeletePermission(c.Context(), coreuser.GenName(c, "testme"), corepermission.ID{
		ObjectType: corepermission.Cloud,
		Key:        "aws",
	})
	c.Assert(err, tc.ErrorIsNil)
...
func (s *serviceSuite) TestReadUserAccessForTarget(c *tc.C) {
	defer s.setupMocks(c).Finish()
	s.state.EXPECT().ReadUserAccessForTarget(gomock.Any(), coreuser.GenName(c, "testme"), gomock.AssignableToTypeOf(corepermission.ID{})).Return(corepermission.UserAccess{}, nil)

	_, err := NewService(s.state).ReadUserAccessForTarget(
		c.Context(),
		coreuser.GenName(c, "testme"),
		corepermission.ID{
			ObjectType: corepermission.Cloud,
			Key:        "aws",
		})
...
func (s *serviceSuite) TestReadUserAccessLevelForTarget(c *tc.C) {
	defer s.setupMocks(c).Finish()
	s.state.EXPECT().ReadUserAccessLevelForTarget(gomock.Any(), coreuser.GenName(c, "testme"), gomock.AssignableToTypeOf(corepermission.ID{})).Return(corepermission.NoAccess, nil)
...
func (s *serviceSuite) TestReadAllUserAccessForUser(c *tc.C) {
	defer s.setupMocks(c).Finish()
	s.state.EXPECT().ReadAllUserAccessForUser(gomock.Any(), coreuser.GenName(c, "testme")).Return(nil, nil)
...
func (s *serviceSuite) TestReadAllAccessForUserAndObjectType(c *tc.C) {
	defer s.setupMocks(c).Finish()
	s.state.EXPECT().ReadAllAccessForUserAndObjectType(gomock.Any(), coreuser.GenName(c, "testme"), corepermission.Cloud).Return(nil, nil)
