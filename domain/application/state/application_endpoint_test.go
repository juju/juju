...
func (s *applicationEndpointStateSuite) TestGetAllEndpointBindingsFreshModel(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(bindings, tc.DeepEquals, map[string]map[string]network.SpaceName{
		"foo": {
...
func (s *applicationEndpointStateSuite) TestGetAllEndpointBindingsDefault(c *tc.C) {
	...
	// Assert:
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(allEndpointBindings, tc.DeepEquals, map[string]map[string]network.SpaceName{
		"foo": {
			"": network.AlphaSpaceName,
		},
		"fii": {
			"":             network.AlphaSpaceName,
			app1rel1Name:   network.AlphaSpaceName,
			app1rel2Name:   network.AlphaSpaceName,
			app1rel3Name:   network.AlphaSpaceName,
			app1Extra1Name: network.AlphaSpaceName,
		},
		"bar": {
			"":             network.AlphaSpaceName,
			app2rel1Name:   network.AlphaSpaceName,
			app2rel2Name:   network.AlphaSpaceName,
			app2rel3Name:   network.AlphaSpaceName,
			app2Extra1Name: network.AlphaSpaceName,
		},
	})
}
...
func (s *applicationEndpointStateSuite) TestGetApplicationsBoundToSpaceDefaultBindings(c *tc.C) {
	...
	alphaApps, err := s.state.GetApplicationsBoundToSpace(c.Context(), network.AlphaSpaceId)
	c.Assert(err, tc.ErrorIsNil)
