...
// UpsertCAASUnit creates or updates the specified application unit, returning an error
...
func (st *ApplicationState) UpsertCAASUnit(ctx context.Context, name string, updateFunc application.StateTxOperationFunc) error {
	db, err := st.DB()
	if err != nil {
		return errors.Trace(err)
	}
