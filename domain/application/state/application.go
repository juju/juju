...
func (st *State) CreateApplication(
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if err := st.refreshConfigHash(ctx, tx, applicationID{ID: appUUID}); err != nil {
...
ON CONFLICT(application_uuid) DO UPDATE SET
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if err := st.refreshConfigHash(ctx, tx, ident); err != nil {
...
func (st *State) refreshConfigHash(ctx context.Context, tx *sqlair.TX, appID applicationID) error {
	setHashQuery := `
