...
func (s *applicationStateSuite) TestApplicationScaleStateNotFound(c *gc.C) {
	err := s.TxnRunner().Txn(context.Background(), func(ctx context.Context, tx *sqlair.TX) error {
		...
		_, err := stateOps.ApplicationScaleState(ctx, coreapplication.ID(uuid.MustNewUUID().String()))
		return err
	})
...
func (s *applicationStateSuite) TestUpdateCAASApplicationUnit(c *gc.C) {
	...
	err := s.state.UpsertCAASUnit(context.Background(), "foo", func(ctx context.Context, ops application.StateTxOperations, id coreapplication.ID) error {
...
func (s *applicationStateSuite) TestUpdateCAASApplicationUnitDead(c *gc.C) {
