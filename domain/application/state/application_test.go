...
func (s *applicationStateSuite) TestShouldAllowCharmUpgradeOnError(c *gc.C) {
	appUUID := s.createApplication(c, "foo", life.Alive)
...
func (s *applicationStateSuite) TestShouldAllowCharmUpgradeOnErrorFalse(c *gc.C) {
	appUUID := s.createApplication(c, "foo", life.Alive)
	s.setCharmUpgradeOnError(c, appUUID, false)
	v, err := s.state.ShouldAllowCharmUpgradeOnError(context.Background(), "foo")
	c.Assert(err, jc.ErrorIsNil)
	c.Check(v, jc.IsFalse)
}

func (s *applicationStateSuite) TestShouldAllowCharmUpgradeOnErrorNotFound(c *gc.C) {
	_, err := s.state.ShouldAllowCharmUpgradeOnError(context.Background(), "foo")
	c.Assert(err, jc.ErrorIs, applicationerrors.ApplicationNotFound)
}

func (s *applicationStateSuite) setCharmUpgradeOnError(c *gc.C, appUUID coreapplication.ID, v bool) {
	err := s.TxnRunner().StdTxn(context.Background(), func(ctx context.Context, tx *sql.Tx) error {
		_, err := tx.Exec(`
UPDATE application
