...
// belong to.
func (st *State) GetAllEndpointBindings(ctx context.Context) (map[string]map[string]network.SpaceName, error) {
	db, err := st.DB()
	if err != nil {
...
`, applicationEndpointBinding{})
	...
	spaceUUIDToName := make(map[network.SpaceUUID]network.SpaceName)
	for _, space := range allSpaces {
		spaceUUIDToName[space.UUID] = space.Name
	}

	result := make(map[string]map[string]network.SpaceName)
	for _, space := range defaultSpaces {
		if _, ok := result[space.ApplicationName]; !ok {
			result[space.ApplicationName] = make(map[string]network.SpaceName)
		}
...
// the given space.
func (st *State) GetApplicationsBoundToSpace(ctx context.Context, uuid network.SpaceUUID) ([]string, error) {
	db, err := st.DB()
	if err != nil {
		return nil, errors.Capture(err)
	}
...
func (st *State) insertApplicationRelationEndpointBindings(
	...
	spaceNamesToUUID map[network.SpaceName]network.SpaceUUID,
	bindings map[string]network.SpaceName,
) error {
	if len(relations) == 0 {
		return nil
	...
	for i, relation := range relations {
		...
		space := sql.Null[network.SpaceUUID]{}
...
func (st *State) checkSpaceNames(ctx context.Context, tx *sqlair.TX, inputs []network.SpaceName) (map[network.SpaceName]network.SpaceUUID, error) {
...
FROM space`, space{})
	...
	namesToUUID := make(map[network.SpaceName]network.SpaceUUID, len(spaces))
