...
func (s *charmServiceSuite) TestGetCharmID(c *tc.C) {
	...
	result, err := s.service.getCharmID(c.Context(), charm.GetCharmArgs{
		Name:     "foo",
		Revision: &rev,
...
func (s *charmServiceSuite) TestIsControllerCharm(c *tc.C) {
	...
	result, err := s.service.IsControllerCharm(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestIsControllerCharmCharmNotFound(c *tc.C) {
	...
	_, err := s.service.IsControllerCharm(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestIsCharmAvailable(c *tc.C) {
	...
	result, err := s.service.IsCharmAvailable(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(result, tc.IsTrue)
}
...
func (s *charmServiceSuite) TestIsCharmAvailableCharmNotFound(c *tc.C) {
	...
	_, err := s.service.IsCharmAvailable(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestSupportsContainers(c *tc.C) {
	...
	result, err := s.service.SupportsContainers(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(result, tc.IsTrue)
}
...
func (s *charmServiceSuite) TestSupportsContainersCharmNotFound(c *tc.C) {
	...
	_, err := s.service.SupportsContainers(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestGetCharmCharmNotFound(c *tc.C) {
	...
	_, _, _, err := s.service.GetCharm(c.Context(), charm.CharmLocator{
		Name:     "foo",
		Revision: 42,
		Source:   charm.LocalSource,
	})
...
func (s *charmServiceSuite) TestGetCharmMetadata(c *tc.C) {
	...
	metadata, err := s.service.GetCharmMetadata(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmMetadataName(c *tc.C) {
	...
	name, err := s.service.GetCharmMetadataName(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmMetadataNameCharmNotFound(c *tc.C) {
	...
	_, err := s.service.GetCharmMetadataName(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestGetCharmMetadataDescription(c *tc.C) {
	...
	description, err := s.service.GetCharmMetadataDescription(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmMetadataDescriptionCharmNotFound(c *tc.C) {
	...
	_, err := s.service.GetCharmMetadataDescription(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestGetCharmMetadataStorage(c *tc.C) {
	...
	storage, err := s.service.GetCharmMetadataStorage(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmMetadataStorageCharmNotFound(c *tc.C) {
	...
	_, err := s.service.GetCharmMetadataStorage(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestGetCharmMetadataResources(c *tc.C) {
	...
	resources, err := s.service.GetCharmMetadataResources(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmMetadataResourcesCharmNotFound(c *tc.C) {
	...
	_, err := s.service.GetCharmMetadataResources(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestCharmManifest(c *tc.C) {
	...
	manifest, err := s.service.GetCharmManifest(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestCharmManifestInvalid(c *tc.C) {
	...
	_, err := s.service.GetCharmManifest(c.Context(), locator)
...
func (s *charmServiceSuite) TestGetCharmActions(c *tc.C) {
	...
	actions, err := s.service.GetCharmActions(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmArchivePath(c *tc.C) {
	...
	path, err := s.service.GetCharmArchivePath(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmArchivePathCharmNotFound(c *tc.C) {
	...
	_, err := s.service.GetCharmArchivePath(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestGetCharmArchive(c *tc.C) {
	...
	reader, hash, err := s.service.GetCharmArchive(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetCharmArchiveCharmNotFound(c *tc.C) {
	...
	_, _, err := s.service.GetCharmArchive(c.Context(), locator)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNotFound)
}
...
func (s *charmServiceSuite) TestSetCharmAvailable(c *tc.C) {
	...
	err := s.service.SetCharmAvailable(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
}
...
func (s *charmServiceSuite) TestAddCharm(c *tc.C) {
	defer s.setupMocks(c).Finish()
	...
	s.state.EXPECT().AddCharm(gomock.Any(), charm.Charm{
		Metadata: charm.Metadata{
			Name:  "foo",
			RunAs: "default",
		},
...
func (s *charmServiceSuite) TestAddCharmCharmhub(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmNoName(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmInvalidSource(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationNameConflict(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationUnknownRole(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationRoleMismatch(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationToReservedNameJuju(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationToReservedNameJujuBlah(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationNameToReservedNameJuju(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationNameToReservedNameJujuBlah(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRequireRelationToReservedNameSucceeds(c *tc.C) {
	defer s.setupMocks(c).Finish()
	...
	s.state.EXPECT().AddCharm(gomock.Any(), gomock.Any(), nil, false).Return(id, charm.CharmLocator{
		Name:         "foo",
		Revision:     1,
		Source:       charm.LocalSource,
		Architecture: architecture.AMD64,
...
func (s *charmServiceSuite) TestAddCharmRequireRelationNameToReservedName(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationToReservedNameWithSpecialCharm(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationToReservedNameOnRequiresValid(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestAddCharmRelationToReservedNameOnRequiresInvalid(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestListCharmLocatorsWithName(c *tc.C) {
	...
	results, err := s.service.ListCharmLocators(c.Context(), "foo")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestListCharmLocatorsWithoutName(c *tc.C) {
	...
	results, err := s.service.ListCharmLocators(c.Context())
	c.Assert(err, tc.ErrorIsNil)
	c.Check(results, tc.HasLen, 1)
	c.Check(results, tc.DeepEquals, expected)
}
...
func (s *charmServiceSuite) TestGetCharmDownloadInfo(c *tc.C) {
	...
	result, err := s.service.GetCharmDownloadInfo(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetAvailableCharmArchiveSHA256(c *tc.C) {
	...
	result, err := s.service.GetAvailableCharmArchiveSHA256(c.Context(), locator)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestResolveUploadCharmLocalCharmNotImporting(c *tc.C) {
	...
	// AddCharm is tested in tests elsewhere, so we can just return a valid
	// charm ID here.
	...
	locator, err := s.service.ResolveUploadCharm(c.Context(), charm.ResolveUploadCharm{
		Source:       corecharm.Local,
		Reader:       file,
		SHA256Prefix: "abc",
		Architecture: arch.AMD64,
...
func (s *charmServiceSuite) TestResolveUploadCharmLocalCharmNotImportingFailedAddCharm(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *charmServiceSuite) TestReserveCharmRevision(c *tc.C) {
	...
	s.state.EXPECT().AddCharm(gomock.Any(), ch, downloadInfo, false).Return(corecharm.ID("id"), charm.CharmLocator{}, nil)

	_, _, err = s.service.ReserveCharmRevision(c.Context(), charm.ReserveCharmRevisionArgs{
		Charm:         charmBase,
		Source:        corecharm.CharmHub,
		ReferenceName: "foo",
		Revision:      1,
...
func (s *charmServiceSuite) TestReserveCharmRevisionAlreadyExists(c *tc.C) {
	...
	id, _, err := s.service.ReserveCharmRevision(c.Context(), charm.ReserveCharmRevisionArgs{
		// We only define required fields to pass code before AddCharm call
		Charm:         charmBase,
		Source:        corecharm.Local,
		ReferenceName: "foo",
		Revision:      42,
...
func (s *charmServiceSuite) TestGetLatestPendingCharmhubCharm(c *tc.C) {
	...
	result, err := s.service.GetLatestPendingCharmhubCharm(c.Context(), "foo", architecture.AMD64)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *charmServiceSuite) TestGetLatestPendingCharmhubCharmInvalidName(c *tc.C) {
	...
	_, err := s.service.GetLatestPendingCharmhubCharm(c.Context(), "!!!foo", architecture.AMD64)
	c.Assert(err, tc.ErrorIs, applicationerrors.CharmNameNotValid)
}
...
func (s *watchableServiceSuite) TestWatchCharms(c *tc.C) {
	...
	s.watcherFactory.EXPECT().NewUUIDsWatcher(c.Context(), "charm", "charms watcher", changestream.All).Return(stringsWatcher, nil)
