...
func (s *applicationServiceSuite) TestGetAllEndpointBindings(c *tc.C) {
	...
	s.state.EXPECT().GetAllEndpointBindings(gomock.Any()).Return(map[string]map[string]network.SpaceName{
		"foo": {"bar": "baz"},
		"bar": {"baz": "qux"},
	}, nil)
...
func (s *applicationServiceSuite) TestGetApplicationsBoundToSpace(c *tc.C) {
	...
	s.state.EXPECT().GetApplicationsBoundToSpace(gomock.Any(), spaceUUID).Return([]string{"foo", "bar"}, nil)
...
func (s *applicationServiceSuite) TestGetApplicationsBoundToSpaceErrors(c *tc.C) {
	...
	spaceUUID := networktesting.GenSpaceUUID(c)
	s.state.EXPECT().GetApplicationsBoundToSpace(gomock.Any(), spaceUUID).Return(nil, errors.Errorf("boom"))
