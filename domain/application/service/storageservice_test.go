...
// storageServiceSuite is a suite of tests for asserting the functionality on
// offer by the [storageService].
...
// [storageService] as a happy path tests. This is a large test that asserts a
...
func (s *storageServiceSuite) TestMakeUnitStorageArgs(c *tc.C) {
	ctrl := s.setupMocks(c)
	defer ctrl.Finish()
