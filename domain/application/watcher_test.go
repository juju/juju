...
func (s *watcherSuite) TestWatchApplicationsWithPendingCharms(c *gc.C) {
	...
	harness.AddTest(func(c *gc.C) {
		id0 = s.createApplication(c, &svc.Service, "foo")
		id1 = s.createApplication(c, &svc.Service, "bar")
	}, func(w watchertest.WatcherC[[]string]) {
		w.Check(
...
WHERE uuid=?`, id0.String())
	...
	harness.AddTest(func(c *gc.C) {
		id2 = s.createApplication(c, &svc.Service, "baz")
	}, func(w watchertest.WatcherC[[]string]) {
		w.Check(
	...
	harness.AddTest(func(c *gc.C) {
		id2 = s.createApplicationWithCharmAndStoragePath(c, &svc.Service, "jaz", &stubCharm{}, "deadbeef", service.AddUnitArg{})
	}, func(w watchertest.WatcherC[[]string]) {
		w.AssertNoChange()
	})
...
func (s *watcherSuite) createApplicationWithCharmAndStoragePath(c *gc.C, svc *service.Service, name string, charm internalcharm.Charm, storagePath string, units ...service.AddUnitArg) coreapplication.ID {
	ctx := context.Background()
