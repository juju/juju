...
func (s *migrationSuite) TestImportOffers(c *tc.C) {
	...
	s.modelDBState.EXPECT().ImportOffers(gomock.Any(), input).Return(nil)
...
func (s *migrationSuite) TestImportOffersFail(c *tc.C) {
	...
	s.modelDBState.EXPECT().ImportOffers(gomock.Any(), input).Return(applicationerrors.ApplicationNotFound)
