...
func (s *baseSuite) setupSimpleTwoEndpointRelation(c *tc.C,
	...
	// Create charm relations (needed for endpoint names in the view)
	charmRelation1 := s.addCharmRelation(c, charmUUID1, charm.Relation{
		Name:      endpoint1Name,
		Role:      role1,
		Interface: "test-interface",
		Scope:     charm.ScopeGlobal,
	})
	charmRelation2 := s.addCharmRelation(c, charmUUID2, charm.Relation{
		Name:      endpoint2Name,
		Role:      role2,
		Interface: "test-interface",
		Scope:     charm.ScopeGlobal,
	...
	// Create applications (needed for application names in the view)
	appUUID1 := s.addApplication(c, charmUUID1, app1Name)
	appUUID2 := s.addApplication(c, charmUUID2, app2Name)

	// Create application endpoints (needed to link apps to charm relations)
	endpointUUID1 := s.addApplicationEndpoint(c, appUUID1, charmRelation1)
	endpointUUID2 := s.addApplicationEndpoint(c, appUUID2, charmRelation2)
	...
	return relationUUID, relationKey
}
