...
func (s *serviceSuite) TestGetSecretBackendByName(c *tc.C) {
	ctrl := s.setupMocks(c)
	defer ctrl.Finish()
	...
	backend, err := svc.GetSecretBackendByName(c.Context(), "myvault")
	c.Assert(err, tc.ErrorIsNil)
	c.Check(backend, tc.DeepEquals, &secretbackend.SecretBackend{
		ID:          "backend-uuid",
		Name:        "myvault",
		BackendType: "vault",
	})
...
func (s *serviceSuite) TestGetSecretBackendByNameNotFound(c *tc.C) {
	ctrl := s.setupMocks(c)
	defer ctrl.Finish()
	...
	s.mockState.EXPECT().GetSecretBackend(gomock.Any(), secretbackend.BackendIdentifier{Name: "missing"}).Return(
		nil, secretbackenderrors.NotFound)

	_, err := svc.GetSecretBackendByName(c.Context(), "missing")
	c.Assert(err, tc.ErrorIs, secretbackenderrors.NotFound)
}
