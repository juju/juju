...
func (s *stateSuite) TestListSecretBackendsIAAS(c *gc.C) {
	...
	modelUUID := s.createModel(c, coremodel.IAAS)
	err = s.state.setModelSecretBackendForTest(context.Background(), modelUUID, "my-backend1")
	c.Assert(err, gc.IsNil)
...
func (s *stateSuite) assertListSecretBackendsForModelIAAS(c *gc.C, includeEmpty bool) {
	modelUUID := s.createModel(c, coremodel.IAAS)
	err := s.state.setModelSecretBackendForTest(context.Background(), modelUUID, "my-backend")
	c.Assert(err, gc.IsNil)
	...
	backends, err := s.state.listSecretBackendsForModelForTest(context.Background(), modelUUID, includeEmpty)
	c.Assert(err, gc.IsNil)
...
func (s *stateSuite) TestGetSecretBackendByName(c *gc.C) {
	...
	backend, err := s.state.getSecretBackendForTest(context.Background(), secretbackend.BackendIdentifier{Name: "my-backend"})
	c.Assert(err, gc.IsNil)
...
func (s *stateSuite) TestGetSecretBackend(c *gc.C) {
	...
	backend, err := s.state.getSecretBackendForTest(context.Background(), secretbackend.BackendIdentifier{ID: backendID})
	c.Assert(err, gc.IsNil)
	c.Assert(backend, jc.DeepEquals, &secretbackend.SecretBackend{
		ID:          backendID,
		Name:        "my-backend",
...
WHERE model_uuid = ?`
	...
	err = s.state.setModelSecretBackendForTest(context.Background(), modelUUID, "another-backend")
	c.Assert(err, gc.IsNil)
...
func (s *stateSuite) TestSetModelSecretBackendBackendNotFound(c *gc.C) {
	modelUUID := s.createModel(c, coremodel.IAAS)
	err := s.state.setModelSecretBackendForTest(context.Background(), modelUUID, "non-existing-backend-name")
	c.Assert(err, jc.ErrorIs, backenderrors.NotFound)
...
func (s *stateSuite) TestGetModelSecretBackendDetails(c *gc.C) {
	...
	result, err := s.state.getModelSecretBackendDetailsForTest(context.Background(), modelUUID)
	c.Assert(err, gc.IsNil)
