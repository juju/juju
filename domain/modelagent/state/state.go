...
`
	...
	stmt, err := st.Prepare(q, rval, args)
	if err != nil {
		return version.Zero, fmt.Errorf("preparing agent version query for model %s/%s: %w", user.Name(), modelName, err)
	}

	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		err := tx.Query(ctx, stmt, args).Get(&rval)
		if errors.Is(err, sql.ErrNoRows) {
			return fmt.Errorf("%w with name %s/%s", modelerrors.NotFound, user.Name(), modelName)
		} else if err != nil {
			return fmt.Errorf("cannot get agent version for model %s/%s: %w", user.Name(), modelName, err)
		}
		return nil
	})
	if err != nil {
	...
	if err != nil {
		return version.Zero, fmt.Errorf("cannot parse agent version %q for model %s/%s: %w",
			rval.TargetAgentVersion, user.Name(), modelName, err)
	}
	return vers, nil
}
