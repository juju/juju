...
// [ControllerDBModelCreationArgs.Validate] function checks for.
func (*typesSuite) TestModelCreationArgsValidation(c *gc.C) {
	userUUID := usertesting.GenUserUUID(c)

	tests := []struct {
		Args    ControllerDBModelCreationArgs
		Name    string
		ErrTest error
	}{
		{
...
func (*typesSuite) TestModelImportArgsValidation(c *gc.C) {
	...
	tests := []struct {
	...
	}{
		{
			...
			Args: ModelImportArgs{
				ControllerDBModelCreationArgs: ControllerDBModelCreationArgs{
					Cloud:       "my-cloud",
					CloudRegion: "my-region",
					Credential: credential.Key{
						Cloud: "cloud",
