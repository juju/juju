...
func (s *modelStateSuite) TestGetMachineAgentStatusPresent(c *tc.C) {
	s.createIAASApplicationWithNUnits(c, "foo", 1)
	...
	err = s.state.SetMachinePresence(c.Context(), coremachine.Name("0"))
	c.Assert(err, tc.ErrorIsNil)

	gotStatus, err := s.state.GetMachineStatus(c.Context(), "0")
	c.Assert(err, tc.ErrorIsNil)

	c.Check(gotStatus.Present, tc.IsTrue)
	assertStatusInfoEqual(c, gotStatus.StatusInfo, status)

	err = s.state.DeleteMachinePresence(c.Context(), coremachine.Name("0"))
	c.Assert(err, tc.ErrorIsNil)
...
func (s *modelStateSuite) TestGetUnitWorkloadStatusesForApplication(c *tc.C) {
	appId, unitUUIDs := s.createIAASApplicationWithNUnits(c, "foo", 1)
	unitUUID := unitUUIDs[0]
...
func (s *modelStateSuite) TestGetUnitWorkloadStatusesForApplicationMultipleUnits(c *tc.C) {
	appId, unitUUIDs := s.createIAASApplicationWithNUnits(c, "foo", 2)
...
func (s *modelStateSuite) TestGetAllUnitStatusesForApplicationUnitsWithoutStatuses(c *tc.C) {
	appId, _ := s.createIAASApplicationWithNUnits(c, "foo", 2)
...
func (s *modelStateSuite) TestGetAllFullUnitStatuses(c *tc.C) {
	_, fooUnitUUIDs := s.createIAASApplicationWithNUnits(c, "foo", 2)
	u1UUID := fooUnitUUIDs[0]
	u2UUID := fooUnitUUIDs[1]
	_, barUnitUUIDs := s.createIAASApplicationWithNUnits(c, "bar", 1)
...
func (s *modelStateSuite) TestSetUnitPresence(c *tc.C) {
	s.createIAASApplicationWithNUnits(c, "foo", 2)
...
func (s *modelStateSuite) TestGetApplicationAndUnitModelStatusesMultiple(c *tc.C) {
	s.createIAASApplicationWithNUnits(c, "foo", 2)
	s.createIAASApplicationWithNUnits(c, "bar", 1)
...
WHERE machine_uuid=?`, mUUID)
	...
	obtainedStatus, err := s.state.GetMachineStatus(c.Context(), machineName.String())
	c.Assert(err, tc.ErrorIsNil)
	c.Check(obtainedStatus, tc.DeepEquals, status.MachineStatusInfo[status.MachineStatusType]{
...
func (s *modelStateSuite) TestGetMachineStatusPendingOnCreateMachine(c *tc.C) {
	_, machineName := s.createMachine(c)
...
func (s *modelStateSuite) TestGetAllMachineStatuses(c *tc.C) {
	...
	c.Check(statuses[machineName1.String()], tc.DeepEquals, status.MachineStatusInfo[status.MachineStatusType]{
		StatusInfo: status.StatusInfo[status.MachineStatusType]{
			Status:  status.MachineStatusStarted,
			Message: "it's started",
			Data:    []byte(`{"foo": "bar"}`),
	...
	c.Check(statuses[machineName2.String()], tc.DeepEquals, status.MachineStatusInfo[status.MachineStatusType]{
		StatusInfo: status.StatusInfo[status.MachineStatusType]{
			Status:  status.MachineStatusPending,
			Message: "it's pending",
			Data:    []byte(`{"bar": "foo"}`),
...
func (s *modelStateSuite) TestGetInstanceStatusSuccess(c *tc.C) {
	machineUUID, machineName := s.createMachine(c)
...
WHERE machine_uuid=?`, machineUUID)
	...
	obtainedStatus, err := s.state.GetInstanceStatus(c.Context(), machineName.String())
	c.Assert(err, tc.ErrorIsNil)
...
// TestGetInstanceStatusMachineStatusNotFoundError asserts that
// GetInstanceStatus returns a MachineStatusNotFound error when a status value
// cannot be found for the given machine.
...
WHERE machine_uuid=?`, machineUUID)
	...
	_, err = s.state.GetInstanceStatus(c.Context(), machineName.String())
	c.Assert(err, tc.ErrorIs, statuserrors.MachineStatusNotFound)
}
...
func (s *modelStateSuite) TestGetAllInstanceStatuses(c *tc.C) {
	...
	c.Assert(statuses[machineName1.String()], tc.DeepEquals, status.StatusInfo[status.InstanceStatusType]{
		Status:  status.InstanceStatusRunning,
		Message: "it's running",
		Data:    []byte(`{"foo": "bar"}`),
	})
	c.Assert(statuses[machineName2.String()], tc.DeepEquals, status.StatusInfo[status.InstanceStatusType]{
		Status:  status.InstanceStatusPending,
		Message: "it's pending",
		Data:    []byte(`{"bar": "foo"}`),
	})
