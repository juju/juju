...
type ImportService interface {
	...
	SetUnitPasswordHash(ctx context.Context, unitName coreunit.Name, passwordHash passwords.PasswordHash) error
}
...
func (i *importOperation) Execute(ctx context.Context, model description.Model) error {
	for _, app := range model.Applications() {
		for _, unit := range app.Units() {
			...
			if err := i.service.SetUnitPasswordHash(ctx, coreunit.Name(unitName), passwords.PasswordHash(passwordHash)); err != nil {
