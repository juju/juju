...
// cloud_instance_status_value table, because we convert between core.status values
// and cloud_instance_status_value based on these associations. This test will catch
...
func (s *stateSuite) TestInstanceStatusValues(c *gc.C) {
	...
	// Check that the status values in the cloud_instance_status_value table match
	// the instance status values in core status.
	rows, err := db.QueryContext(context.Background(), "SELECT id, status FROM cloud_instance_status_value")
	defer rows.Close()
	c.Assert(err, jc.ErrorIsNil)
