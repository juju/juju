...
func (s *stateSuite) assertInsertCloud(c *gc.C, st *State, cloud cloud.Cloud) string {
	err := st.UpsertCloud(context.Background(), "admin", cloud)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestUpsertCloudUpdateExisting(c *gc.C) {
	...
	err := st.UpsertCloud(context.Background(), "admin", cld)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestListClouds(c *gc.C) {
	st := NewState(s.TxnRunnerFactory())
	err := st.UpsertCloud(context.Background(), "admin", testCloud)
	c.Assert(err, jc.ErrorIsNil)
	err = st.UpsertCloud(context.Background(), "admin", testCloud2)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestWatchCloud(c *gc.C) {
	...
	err = s.TxnRunner().StdTxn(context.Background(), func(ctx context.Context, tx *sql.Tx) error {
		err := st.UpsertCloud(ctx, "admin", cld)
		return err
	})
	c.Assert(err, jc.ErrorIsNil)
	wc.AssertOneChange()
