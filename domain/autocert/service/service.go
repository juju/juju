...
func (s *Service) Put(ctx context.Context, name string, data []byte) error {
	s.logger.Tracef(context.TODO(), "storing autocert %s with contents '%+v' in the autocert cache", name, string(data))
	return s.st.Put(ctx, name, data)
}

// Get implements autocert.Cache.Get.
func (s *Service) Get(ctx context.Context, name string) ([]byte, error) {
	s.logger.Tracef(context.TODO(), "retrieving autocert %s from the autocert cache", name)
...
func (s *Service) Delete(ctx context.Context, name string) error {
	s.logger.Tracef(context.TODO(), "removing autocert %s from the autocert cache", name)
