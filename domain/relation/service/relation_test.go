...
func (s *relationServiceSuite) TestAddRelation(c *gc.C) {
	...
	s.state.EXPECT().AddRelation(gomock.Any(), relation.EndpointIdentifier{
		ApplicationName: "application-1",
...
func (s *parseKeySuite) TestParseRelationKeyError(c *gc.C) {
	tests := []struct {
		...
		summary:     "not fully qualified endpoint",
		relationKey: "app_1:end_1 app2",
		errorRegex:  "expected endpoints of form <application-name>:<endpoint-name>, got \"app2\"",
	}}
