...
func (s *addRelationSuite) TestAddRelation(c *gc.C) {
	...
	// Act
	ep1, ep2, err := s.state.AddRelation(context.Background(), relation.EndpointIdentifier{
	...
	ep3, ep4, err := s.state.AddRelation(context.Background(), relation.EndpointIdentifier{
		ApplicationName: "application-1",
		EndpointName:    "req",
	}, relation.EndpointIdentifier{
		ApplicationName: "application-2",
...
`
	...
	c.Check(epUUIDsByRelID[1], jc.SameContents, []string{epUUID1.String(), epUUID2.String()}, gc.Commentf("full map: %v", epUUIDsByRelID))
	c.Check(epUUIDsByRelID[2], jc.SameContents, []string{epUUID3.String(), epUUID4.String()}, gc.Commentf("full map: %v", epUUIDsByRelID))
}

func (s *addRelationSuite) TestAddRelationErrorInfersEndpoint(c *gc.C) {
	// Act
	_, _, err := s.state.AddRelation(context.Background(), relation.EndpointIdentifier{
		ApplicationName: "application-1",
	}, relation.EndpointIdentifier{
		ApplicationName: "application-2",
	})
...
func (s *addRelationSuite) TestInferEndpointsError(c *gc.C) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expectedError: relationerrors.NoRelationFound,
		},
		{
...
func (s *relationSuite) addCharmRelation(c *gc.C, charmUUID, charmRelationUUID string, r charm.Relation) {
	s.query(c, `
