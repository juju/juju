...
func (s *relationSuite) TestGetRelationDetailsInScopeUnits(c *tc.C) {
	// Arrange: Add two endpoints and a relation on them.
	relationID := 7
...
func (s *relationSuite) TestEnterScope(c *tc.C) {
	...
	// Act: Enter scope.
	relationUnitUUID, err := s.state.EnterScope(c.Context(), relationUUID, unitName, settings)
...
func (s *relationSuite) TestEnterScopeSubordinate(c *tc.C) {
	...
	obtainedRelationUnitUUID, err := s.state.EnterScope(c.Context(), relationUUID, unitName1, map[string]string{})
