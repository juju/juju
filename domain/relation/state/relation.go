...
WHERE ae.application_uuid IN ($uuids[:])`, change{}, uuids{})
	...
	// Empty hash would be translated into 0, which means that application and
	// units which doesn't have a setting_hash will get a zero version number
	hashToInt := func(hash string) int64 {
		sha := []byte(hash)
		if len(sha) < 8 {
			// Avoid panic in case of empty hash
