...
func (s *relatedUnitWatcherSuite) TestGetRelatedEndpointUUIDForUnit(c *gc.C) {
	...
	// Act
	var gotEndpointUUID string
	var err error
	err = s.Txn(c, func(ctx context.Context, tx *sqlair.TX) error {
		gotEndpointUUID, err = s.state.getRelatedRelationEndpointUUIDForUnit(ctx, tx, "app1/0", relationUUID)
		return err
	})

	// Assert
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(gotEndpointUUID, gc.Equals, expectedEndpoint)
}
...
func (s *relatedUnitWatcherSuite) TestGetRelatedEndpointUUIDForUnitPeerRelation(c *gc.C) {
	...
	// Assert
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(gotEndpointUUID, gc.Equals, "")
}
...
func (s *relatedUnitWatcherSuite) TestGetRelatedUnits(c *gc.C) {
	...
	// create some units
	createUnit := func(appID application.ID) func(name coreunit.Name) getUnit {
		return func(name coreunit.Name) getUnit {
			return getUnit{
