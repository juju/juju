...
func (s *stateSuite) TestSetBlock(c *tc.C) {
	...
	err := st.SetBlock(c.Context(), int8(blockcommand.DestroyBlock), "block-message")
...
func (s *stateSuite) TestSetBlockWithNoMessage(c *tc.C) {
	st := NewState(s.TxnRunnerFactory())
	err := st.SetBlock(c.Context(), int8(blockcommand.DestroyBlock), "")
	c.Assert(err, tc.ErrorIsNil)
}

func (s *stateSuite) TestRemoveBlockWithNoExistingBlock(c *tc.C) {
	st := NewState(s.TxnRunnerFactory())
	err := st.RemoveBlock(c.Context(), int8(blockcommand.DestroyBlock))
...
func (s *stateSuite) TestGetBlocks(c *tc.C) {
	...
	err := st.SetBlock(c.Context(), int8(blockcommand.DestroyBlock), "")
	c.Assert(err, tc.ErrorIsNil)
	err = st.SetBlock(c.Context(), int8(blockcommand.ChangeBlock), "change me")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *stateSuite) TestGetBlockMessageWithNoExistingBlock(c *tc.C) {
	st := NewState(s.TxnRunnerFactory())
	message, err := st.GetBlockMessage(c.Context(), int8(blockcommand.DestroyBlock))
...
func (s *stateSuite) TestGetBlockMessage(c *tc.C) {
	st := NewState(s.TxnRunnerFactory())
	err := st.SetBlock(c.Context(), int8(blockcommand.DestroyBlock), "destroy me")
	c.Assert(err, tc.ErrorIsNil)
