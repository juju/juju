...
type WatcherState interface {
	...
	GetMachinesForEndpoints(ctx context.Context, endpointUUIDs []string) ([]coremachine.Name, error)
...
func (s *WatchableService) endpointToMachineMapper(
	...
	machineNames, err := s.st.GetMachinesForEndpoints(ctx, endpointUUIDs)
	if err != nil {
		return nil, err
	}
