...
def assess_caas_charm_deployment(client):
    ...
    gitlab_charm_path = local_charm_path(charm='k8s-gitlab', juju_ver=client.version)
    k8s_model.deploy(
        charm=gitlab_charm_path,
        config='juju-external-hostname={}'.format(external_hostname),
        resource="gitlab_image=gitlab/gitlab-ce:11.0.6-ce.0",
    )

    mysql_charm_path = local_charm_path(charm='k8s-mariadb', juju_ver=client.version)
    k8s_model.deploy(
