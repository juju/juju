...
func renderTemplates(tableColumns map[string][]columnInfo, destPackage string) (string, error) {
	...
	for table, columnInfos := range tableColumns {
		view = append(view, tableData{
			...
			UpdateConditions: updateConditions(table, columnInfos),
		})
	}
...
func updateConditions(table string, columnInfos []columnInfo) []string {
	if table == "model_migration_export" {
		return []string{
			"NEW.uuid != OLD.uuid",
			"NEW.model_uuid != OLD.model_uuid",
			"NEW.target_controller_uuid != OLD.target_controller_uuid",
	...
	conditions := make([]string, 0, len(columnInfos))
	for _, column := range columnInfos {
		conditions = append(conditions, updateCondition(column))
	}
	return conditions
}

func updateCondition(info columnInfo) string {
	if info.AllowsNull {
...
END;
{{$total := len .UpdateConditions}}
-- update trigger for {{title .Name}}
CREATE TRIGGER trg_log_{{.Name}}_update
AFTER UPDATE ON {{.Name}} FOR EACH ROW
WHEN {{range $index, $condition := .UpdateConditions}}
	{{ $condition }}{{if (notLast $index $total)}} OR{{end}}{{end}}
BEGIN
    INSERT INTO change_log (edit_type_id, namespace_id, changed, created_at)
