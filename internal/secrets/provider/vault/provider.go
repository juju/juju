...
func (p vaultProvider) CleanupModel(ctx context.Context, cfg *provider.ModelBackendConfig) (err error) {
	defer func() {
		if err == nil {
			...
			logger.Warningf(ctx, "failed to cleanup secrets for model %q: %v", cfg.ModelUUID, err)
			err = nil
		}
	}()
...
func (p vaultProvider) RestrictedConfig(
	...
	logger.Debugf(ctx, "owned secrets: %#v", owned)
	...
	logger.Debugf(ctx, "consumed secret revisions: %#v", readRevs)
	...
	logger.Tracef(ctx, "policy rules for %q: %#v", policyName, rules)

	ttl := fmt.Sprintf(
		"%dm", int(math.Ceil(secrets.IssuedTokenValidity.Minutes())),
	)
