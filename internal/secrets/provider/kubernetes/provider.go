...
func (p k8sProvider) RestrictedConfig(
	...
	logger.Tracef(ctx, "getting k8s backend config for %q, owned %v, readRevs %v",
		accessor, owned, readRevs)
...
func (k *kubernetesClient) createSecretAccessToken(
	...
	defer func() {
		...
		logger.Warningf(context.TODO(), "error ensuring secret service account for %s: %v", accessor, err)
	...
	labels := labelsForServiceAccount(k.modelName, k.modelUUID, &accessor)
	...
	appName := accessor.ID
	if accessor.Kind == coresecrets.UnitAccessor {
