...
type InitReader interface {
	...
	GetInitConfig() (map[string]any, error)
	...
	ExtractPropertiesFromConfig([]string, map[string]any, corelogger.Logger) map[string]any
}
...
func (r *MachineInitReader) GetInitConfig() (map[string]any, error) {
	if ostype.OSTypeForName(r.config.Base.OS) != ostype.Ubuntu {
		logger.Debugf(context.TODO(), "not attempting to get init config for %s container", r.config.Base.DisplayString())
		return nil, nil
	}

	base, err := utilsos.HostBase()
	if err != nil || r.config.Base != base {
		logger.Debugf(context.TODO(), "not attempting to get init config for %s, base of machine and container differ", r.config.Base.DisplayString())
		return nil, nil
	}
...
func (r *MachineInitReader) getMachineCloudCfgDirData() (map[string]any, error) {
...
func (r *MachineInitReader) unmarshallConfigFile(file string) (map[string]any, error) {
	...
	logger.Tracef(context.TODO(), "unmarshall failed (%s), file may be compressed", err.Error())
	...
	logger.Tracef(context.TODO(), "Gunzip of %q failed (%s), maybe it is encoded", file, err)
	...
	logger.Tracef(context.TODO(), "Decoding of %q failed (%s), maybe it is encoded and gzipped", file, err)
...
func fileAsConfigMap(file string) ([]byte, map[string]any, error) {
...
func (r *MachineInitReader) ExtractPropertiesFromConfig(
	keys []string, cfg map[string]any, log corelogger.Logger,
) map[string]any {
	foundDataMap := make(map[string]any)
	for _, k := range keys {
		...
		case "apt-security", "apt-primary", "apt-sources", "apt-sources_list":
			if val, ok := cfg["apt"]; ok {
				for k, v := range nestedAptConfig(key, val, log) {
					...
					if apt, ok := foundDataMap["apt"].(map[string]any); ok {
						apt[k] = v
					} else {
						foundDataMap["apt"] = map[string]any{
							k: v,
						}
					}
				}
			} else {
				log.Debugf(context.TODO(), "%s not found in machine init data", key)
			}
		case "ca-certs":
...
func nestedAptConfig(key string, val any, log corelogger.Logger) map[string]any {
...
func bytesAsConfigMap(raw []byte) (map[string]any, error) {
	dataMap := make(map[string]any)
	err := yaml.Unmarshal(raw, &dataMap)
	return dataMap, errors.Trace(err)
}

func interfaceToMapStringInterface(in any) map[string]any {
	if inMap, ok := in.(map[any]any); ok {
