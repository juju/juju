...
func TestQuayContainerRegistrySuite(t *testing.T) {
		...
		gomock.InOrder(
			...
			s.mockRoundTripper.EXPECT().RoundTrip(gomock.Any()).DoAndReturn(
				func(req *http.Request) (*http.Response, error) {
					...
					c.Check(req.URL.String(), tc.Equals, `https://quay.io/v2/`)
			...
			s.mockRoundTripper.EXPECT().RoundTrip(gomock.Any()).DoAndReturn(
				func(req *http.Request) (*http.Response, error) {
					c.Check(req.Header, tc.DeepEquals, http.Header{"Authorization": []string{"Basic " + s.authToken}})
					c.Check(req.Method, tc.Equals, `GET`)
					c.Check(req.URL.String(), tc.Equals, `https://quay.io/v2/token?scope=repository%3Ajujuqa%2Fjujud-operator%3Apull&service=quay.io`)
					return &http.Response{
						Request:    req,
...
`[1:]
	...
	gomock.InOrder(
		s.mockRoundTripper.EXPECT().RoundTrip(gomock.Any()).DoAndReturn(func(req *http.Request) (*http.Response, error) {
			c.Check(req.Header, tc.DeepEquals, http.Header{})
			c.Check(req.Method, tc.Equals, `GET`)
			c.Check(req.URL.String(), tc.Equals, `https://quay.io/v2/jujuqa/jujud-operator/tags/list`)
			resps := &http.Response{
				Request:    req,
				StatusCode: http.StatusOK,
