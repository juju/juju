...
func (s *objectStoreFactorySuite) TestNewObjectStore(c *gc.C) {
	...
	obj, err := ObjectStoreFactory(context.Background(), DefaultBackendType(), "inferi", WithLogger(jujutesting.NewCheckLogger(c)))
...
func (s *objectStoreFactorySuite) TestNewObjectStoreInvalidBackend(c *gc.C) {
	...
	_, err := ObjectStoreFactory(context.Background(), objectstore.BackendType("blah"), "inferi", WithLogger(jujutesting.NewCheckLogger(c)))
