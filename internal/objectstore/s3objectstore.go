	...
	for {
		...
		case req := <-t.requests:
			...
			case opGetBySHA256:
				...
				// with the drain requests we can at least attempt to get the
				// file from the file accessor for intermediate content.
				reader, size, err := t.getBySHA256(ctx, req.sha256, strategy)
			...
			case opGetBySHA256Prefix:
				...
				// with the drain requests we can at least attempt to get the
				// file from the file accessor for intermediate content.
				reader, size, err := t.getBySHA256Prefix(ctx, req.sha256, strategy)
...
func (t *s3ObjectStore) getBySHA256(ctx context.Context, sha256 string, strategy accessStrategy) (io.ReadCloser, int64, error) {
