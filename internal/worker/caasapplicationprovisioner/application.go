...
func (a *appWorker) loop() error {
	...
	appLife, err := a.applicationService.GetApplicationLife(ctx, name)
	if errors.Is(err, applicationerrors.ApplicationNotFound) {
	...
	appTrustWatcher, err := a.unitFacade.WatchApplicationTrustHash(ctx, name)
	if err != nil {
	...
	for {
		...
		case <-trustChan:
			...
			err := a.ops.EnsureTrust(ctx, name, app, a.unitFacade, a.logger)
			if errors.Is(err, errors.NotFound) {
		...
		if shouldRefresh {
			if err = a.ops.RefreshApplicationStatus(ctx, name, a.appID, app, appLife, a.facade, a.statusService, a.logger); err != nil {
