...
func (s *ApplicationWorkerSuite) TestLifeDead(c *tc.C) {
	...
	gomock.InOrder(
		...
		applicationService.EXPECT().GetApplicationLife(gomock.Any(), "test").Return(life.Dead, nil),
...
func (s *ApplicationWorkerSuite) TestWorker(c *tc.C) {
	...
	gomock.InOrder(
		applicationService.EXPECT().GetApplicationName(gomock.Any(), s.appID).Return("test", nil),
		broker.EXPECT().Application("test", caas.DeploymentStateful).Return(app),
		applicationService.EXPECT().GetApplicationLife(gomock.Any(), "test").Return(life.Alive, nil),
		...
		// trustChan fired
		ops.EXPECT().EnsureTrust(gomock.Any(), "test", app, unitFacade, gomock.Any()).Return(errors.NotFound),
		ops.EXPECT().EnsureTrust(gomock.Any(), "test", app, unitFacade, gomock.Any()).DoAndReturn(func(_ context.Context, _ string, _ caas.Application, _ caasapplicationprovisioner.CAASUnitProvisionerFacade, _ logger.Logger) error {
			appUnitsChan <- nil
			return nil
		}),
		...
		applicationService.EXPECT().GetApplicationLife(gomock.Any(), "test").Return(life.Dying, nil),
...
func (s *ApplicationWorkerSuite) TestWorkerStatusOnly(c *tc.C) {
	...
	gomock.InOrder(
		...
		applicationService.EXPECT().GetApplicationLife(gomock.Any(), "controller").Return(life.Alive, nil),
		...
		unitFacade.EXPECT().WatchApplicationTrustHash(gomock.Any(), "controller").Return(watchertest.NewMockStringsWatcher(trustChan), nil),
		...
		// provisioningInfoChan fired
		applicationService.EXPECT().GetApplicationLife(gomock.Any(), "controller").DoAndReturn(func(_ context.Context, _ string) (life.Value, error) {
			provisioningInfoChan <- struct{}{}
