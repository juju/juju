...
type StatusService interface {
	...
	// exist.
	GetUnitWorkloadStatusesForApplication(ctx context.Context, id coreapplication.ID) (map[unit.Name]status.StatusInfo, error)
	...
	for {
		...
		case apps, ok := <-appWatcher.Changes():
			...
			for _, id := range apps {
				...
				err = p.runner.StartWorker(id, startFunc)
				if err != nil {
					return errors.Trace(err)
				}
			}
