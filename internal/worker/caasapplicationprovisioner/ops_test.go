...
func (s *OpsSuite) TestEnsureScaleAlive(c *gc.C) {
	...
	gomock.InOrder(
		...
		facade.EXPECT().ProvisioningInfo("test").Return(api.ProvisioningInfo{}, nil),
...
func (s *OpsSuite) TestEnsureScaleWithAttachStorage(c *gc.C) {
	...
	// ProvisioningInfo with filesystem attachments
	...
	gomock.InOrder(
		...
		facade.EXPECT().ProvisioningState("test").Return(nil, nil),
		facade.EXPECT().ProvisioningInfo("test").Return(provisioningInfo, nil),
		facade.EXPECT().SetProvisioningState("test", ps).Return(nil),
		facade.EXPECT().Units("test").Return(units, nil),
		app.EXPECT().EnsurePVC(provisioningInfo.Filesystems, provisioningInfo.FilesystemUnitAttachments).Return(nil),
...
func (s *OpsSuite) TestEnsureScaleWithAttachStorageEnsurePVCFails(c *gc.C) {
	...
	gomock.InOrder(
		...
		facade.EXPECT().SetProvisioningState("test", ps).Return(nil),
		facade.EXPECT().Units("test").Return(units, nil),
		app.EXPECT().EnsurePVC(provisioningInfo.Filesystems, provisioningInfo.FilesystemUnitAttachments).Return(errors.New("PVC creation failed")),
	)
...
func (s *OpsSuite) TestAppDying(c *gc.C) {
	...
	gomock.InOrder(
		...
		app.EXPECT().EnsurePVC(nil, nil).Return(nil),
