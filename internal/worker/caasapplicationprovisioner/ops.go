...
func appAlive(ctx context.Context, appName string, app caas.Application,
	password string, lastApplied *caas.ApplicationConfig,
	facade CAASProvisionerFacade, clk clock.Clock, logger logger.Logger) error {
	logger.Debugf(context.TODO(), "ensuring application %q exists", appName)
	...
	logger.Debugf(context.TODO(), "application %q was %q", appName, reason)
	return nil
}
...
func appDying(
	...
	logger.Debugf(context.TODO(), "application %q dying", appName)
...
func appDead(
	...
	logger.Debugf(context.TODO(), "application %q dead", appName)
	err := app.Delete()
	if err != nil {
...
func checkCharmFormat(
	...
	if errors.Is(err, errors.NotFound) {
		logger.Debugf(context.TODO(), "application %q no longer exists", appName)
		return false, nil
	} else if err != nil {
...
func ensureTrust(
	...
	logger.Debugf(context.TODO(), "updating application %q trust to %v", appName, desiredTrust)
...
func updateState(
	...
	if err != nil {
		...
		logger.Warningf(context.TODO(), "update units %v", err)
	}
...
func reconcileDeadUnitScale(
	...
	logger.Infof(context.TODO(), "scaling application %q to desired scale %d", appName, desiredScale)
	...
	for _, deadUnit := range deadUnits {
		logger.Infof(context.TODO(), "removing dead unit %s", deadUnit.Tag.Id())
...
func ensureScale(
	...
	logger.Debugf(context.TODO(), "updating application %q scale to %d", appName, desiredScale)
	...
	if ps.ScaleTarget >= len(units) {
		logger.Infof(context.TODO(), "scaling application %q to desired scale %d", appName, ps.ScaleTarget)
		err = app.Scale(ps.ScaleTarget)
		if appLife != life.Alive && errors.Is(err, errors.NotFound) {
			logger.Infof(context.TODO(), "dying application %q is already removed from k8s", appName)
		} else if err != nil {
			return err
		}
		return updateProvisioningState(ctx, appName, false, 0, facade)
	...
	if ps.ScaleTarget != desiredScale {
		...
		logger.Debugf(context.TODO(), "application %q currently scaling to %d but desired scale is %d", appName, ps.ScaleTarget, desiredScale)
		return tryAgain
	}
...
func setApplicationStatus(
	...
	logger.Tracef(context.TODO(), "updating application %q status to %q, %q, %v", appName, s, reason, data)
