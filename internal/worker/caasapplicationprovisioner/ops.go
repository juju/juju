...
type ApplicationOps interface {
	...
	RefreshApplicationStatus(ctx context.Context, appName string, appID coreapplication.ID, app caas.Application, appLife life.Value,
		facade CAASProvisionerFacade, statusService StatusService, logger logger.Logger) error
...
func (applicationOps) RefreshApplicationStatus(
	...
	return refreshApplicationStatus(ctx, appName, appID, app, appLife, facade, statusService, logger)
}
...
func appAlive(ctx context.Context, appName string, app caas.Application,
	...
	if !reflect.DeepEqual(config, *lastApplied) {
		if err = app.Ensure(config); err != nil {
			_ = setApplicationStatus(ctx, appName, status.Error, err.Error(), nil, statusService, logger)
...
func refreshApplicationStatus(
	...
	unitStatuses, err := statusService.GetUnitWorkloadStatusesForApplication(ctx, appID)
	...
	if st.DesiredReplicas > 0 && st.DesiredReplicas > readyUnitsCount {
		...
		return setApplicationStatus(ctx, appName, status.Waiting, "waiting for units to settle down", nil, statusService, logger)
	}
	return setApplicationStatus(ctx, appName, status.Active, "", nil, statusService, logger)
}
...
func reconcileDeadUnitScale(
	...
	// TODO: just get a list of the units and their life.
	unitStatuses, err := statusService.GetUnitWorkloadStatusesForApplication(ctx, appID)
	if err != nil {
	...
	unitsToRemove := len(unitStatuses) - desiredScale

	var deadUnits []coreunit.Name
	for unitName := range unitStatuses {
		unitLife, err := applicationService.GetUnitLife(ctx, unitName)
		if err != nil {
			return fmt.Errorf("getting life for unit %q: %w", unitName, err)
		}
...
func ensureScale(
	...
	// TODO: Get the unit count another way.
	unitStatuses, err := statusService.GetUnitWorkloadStatusesForApplication(ctx, appID)
	if err != nil {
		return err
	}
	if ps.ScaleTarget >= len(unitStatuses) {
