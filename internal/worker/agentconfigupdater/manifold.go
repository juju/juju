		...
		Start: func(ctx context.Context, getter dependency.Getter) (worker.Worker, error) {
			...
			err = agent.ChangeConfig(func(config jujuagent.ConfigSetter) error {
				...
				if mongoProfileChanged {
					logger.Debugf(context.TODO(), "setting agent config mongo memory profile: %q => %q", agentsMongoMemoryProfile, configMongoMemoryProfile)
					config.SetMongoMemoryProfile(configMongoMemoryProfile)
				}
				if jujuDBSnapChannelChanged {
					logger.Debugf(context.TODO(), "setting agent config mongo snap channel: %q => %q", agentsJujuDBSnapChannel, configJujuDBSnapChannel)
					config.SetJujuDBSnapChannel(configJujuDBSnapChannel)
				}
				if queryTracingEnabledChanged {
					logger.Debugf(context.TODO(), "setting agent config query tracing enabled: %t => %t", agentsQueryTracingEnabled, configQueryTracingEnabled)
					config.SetQueryTracingEnabled(configQueryTracingEnabled)
				}
				if queryTracingThresholdChanged {
					logger.Debugf(context.TODO(), "setting agent config query tracing threshold: %d => %d", agentsQueryTracingThreshold, configQueryTracingThreshold)
					config.SetQueryTracingThreshold(configQueryTracingThreshold)
				}
				if openTelemetryEnabledChanged {
					logger.Debugf(context.TODO(), "setting open telemetry enabled: %t => %t", agentsOpenTelemetryEnabled, configOpenTelemetryEnabled)
					config.SetOpenTelemetryEnabled(configOpenTelemetryEnabled)
				}
				if openTelemetryEndpointChanged {
					logger.Debugf(context.TODO(), "setting open telemetry endpoint: %q => %q", agentsOpenTelemetryEndpoint, configOpenTelemetryEndpoint)
					config.SetOpenTelemetryEndpoint(configOpenTelemetryEndpoint)
				}
				if openTelemetryInsecureChanged {
					logger.Debugf(context.TODO(), "setting open telemetry insecure: %t => %t", agentsOpenTelemetryInsecure, configOpenTelemetryInsecure)
					config.SetOpenTelemetryInsecure(configOpenTelemetryInsecure)
				}
				if openTelemetryStackTracesChanged {
					logger.Debugf(context.TODO(), "setting open telemetry stack trace: %t => %t", agentsOpenTelemetryStackTraces, configOpenTelemetryStackTraces)
					config.SetOpenTelemetryStackTraces(configOpenTelemetryStackTraces)
				}
				if openTelemetrySampleRatioChanged {
					logger.Debugf(context.TODO(), "setting open telemetry sample ratio: %f => %f", agentsOpenTelemetrySampleRatio, configOpenTelemetrySampleRatio)
					config.SetOpenTelemetrySampleRatio(configOpenTelemetrySampleRatio)
				}
				if openTelemetryTailSamplingThresholdChanged {
					logger.Debugf(context.TODO(), "setting open telemetry tail sampling threshold: %f => %f", agentsOpenTelemetryTailSamplingThreshold, configOpenTelemetryTailSamplingThreshold)
					config.SetOpenTelemetryTailSamplingThreshold(configOpenTelemetryTailSamplingThreshold)
				}
				if objectStoreTypeChanged {
					logger.Debugf(context.TODO(), "setting object store type: %q => %q", agentsObjectStoreType, configObjectStoreType)
			...
			if mongoProfileChanged {
				logger.Infof(context.TODO(), "restarting agent for new mongo memory profile")
				return nil, jworker.ErrRestartAgent
			} else if jujuDBSnapChannelChanged {
				logger.Infof(context.TODO(), "restarting agent for new mongo snap channel")
				return nil, jworker.ErrRestartAgent
			} else if queryTracingEnabledChanged {
				logger.Infof(context.TODO(), "restarting agent for new query tracing enabled")
				return nil, jworker.ErrRestartAgent
			} else if queryTracingThresholdChanged {
				logger.Infof(context.TODO(), "restarting agent for new query tracing threshold")
				return nil, jworker.ErrRestartAgent
			} else if openTelemetryEnabledChanged {
				logger.Infof(context.TODO(), "restarting agent for new open telemetry enabled")
				return nil, jworker.ErrRestartAgent
			} else if openTelemetryEndpointChanged {
				logger.Infof(context.TODO(), "restarting agent for new open telemetry endpoint")
				return nil, jworker.ErrRestartAgent
			} else if openTelemetryInsecureChanged {
				logger.Infof(context.TODO(), "restarting agent for new open telemetry insecure")
				return nil, jworker.ErrRestartAgent
			} else if openTelemetryStackTracesChanged {
				logger.Infof(context.TODO(), "restarting agent for new open telemetry stack traces")
				return nil, jworker.ErrRestartAgent
			} else if openTelemetrySampleRatioChanged {
				logger.Infof(context.TODO(), "restarting agent for new open telemetry sample ratio")
				return nil, jworker.ErrRestartAgent
			} else if openTelemetryTailSamplingThresholdChanged {
				logger.Infof(context.TODO(), "restarting agent for new open telemetry tail sampling threshold")
				return nil, jworker.ErrRestartAgent
			} else if objectStoreTypeChanged {
				logger.Infof(context.TODO(), "restarting agent for new object store type")
				return nil, jworker.ErrRestartAgent
			}
			...
			if err := getter.Get(config.CentralHubName, &hub); err != nil {
				logger.Tracef(context.TODO(), "hub dependency not available")
				return nil, err
			}
