...
func (r *remoteServer) connect(ctx context.Context) bool {
	...
	_ = retry.Call(retry.CallArgs{
		Func: func() error {
			...
			conn, err := r.newWriter(ctx, r.info)
			if err != nil {
...
func (r *remoteServer) interruptConnecting() {
	...
	if r.stopConnecting != nil {
		r.logger.Debugf(context.Background(), "interrupting the pending connect loop")
		close(r.stopConnecting)
		r.stopConnecting = nil
	}
}
...
func (s *remoteServer) scopedContext() (context.Context, context.CancelFunc) {
