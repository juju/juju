...
func (s *subscriber) waitForDeath() error {
	s.config.Logger.Tracef(context.TODO(), "wait for catacomb dying before unsubscribe")
	defer s.unsubAll()
	defer s.unsubServerDetails()

	<-s.catacomb.Dying()
	s.config.Logger.Tracef(context.TODO(), "dying now")
	return s.catacomb.ErrDying()
}

func (s *subscriber) apiServerChanges(topic string, details apiserver.Details, err error) {
	s.config.Logger.Tracef(context.TODO(), "apiServerChanges: %#v", details)
	// Make sure we have workers for the defined details.
	if err != nil {
		// This should never happen.
		s.config.Logger.Errorf(context.TODO(), "subscriber callback error: %v", err)
		return
	}
	...
	for id, apiServer := range details.Servers {
		...
		if err != nil {
			// This should never happen.
			s.config.Logger.Errorf(context.TODO(), "subscriber origin tag error: %v", err)
			continue
		}
		...
		default:
			// This should never happen.
			s.config.Logger.Errorf(context.TODO(), "unknown subscriber origin tag: %v", originTag)
			continue
		}
		...
		if found {
			s.config.Logger.Tracef(context.TODO(), "update addresses for %s to %v", target, addresses)
			server.UpdateAddresses(addresses)
		} else {
			s.config.Logger.Debugf(context.TODO(), "new forwarder for %s", target)
			...
			if err != nil {
				s.config.Logger.Errorf(context.TODO(), "unable to add new remote server for %q, %v", target, err)
				continue
			}
	...
	for name, server := range s.servers {
		if !apiServers.Contains(name) {
			s.config.Logger.Debugf(context.TODO(), "%s no longer listed as an apiserver", name)
			server.Kill()
			err := server.Wait()
			if err != nil {
				s.config.Logger.Errorf(context.TODO(), "%v", err)
			}
			delete(s.servers, name)
		}
	}
	s.config.Logger.Tracef(context.TODO(), "update complete")
}
...
func (s *subscriber) forwardMessage(topic string, data map[string]interface{}) {
	if data["origin"] != s.config.Origin {
		...
		// Nothing to do.
		s.config.Logger.Tracef(context.TODO(), "skipping message %q as origin not ours", topic)
		return
	}
	...
	if data["local-only"] == true {
		...
		s.config.Logger.Tracef(context.TODO(), "skipping message %q as local-only", topic)
		return
	}

	s.config.Logger.Tracef(context.TODO(), "forward message %q", topic)
