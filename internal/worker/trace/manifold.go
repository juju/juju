	...
	return dependency.Manifold{
		...
		Start: func(ctx context.Context, getter dependency.Getter) (worker.Worker, error) {
			...
			if !currentConfig.OpenTelemetryEnabled() {
				config.Logger.Infof(context.TODO(), "OpenTelemetry disabled, not starting trace worker")
				return NewNoopWorker(), nil
			}

			endpoint := currentConfig.OpenTelemetryEndpoint()

			config.Logger.Infof(context.TODO(), "OpenTelemetry enabled, starting trace worker using endpoint %q", endpoint)
