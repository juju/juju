...
func (s *machineSessionSuite) startTestServer(_ *gc.C) *testServer {
	ts := &testServer{}
...
func (s *machineSessionSuite) TestMachineSessionProxy(c *gc.C) {
	...
	testServer := s.startTestServer(c)
	...
	handlers, err := NewHandlers(virtualhostname.Info{}, s.mockConnector, loggo.GetLogger("test"))
	c.Assert(err, jc.ErrorIsNil)

	handlers.SessionHandler(s.userSession)
...
func (s *machineSessionSuite) TestConnectToMachineError(c *gc.C) {
	...
	c.Check(s.userSession.stderr.String(), gc.Equals, "failed to connect to machine: fake-connection-error\n")
}
