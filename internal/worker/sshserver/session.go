...
// SSHConnecter is an interface that defines the methods required to
// connect to a remote SSH server and determine the model type of the target.
type SSHConnecter interface {
	Connect(destinationAddress string) (*gossh.Client, error)
	ModelType(destinationAddress string) (state.ModelType, error)
}

type sessionHandler struct {
	connector SSHConnecter
}
...
func (s *sessionHandler) Handle(session ssh.Session, destinationAddress string) {
	handleError := func(err error) {
		_, _ = session.Stderr().Write([]byte(err.Error() + "\n"))
		_ = session.Exit(1)
	}
	modelType, err := s.connector.ModelType(destinationAddress)
	if err != nil {
		err = errors.Annotate(err, "failed to get model type")
		handleError(err)
		return
	}
	...
	case state.ModelTypeIAAS:
		if err := s.MachineSessionProxy(session, destinationAddress); err != nil {
			err = errors.Annotate(err, "failed to proxy machine session")
			handleError(err)
		}
	default:
		err = errors.Errorf("unknown model type %s", modelType)
		handleError(err)
	}
}

func (s *sessionHandler) K8sSessionProxy(sess ssh.Session) error {
	return errors.New("k8s session proxy not implemented")
}

func (s *sessionHandler) MachineSessionProxy(userSession ssh.Session, destinationAddress string) error {
	client, err := s.connector.Connect(destinationAddress)
	if err != nil {
		return err
	}
	defer client.Close()
