...
type machineSessionSuite struct {
	...
	mockConnector *MockSSHConnecter
}
...
func (s *machineSessionSuite) setupMocks(c *gc.C) *gomock.Controller {
	...
	s.mockConnector = NewMockSSHConnecter(ctrl)
	return ctrl
}
...
func (s *machineSessionSuite) TestMachineSessionProxy(c *gc.C) {
	...
	s.mockConnector.EXPECT().Connect(gomock.Any()).DoAndReturn(
		func(s string) (*gossh.Client, error) {
			c.Check(s, gc.Equals, "my-fake-destination")
	...
	err = sessionHandler.MachineSessionProxy(s.userSession, "my-fake-destination")
...
func (s *machineSessionSuite) TestConnectToMachineError(c *gc.C) {
	...
	s.mockConnector.EXPECT().ModelType(gomock.Any()).Return(state.ModelTypeIAAS, nil)
	...
	sessionHandler.Handle(s.userSession, "my-fake-destination")
...
func (s *machineSessionSuite) TestFailureToGetModelType(c *gc.C) {
	defer s.setupMocks(c).Finish()

	isPty := false
	s.setupUserSession(c, isPty, "neovim")

	s.mockConnector.EXPECT().ModelType(gomock.Any()).Return(state.ModelTypeIAAS, errors.New("fetch-model-error"))
	...
	c.Check(s.userSession.exitCode, gc.Equals, 1)
	c.Check(s.userSession.stdout.String(), gc.Equals, "")
	c.Check(s.userSession.stderr.String(), gc.Equals, "failed to get model type: fetch-model-error\n")
}
