...
func (s *sshServerSuite) TestSSHServerNoAuth(c *gc.C) {
	...
	s.sessionHandler.EXPECT().Handle(gomock.Any(), gomock.Any()).DoAndReturn(
		func(session ssh.Session, destination string) {
			c.Check(destination, gc.Equals, "1.postgresql.8419cd78-4993-4c3a-928e-c646226beeee.juju.local")
			_, _ = session.Write(fmt.Appendf([]byte{}, "Your final destination is: %s\n", destination))
		},
	)
	output, err := terminatingSession.CombinedOutput("")
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(string(output), gc.Equals, "Your final destination is: 1.postgresql.8419cd78-4993-4c3a-928e-c646226beeee.juju.local\n")
