...
func NewFirewaller(cfg Config) (worker.Worker, error) {
	...
	fw := &Firewaller{
		...
		clk:                        clk,
		logger:                     cfg.Logger,
		serviceFactory:             cfg.ServiceFactoryGetter.FactoryForModel(cfg.ModelUUID),
...
func (fw *Firewaller) setUp(ctx stdcontext.Context) error {
	var err error
	fw.machinesWatcher, err = fw.serviceFactory.Machine().WatchMachines(ctx)
	if err != nil {
...
func (fw *Firewaller) startMachine(ctx context.Context, tag names.MachineTag) error {
	...
	case change, ok := <-unitw.Changes():
		...
		err = fw.unitsChanged(ctx, &unitsChange{machined, change})
		if err != nil {
