...
func (s *manifoldSuite) TestManifoldInputs(c *tc.C) {
	...
	c.Assert(manifold.Inputs, tc.DeepEquals, []string{"domain-services", "clock"})
}
...
func (s *manifoldSuite) TestManifoldUninstallWhenFeatureFlagDisabled(c *tc.C) {
	defer s.setupMocks(c).Finish()
	...
	c.Assert(errors.Is(err, dependency.ErrUninstall), tc.IsTrue)
}
