...
func (mr *Machiner) SetUp(ctx context.Context) (watcher.NotifyWatcher, error) {
	...
	case corelife.IsNotAlive(m.Life()):
		...
		logger.Infof(context.TODO(), "%q not alive", mr.config.Tag)
		return m.Watch(ctx)
	case mr.config.ClearMachineAddressesOnStart:
		logger.Debugf(context.TODO(), "machiner configured to reset machine %q addresses to empty", mr.config.Tag)
	...
	logger.Infof(context.TODO(), "%q started", mr.config.Tag)
...
func setMachineAddresses(ctx context.Context, tag names.MachineTag, m Machine) error {
	...
	logger.Infof(context.TODO(), "setting addresses for %q to %v", tag, hostAddresses)
...
func (mr *Machiner) Handle(ctx context.Context) error {
	...
	if life == corelife.Alive {
		...
		if err != nil {
			...
			logger.Warningf(context.TODO(), "not updating network config: no observed config found to update")
		}
		...
		logger.Debugf(context.TODO(), "observed network config updated for %q to %+v", mr.config.Tag, observedConfig)

		return nil
	}

	logger.Debugf(context.TODO(), "%q is now %s", mr.config.Tag, life)
	...
	if err := mr.machine.EnsureDead(ctx); err != nil {
		if params.IsCodeHasAssignedUnits(err) {
			logger.Tracef(context.TODO(), "machine still has units")
			return nil
		}
		if params.IsCodeMachineHasAttachedStorage(err) {
			logger.Tracef(context.TODO(), "machine still has storage attached")
			return nil
		}
		if params.IsCodeTryAgain(err) {
			logger.Tracef(context.TODO(), "waiting for machine to be removed as a controller")
			return nil
		}
		if params.IsCodeMachineHasContainers(err) {
			logger.Tracef(context.TODO(), "machine still has containers")
		...
		if e := mr.machine.SetStatus(ctx, status.Error, errors.Annotate(err, "destroying machine").Error(), nil); e != nil {
			logger.Errorf(context.TODO(), "failed to set status for error %v ", err)
		}
