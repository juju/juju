...
func (c *APIAddressUpdater) Handle(ctx context.Context) error {
	...
	if len(hps) == 0 {
		c.config.Logger.Warningf(context.TODO(), "empty API host ports received. Updating using existing entries.")
	}

	c.config.Logger.Debugf(context.TODO(), "updating API hostPorts to %+v", hps)
	c.mu.Lock()
...
func (c *APIAddressUpdater) getAddresses(ctx context.Context) ([]corenetwork.ProviderHostPorts, error) {
	...
	// Logging to identify lp: 1888453
	if len(hpsToSet) == 0 {
		c.config.Logger.Warningf(context.TODO(), "get address returning zero results after filtering, non filtered list: %v", addresses)
	}
