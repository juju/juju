...
func (s *HandleSuite) TestInvalidAction(c *gc.C) {
	results, err := machineactions.HandleAction(context.Background(), "invalid", nil)
	c.Assert(err, gc.ErrorMatches, "unexpected action invalid")
	c.Assert(results, gc.IsNil)
}

func (s *HandleSuite) TestValidActionInvalidParams(c *gc.C) {
	results, err := machineactions.HandleAction(context.Background(), actions.JujuExecActionName, nil)
...
func (s *HandleSuite) TestTimeoutRun(c *gc.C) {
	...
	results, err := machineactions.HandleAction(context.Background(), actions.JujuExecActionName, params)
