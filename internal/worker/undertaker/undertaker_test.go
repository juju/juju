...
func (s *OldUndertakerSuite) TestAlreadyDeadRemoves(c *gc.C) {
	...
	stub.CheckCallNames(c, "WatchModel", "ModelInfo", "SetStatus", "ModelConfig", "CloudSpec", "Destroy", "RemoveModelSecrets", "RemoveModelProfiles", "RemoveModel")
}
...
func (s *OldUndertakerSuite) TestSetStatusDestroying(c *gc.C) {
	...
	stub.CheckCallNames(c,
		...
		"SetStatus", "ModelConfig", "CloudSpec", "Destroy", "RemoveModelSecrets", "RemoveModelProfiles", "RemoveModel")
	stub.CheckCall(
...
func (s *OldUndertakerSuite) TestRemoveModelErrorFatal(c *gc.C) {
	s.fix.errors = []error{nil, nil, nil, nil, nil, nil, nil, nil, errors.New("pow")}
	s.fix.info.Result.Life = "dead"
	s.fix.dirty = true
	stub := s.fix.run(c, func(w worker.Worker) {
		err := workertest.CheckKilled(c, w)
	...
	mainCalls, destroyCloudCalls := s.sortCalls(c, stub)
	c.Assert(mainCalls, jc.DeepEquals, []string{"WatchModel", "ModelInfo", "SetStatus", "RemoveModelSecrets", "RemoveModelProfiles", "RemoveModel"})
	c.Assert(destroyCloudCalls, jc.DeepEquals, []string{"ModelConfig", "CloudSpec", "Destroy"})
}

func (s *OldUndertakerSuite) TestRemoveModelProfileErrorFatal(c *gc.C) {
	s.fix.errors = []error{nil, nil, nil, nil, nil, nil, nil, errors.New("connection issue")}
	s.fix.info.Result.Life = "dead"
	s.fix.dirty = true
	stub := s.fix.run(c, func(w worker.Worker) {
		err := workertest.CheckKilled(c, w)
		c.Check(err, gc.ErrorMatches, "cannot remove model profiles: connection issue")
	})
	mainCalls, destroyCloudCalls := s.sortCalls(c, stub)
	c.Assert(mainCalls, jc.DeepEquals, []string{"WatchModel", "ModelInfo", "SetStatus", "RemoveModelSecrets", "RemoveModelProfiles"})
	c.Assert(destroyCloudCalls, jc.DeepEquals, []string{"ModelConfig", "CloudSpec", "Destroy"})
}
...
func (s *OldUndertakerSuite) TestEnvironDestroyTimeout(c *gc.C) {
	...
	mainCalls, destroyCloudCalls := s.sortCalls(c, stub)
	c.Assert(mainCalls, jc.DeepEquals, []string{"WatchModel", "ModelInfo", "SetStatus", "WatchModelResources", "ProcessDyingModel", "SetStatus", "RemoveModelSecrets", "RemoveModelProfiles", "RemoveModel"})
	c.Assert(destroyCloudCalls, jc.DeepEquals, []string{"ModelConfig", "CloudSpec", "Destroy"})
