...
func (rh *runHook) Prepare(ctx stdcontext.Context, state State) (*State, error) {
	...
	if leaderNeeded {
		...
		if err == nil && !isLeader {
			rh.logger.Infof(stdcontext.TODO(), "unit is no longer the leader; skipping %q execution", name)
...
func (rh *runHook) Execute(ctx stdcontext.Context, state State) (*State, error) {
	...
	case cause == runner.ErrTerminated:
		...
		rh.logger.Warningf(stdcontext.TODO(), "hook %q was terminated", rh.name)
		step = Queued
		return stateChange{
			Kind:     RunHook,
			Step:     step,
	...
	case err == nil:
	default:
		rh.logger.Errorf(stdcontext.TODO(), "hook %q (via %s) failed: %v", rh.name, handlerType, err)
		rh.callbacks.NotifyHookFailed(rh.name, rh.runner.Context())
		return nil, ErrHookFailed
	}

	if rh.hookFound {
		rh.logger.Infof(stdcontext.TODO(), "ran %q hook (via %s)", rh.name, handlerType)
		rh.callbacks.NotifyHookCompleted(rh.name, rh.runner.Context())
	} else {
		rh.logger.Infof(stdcontext.TODO(), "skipped %q hook (missing)", rh.name)
	}
...
func (rh *runHook) beforeHook(ctx stdcontext.Context, state State) error {
	...
	if err != nil {
		rh.logger.Errorf(stdcontext.TODO(), "error updating workload status before %v hook: %v", rh.info.Kind, err)
		return err
	}
	return nil
}
...
func (rh *runHook) afterHook(stdCtx stdcontext.Context, state State) (_ bool, err error) {
	defer func() {
		if err != nil {
			rh.logger.Errorf(stdcontext.TODO(), "error updating workload status after %v hook: %v", rh.info.Kind, err)
		}
	}()
	...
	case hooks.Start:
		...
		rh.logger.Debugf(stdcontext.TODO(), "unit %v has started but has not yet set status", ctx.UnitName())
...
func (rh *runHook) Commit(ctx stdcontext.Context, state State) (*State, error) {
	...
	case hooks.SecretRotate:
		...
		rh.logger.Debugf(stdcontext.TODO(), "set secret rotated for %q, original rev %v", rh.info.SecretURI, originalRevision)
