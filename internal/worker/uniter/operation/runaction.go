...
// of the operation.
func (ra *runAction) RemoteStateChanged(ctx context.Context, snapshot remotestate.Snapshot) {
	actionID := ra.action.ID()
	change, ok := snapshot.ActionChanged[actionID]
	if !ok {
		ra.logger.Errorf(ctx, "action %s missing action changed entry", actionID)
		// Shouldn't happen.
		return
	}
	if ra.change < change {
		ra.change = change
		ra.logger.Errorf(ctx, "running action %s changed", actionID)
		select {
