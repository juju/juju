...
func (runner *runner) runJujuExecAction(ctx stdcontext.Context) (err error) {
	...
	logger.Debugf(stdcontext.TODO(), "juju-exec action is running")
	...
	if !ok {
		logger.Debugf(stdcontext.TODO(), "unable to read juju-exec action timeout, will continue running action without one")
	}
...
func (runner *runner) RunAction(ctx stdcontext.Context, actionName string) (HookHandlerType, error) {
	...
	runner.logger().Debugf(stdcontext.TODO(), "running action %q", actionName)
	...
	if session, _ := debugctx.FindSession(); session != nil && session.MatchHook(hookName) {
		...
		if session.DebugAt() != "" {
			if hookHandlerType == InvalidHookHandler {
				logger.Infof(stdcontext.TODO(), "debug-code active, but hook %s not implemented (skipping)", hookName)
				return InvalidHookHandler, err
			}
			logger.Infof(stdcontext.TODO(), "executing %s via debug-code; %s", hookName, hookHandlerType)
		} else {
			logger.Infof(stdcontext.TODO(), "executing %s via debug-hooks; %s", hookName, hookHandlerType)
		}
...
func (runner *runner) startJujucServer() (*jujuc.Server, error) {
	...
	runner.logger().Debugf(stdcontext.TODO(), "starting jujuc server %v", socket)
