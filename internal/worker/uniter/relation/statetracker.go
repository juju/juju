...
func (r *relationStateTracker) loadInitialState(ctx stdcontext.Context) error {
	...
	if r.logger.IsLevelEnabled(logger.TRACE) {
		if mgr, ok := r.stateMgr.(*stateManager); ok {
			r.logger.Tracef(stdcontext.TODO(), "initialising relation state tracker: %# v", pretty.Formatter(mgr.relationState))
		}
	}
...
func (r *relationStateTracker) joinRelation(ctx stdcontext.Context, rel api.Relation) (err error) {
	...
	r.logger.Tracef(stdcontext.TODO(), "%q (re-)joining: %q", unitName, rel)
	...
	defer func() {
		if e := worker.Stop(unitWatcher); e != nil {
			if err == nil {
				...
				r.logger.Errorf(stdcontext.TODO(), "while stopping unit watcher: %v", e)
			}
		}
	}()
	...
	for {
		...
		case _, ok := <-unitWatcher.Changes():
			...
			if params.IsCodeCannotEnterScopeYet(err) {
				r.logger.Infof(stdcontext.TODO(), "cannot enter scope for relation %q; waiting for subordinate to be removed", rel)
				continue
			} else if err != nil {
				return errors.Trace(err)
			}
			...
			r.logger.Debugf(stdcontext.TODO(), "unit %q (leader=%v) entered scope for relation %q", unitName, isLeader, rel)
			if isLeader {
...
func (r *relationStateTracker) SynchronizeScopes(ctx stdcontext.Context, remote remotestate.Snapshot) error {
	...
	if isTraceEnabled {
		r.logger.Tracef(stdcontext.TODO(), "%q synchronise scopes for remote relations %# v", r.unit.Name(), pretty.Formatter(remote.Relations))
	}
	...
	for id, relationSnapshot := range remote.Relations {
		if relr, found := r.relationers[id]; found {
			...
			if isTraceEnabled {
				r.logger.Tracef(stdcontext.TODO(), "already seen relation id %v", id)
			}
			continue
		}
		...
		if err != nil {
			if params.IsCodeNotFoundOrCodeUnauthorized(err) {
				r.relationGone(id)
				r.logger.Tracef(stdcontext.TODO(), "relation id %v has been removed", id)
				continue
			}
			return errors.Trace(err)
		}
		...
		if !r.RelationCreated(id) {
			if charmSpec == nil {
				...
				if err != nil {
					...
					r.logger.Warningf(stdcontext.TODO(), "charm deleted, skipping relation endpoint check for %q", rel)
				}
			}
			if charmSpec != nil && !ep.ImplementedBy(charmSpec) {
				r.logger.Warningf(stdcontext.TODO(), "skipping relation %q with unknown endpoint %q", rel, ep.Name)
				continue
			}
		}
...
func (r *relationStateTracker) PrepareHook(hookInfo hook.Info) (string, error) {
	...
	if !found {
		...
		r.logger.Warningf(stdcontext.TODO(), "preparing hook %v for %v, relation %d has been removed", hookInfo.Kind, r.principalName, hookInfo.RelationId)
...
func (r *relationStateTracker) CommitHook(ctx stdcontext.Context, hookInfo hook.Info) (err error) {
	...
	if !found {
		...
		// to continue with the next operation.
		r.logger.Warningf(stdcontext.TODO(), "committing hook %v for %v, relation %d has been removed", hookInfo.Kind, r.principalName, hookInfo.RelationId)
		return nil
	}
