...
func (n *pebbleNoticer) run(containerName string) (err error) {
	...
	n.logger.Debugf(context.TODO(), "container %q: pebbleNoticer starting", containerName)
	defer n.logger.Debugf(context.TODO(), "container %q: pebbleNoticer stopped, error %v", containerName, err)
	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &socketNotFound) {
				...
				n.logger.Debugf(context.TODO(), "container %q: socket %q not found, waiting %s",
					containerName, socketNotFound.Path, errorDelay)
			} else {
				n.logger.Errorf(context.TODO(), "container %q: WaitNotices error, waiting %s: %v",
...
func (n *pebbleNoticer) processNotice(containerName string, notice *client.Notice, pebbleClient PebbleClient) error {
	...
	case client.ChangeUpdateNotice:
		...
		if kind != "perform-check" && kind != "recover-check" {
			n.logger.Debugf(context.TODO(), "container %q: ignoring %s notice, kind %s", containerName, notice.Type, kind)
			return nil
		}
		...
		default:
			n.logger.Debugf(context.TODO(), "container %q: ignoring %s, status %s", containerName, kind, chg.Status)
			return nil
		}
		event = container.WorkloadEvent{
			Type:         eventType,
	...
	default:
		n.logger.Debugf(context.TODO(), "container %q: ignoring %s notice", containerName, notice.Type)
		return nil
	}

	n.logger.Debugf(context.TODO(), "container %q: processing %s notice, key %q", containerName, notice.Type, notice.Key)
