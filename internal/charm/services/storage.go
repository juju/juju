...
func (s *CharmStorage) Store(ctx context.Context, charmURL string, downloadedCharm charmdownloader.DownloadedCharm) (string, error) {
	s.logger.Tracef(context.TODO(), "store %q", charmURL)
	...
	if err != nil {
		...
		if err := s.objectStore.Remove(ctx, storagePath); err != nil {
			if alreadyUploaded {
				s.logger.Errorf(context.TODO(), "cannot remove duplicated charm archive from storage: %v", err)
			} else {
				s.logger.Errorf(context.TODO(), "cannot remove unsuccessfully recorded charm archive from storage: %v", err)
			}
		}
		if alreadyUploaded {
