...
func (d *Downloader) DownloadAndStore(ctx context.Context, charmURL *charm.URL, requestedOrigin corecharm.Origin, force bool) (corecharm.Origin, error) {
	...
	if err := d.storage.PrepareToStoreCharm(charmURL.String()); err != nil {
		...
		if alreadyUploadedErr, valid := errors.Cause(err).(errCharmAlreadyStored); valid {
			d.logger.Debugf(ctx, "%v", alreadyUploadedErr)
	...
	defer func() {
		...
		if err := os.Remove(tmpFile.Name()); err != nil {
			d.logger.Warningf(ctx, "unable to remove temporary charm download path %q", tmpFile.Name())
		}
	}()
...
func (d *Downloader) downloadAndHash(ctx context.Context, charmName string, requestedOrigin corecharm.Origin, repo CharmRepository, dstPath string) (DownloadedCharm, corecharm.Origin, error) {
	d.logger.Debugf(ctx, "downloading charm %q from requested origin %v", charmName, requestedOrigin)
	...
	d.logger.Debugf(ctx, "downloaded charm %q from actual origin %v", charmName, actualOrigin)
	...
	d.logger.Tracef(ctx, "downloadResult(%q) sha: %q, size: %d", f.Name(), sha, size)
