...
// is called.
//
// AuthResult implements the [auth.AuthResult] interface.
...
// For the moment this value is hard coded but should be swapped out for a more
// unique domain that uniquely represents the JAAS controller.
const jaasUserDomain string = "jaas"
...
func (a AuthResult) AuthenticatedActor(ctx context.Context) (
	...
	userName, err := coreuser.ParseNameWithDomain(a.userName, userDomain)
	...
	// Ensure that an external user exists in the controller to represents the
	// JAAS user.
	userUUID, err := a.userService.EnsureExternalUser(
		ctx, userName, a.userName,
	)
	if err != nil {
		return "", "", errors.Errorf(
			"ensuring external JAAS user %q with domain %q exists in the controller: %w",
			a.userName, userDomain, err,
		)
	}
