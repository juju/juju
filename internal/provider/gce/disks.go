...
func (c instanceCache) update(gceClient gceConnection, ctx context.Context, ids ...string) error {
	if len(ids) == 1 {
		if _, ok := c[ids[0]]; ok {
			return nil
		}
...
func (v *volumeSource) attachOneVolume(ctx context.Context, volumeName string, mode google.DiskMode, instanceId string) (*google.AttachedDisk, error) {
