...
func (s *cinderVolumeSource) CreateVolumes(
	...
	for i, arg := range args {
		...
		if err != nil {
			...
			if denied := common.HandleCredentialError(ctx, s.credentialInvalidator, IsAuthorisationFailure, err); denied {
...
func (s *cinderVolumeSource) ListVolumes(ctx envcontext.ProviderCallContext) ([]string, error) {
	...
	if err != nil {
		handleCredentialError(ctx, s.credentialInvalidator, err)
		return nil, errors.Trace(err)
	}
...
func foreachVolume(ctx context.Context, invalidator environs.CredentialInvalidator, storageAdaptor OpenstackStorage, volumeIds []string, f func(context.Context, environs.CredentialInvalidator, OpenstackStorage, string) error) []error {
...
func destroyVolume(ctx context.Context, invalidator environs.CredentialInvalidator, storageAdaptor OpenstackStorage, volumeId string) error {
	...
	if err != nil {
		...
		handleCredentialError(ctx, invalidator, err)
		return errors.Trace(err)
	}
...
func releaseVolume(ctx context.Context, invalidator environs.CredentialInvalidator, storageAdaptor OpenstackStorage, volumeId string) error {
...
func (s *cinderVolumeSource) AttachVolumes(ctx envcontext.ProviderCallContext, args []storage.VolumeAttachmentParams) ([]storage.AttachVolumesResult, error) {
	...
	for i, arg := range args {
		...
		if err != nil {
			results[i].Error = errors.Trace(err)
			if denial := handleCredentialError(ctx, s.credentialInvalidator, err); denial {
				// We do not want to continue here as we'll 100% fail if we got
				// unauthorised error.
				break
			}
			continue
...
func (s *cinderVolumeSource) ImportVolume(ctx envcontext.ProviderCallContext, volumeId string, resourceTags map[string]string) (storage.VolumeInfo, error) {
...
func detachVolumes(ctx context.Context, invalidator environs.CredentialInvalidator, storageAdaptor OpenstackStorage, args []storage.VolumeAttachmentParams) []error {
