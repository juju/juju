...
func (s *applierSuite) TestRun(c *tc.C) {
	...
	c.Assert(applier.Run(c.Context(), false), tc.ErrorIsNil)
}
...
func (s *applierSuite) TestRunApplyFailedWithRollBackForNewResource(c *tc.C) {
	...
	c.Assert(applier.Run(c.Context(), false), tc.ErrorMatches, `applying resource "r2": something was wrong`)
}
...
func (s *applierSuite) TestRunApplyResourceVersionChanged(c *tc.C) {
	...
	c.Assert(applier.Run(c.Context(), false), tc.ErrorMatches, `applying resource "r1": resource version conflict`)
}
...
func (s *applierSuite) TestRunDeleteFailedWithRollBack(c *tc.C) {
	...
	c.Assert(applier.Run(c.Context(), false), tc.ErrorMatches, `deleting resource "r2": something was wrong`)
}
