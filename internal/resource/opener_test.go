...
func (s *OpenerSuite) expectCacheMethods(res resource.Resource, numConcurrentRequests int) {
	if s.unitName != "" {
		s.resources.EXPECT().OpenResourceForUniter("postgresql/0", "wal-e").DoAndReturn(func(unitName, resName string) (resource.Resource, io.ReadCloser, error) {
			s.unleash.Lock()
			defer s.unleash.Unlock()
			return resource.Resource{}, io.NopCloser(bytes.NewBuffer([]byte{})), errors.NotFoundf("wal-e")
		})
	} else {
		s.resources.EXPECT().OpenResource("postgresql", "wal-e").Return(resource.Resource{}, io.NopCloser(bytes.NewBuffer([]byte{})), errors.NotFoundf("wal-e"))
	}
...
func (s *OpenerSuite) TestGetResourceErrorReleasesLock(c *gc.C) {
	...
	c.Check(opened.Resource, gc.DeepEquals, resource.Resource{})
