...
// ResolveBootstrapAddress returns the unique best provider address for the
// initial controller. If the provider result is absent or ambiguous, it tries
// the host's local interfaces before falling back to loopback.
func (m *NodeManager) ResolveBootstrapAddress(
	providerAddresses corenetwork.ProviderAddresses,
	source corenetwork.ConfigSource,
) (string, error) {
	cloudLocal := providerAddresses.AllMatchingScope(
		corenetwork.ScopeMatchCloudLocal,
	).Values()
	if len(cloudLocal) == 1 {
		return cloudLocal[0], nil
	}
	...
	return dqliteBootstrapBindIP, nil
}
