	...
	for _, v := range services {
		spec := &core.Service{
			ObjectMeta: v1.ObjectMeta{
				...
				Labels:      LabelsMerge(v.Labels, getServiceLabels(appName, k.IsLegacyLabels())),
...
func (k *kubernetesClient) ensureK8sService(spec *core.Service) (func(), error) {
	...
	existing, err := api.Get(spec.Name, v1.GetOptions{})
	if err == nil {
	...
	if k8serrors.IsNotFound(err) {
		...
		svcCreated, err = api.Create(spec)
		if err == nil {
...
func (k *kubernetesClient) deleteServices(appName string) error {
	...
	services, err := api.List(
		v1.ListOptions{
			LabelSelector: LabelSetToSelector(getServiceLabels(appName, k.IsLegacyLabels())).String(),
		},
	)
	if err != nil {
		return errors.Trace(err)
