...
func (k *klogAdapter) Info(level int, msg string, keysAndValues ...any) {
	if prefix, ok := klogSuppressedPrefixes.Matches(msg); ok && prefix != nil {
		prefix.Do(context.TODO(), k.Logger.Infof, msg, keysAndValues...)
		return
	}
...
func (k klogSuppressMessagePrefix) Do(ctx context.Context, loggerFn func(context.Context, string, ...any), msg string, args ...any) {
	// If we don't have a rate, just call the function directly.
	if k.rate == nil {
		loggerFn(ctx, msg, args)
		return
	}
