...
var logger = internallogger.GetLogger("juju.kubernetes.provider.application")
	...
	for _, p := range pods {
		...
		for _, volMount := range p.Spec.Containers[0].VolumeMounts {
			...
			if vol.Secret != nil &&
				...
				logger.Tracef(ctx, "ignoring volume source for service account secret: %v", vol.Name)
				continue
			}
			if vol.Projected != nil {
				logger.Tracef(ctx, "ignoring volume source for projected volume: %v", vol.Name)
				continue
			}
			if vol.ConfigMap != nil {
				logger.Tracef(ctx, "ignoring volume source for configMap volume: %v", vol.Name)
				continue
			}
			if vol.HostPath != nil {
				logger.Tracef(ctx, "ignoring volume source for hostPath volume: %v", vol.Name)
				continue
			}
			if vol.EmptyDir != nil {
			...
			logger.Tracef(ctx, "filesystem info for %v: %+v", volMount.Name, *fsInfo)
...
func (a *app) configureStorage(
	...
	logger.Tracef(ctx, "persistent volume claim name mapping = %v", pvcNames)
	...
	for index, fs := range filesystems {
		...
		logger.Debugf(ctx, "%s has filesystem %s: %s", a.name, fs.StorageName, pretty.Sprint(fs))
		...
		pvcNameGetter := func(volName string) string {
			if n, ok := pvcNames[volName]; ok {
				logger.Debugf(ctx, "using existing persistent volume claim %q (volume %q)", n, volName)
				return n
			}
		...
		if vol != nil && handleVolume != nil {
			logger.Debugf(ctx, "using volume for %s filesystem %s: %s", a.name, fs.StorageName, pretty.Sprint(*vol))
		...
		if sc != nil && handleStorageClass != nil {
			logger.Debugf(ctx, "creating storage class for %s filesystem %s: %s", a.name, fs.StorageName, pretty.Sprint(*sc))
		...
		if pvc != nil && handlePVC != nil {
			logger.Debugf(ctx, "using persistent volume claim for %s filesystem %s: %s", a.name, fs.StorageName, pretty.Sprint(*pvc))
