...
func (s *K8sBrokerSuite) TestEnsureServiceWithServiceAccountCreateNewClusterRole(c *gc.C) {
	ctrl := s.setupController(c)
	defer ctrl.Finish()
...
func (s *K8sBrokerSuite) TestEnsureServiceWithServiceAccountRefToExistingClusterRole(c *gc.C) {
	ctrl := s.setupController(c)
	defer ctrl.Finish()
